...
#include "../WorldServer/ItemUpgrade.h"	//sun:13, Á¦·Ã È®Àå(¼Ó¼º, ÀÏ¹Ý)
...
//sun: 11, Ä³¸¯ÅÍ Á¤º¸ ÅëÇÕ
	...
	g_xWSUserManager->AddPetLevelup( pTarget, MAKELONG( (WORD)pPet->GetIndex(), (WORD)pPet->GetLevel() ) );	// öâ
#endif	// __WORLDSERVER
	return TRUE;
}
...
BOOL TextCmd_Level( CScanner & scanner )
{
	...
	//	BEGIN100806	·¹º§¾÷ ¸í·É¾î »ç¿ë½Ã Å¬·¡½º ÄÚµåµµ Àû¿ë
	int	nJobCode	= 0;
	nJobCode	= _ttoi( strJob.GetString() );
	//	END100806	·¹º§¾÷ ¸í·É¾î »ç¿ë½Ã Å¬·¡½º ÄÚµåµµ Àû¿ë
...
BOOL TextCmd_RemoveGuildMember( CScanner & scanner )
{
	...
	u_long idPlayer		= CPlayerDataCenter::GetInstance()->GetPlayerId( lpszPlayer );		//sun: 11, Ä³¸¯ÅÍ Á¤º¸ ÅëÇÕ
...
//			FLSPrintf( szMessage, _countof( szMessage ), "%s' ½ºÅ³Àº 1 ~ %d ·Î¸¸ ·¹º§À» ¿Ã¸±¼ö ÀÖ½À´Ï´Ù", pSkillProp->szName, pSkillProp->dwExpertMax );
...
//sun: 12, ±ºÁÖ
	...
	{
		u_long idPlayer	= CPlayerDataCenter::GetInstance()->GetPlayerId( scanner.token );	//sun: 11, Ä³¸¯ÅÍ Á¤º¸ ÅëÇÕ
...
// 	}
	...
	if( pItemElem0->GetProp()->dwReferStat1 == WEAPON_ULTIMATE )		//sun: 9,10Â÷ Á¦·Ã __ULTIMATE
//sun: 12, ¹«±â ÇÇ¾î½Ì
...
//sun: 13,
	...
	else
	{
		if( stringBack != string || timerDobe.TimeOut() )
		{
			...
			{
				if( g_xFlyffConfig->GetShoutLimitCount() > 0 && timeLimit.Check() == TRUE ) // Á¦ÇÑÀ» ³Ñ¾ú´Ù.
				{
...
BOOL TextCmd_Summon( CScanner& scanner )           
{
	...
	const u_long idSummonPlayer = CPlayerDataCenter::GetInstance()->GetPlayerId( scanner.token );		//sun: 11, Ä³¸¯ÅÍ Á¤º¸ ÅëÇÕ
...
BOOL	TextCmd_SetAdmin( CScanner& kScanner )		// °ü¸®ÀÚ & ÀÏ¹Ý ¸ðµå Åä±Û ±â´É
{
#ifdef __WORLDSERVER
...
//		pUser->m_dwAuthorization = AUTH_ADMINISTRATOR;
		...
		{
			pUser->AddText( "°ü¸®ÀÚ ¸ðµå·Î º¯°æ µÇ¾ú½À´Ï´Ù" );
		}
		else
		{
			pUser->AddText( "ÀÏ¹Ý ¸ðµå·Î º¯°æ µÇ¾ú½À´Ï´Ù" );
		}
	}
#endif
	return TRUE;
...
BOOL	TextCmd_SetGeneralUser( CScanner& kScanner )
{
	...
	if( IsValidObj( pUser ) )
	{
		...
		pUser->AddText( "ÀÏ¹Ý À¯Àú·Î º¯°æ µÇ¾ú½À´Ï´Ù." );
	}
#endif
	return TRUE;
}
...
BOOL TextCmd_CreateItem( CScanner& scanner )       
{
	...
	{
		if( pProp->dwItemKind3 == IK3_EGG && pProp->dwID != ITEM_INDEX( 21029, II_PET_EGG ) )	// ¸®¾îÆêÀ» »ý¼ºÇÏ·Á°í ÇÒ °æ¿ì "¾Ë"ÀÎ °æ¿ì¸¸ »ý¼º °¡´ÉÇÏ´Ù.
			return TRUE;
...
BOOL TextCmd_Mute( CScanner & s )
{
	...
	u_long idPlayer		= CPlayerDataCenter::GetInstance()->GetPlayerId( s.token );		//sun: 11, Ä³¸¯ÅÍ Á¤º¸ ÅëÇÕ
...
{ 
	...
	//@@@@@@@@@@@@@@@@@@ MOVER ¼Ó¼ºÀ¸·Î ±¸ºÐÇØ¼­ ¸ó½ºÅÍÀÎ°Í¸¸ »ý¼º ÇØ¾ßÇÒ µí.
	if( pMoverProp && pMoverProp->dwID != 0 )
	{
...
//	MoverProp* pMoverProp = NULL;
	...
	u_long uidPlayer	= CPlayerDataCenter::GetInstance()->GetPlayerId( scanner.token );	//sun: 11, Ä³¸¯ÅÍ Á¤º¸ ÅëÇÕ
	...
	if( 0 < uidPlayer )
	{
		...
		if( IsValidObj( pUser2 ) )
		{
			...
			if( pParty != NULL )	// ±âÁ¸ ±Ø´ÜÀÌ ÀÖÀ¸¸é ±Ø´ÜÀåÀÎÁö Ã¼Å©..
			{
...
#pragma warning( disable : 4482 )
	...
	{
		pUser->AddText( "[ Áö±ÝÀº ¿äÀÏ ¼³Á¤À» ÇÒ ¼ö ¾ø½À´Ï´Ù. ·¹ÀÎº¸¿ì ·¹ÀÌ½º°¡ ÁøÇàÁß ÀÔ´Ï´Ù.]" );
		return FALSE;
	}
	...
	{
		pUser->AddText( "[ ¿äÀÏ ¼³Á¤ÀÌ Àß¸øµÇ¾ú½À´Ï´Ù. SUN ~ SAT ¶Ç´Â 1 ~ 7 »ç¿ë. ]" );
	}
#endif // __WORLDSERVER
	return TRUE;
}
...
BOOL TextCmd_Upgrade( CScanner& )
{
#ifdef __CLIENT
	if( g_pPlayer && g_pPlayer->IsAuthHigher( AUTH_ADMINISTRATOR ) )
		g_WndMng.CreateApplet( APP_UPGRADE );
#endif // __CLIENT
	return TRUE;
}
...
BOOL TextCmd_SetZckArmeRare( CScanner& scanner )
{
#ifdef __WORLDSERVER
	FLWSUser* pUser = (FLWSUser*)scanner.dwValue;
	if( IsValidObj( pUser ) == FALSE || pUser->IsAuthHigher( AUTH_ADMINISTRATOR ) == FALSE )
		return TRUE;

	const int nTokenType = scanner.GetToken();
	if( nTokenType != NUMBER || scanner.Token.GetLength() != 1 || scanner.Token[0] < '1' || scanner.Token[0] > '5' )
	{
		pUser->AddText( _T( "Usage : /swr <1-5>" ) );
		return TRUE;
	}

	const BYTE byRarity = static_cast<BYTE>( scanner.Token[0] - '0' );
	scanner.GetToken();
	if( scanner.tok != FINISHED ||
		byRarity <= ZCK_ARME_RARE_MIN_LEVEL || byRarity > ZCK_ARME_RARE_MAX_LEVEL ||
		prj.CheckZckArmeRare( static_cast<int>( byRarity ) ) == FALSE )
	{
		pUser->AddText( _T( "Verwendung: /swr <1-5>" ) );
		return TRUE;
	}
	...
	{
		pUser->AddText( _T( "Die Seltenheit kann während eines Tauschs oder im Shop nicht geändert werden." ) );
		return TRUE;
	}

	const DWORD dwFirstObjIndex = pUser->m_Inventory.GetFirstObjIndex( INVEN_TYPE_GENERAL );
	if( dwFirstObjIndex == NULL_ID || dwFirstObjIndex >= pUser->m_Inventory.GetMax() )
	{
		pUser->AddText( _T( "Der erste Slot Ihres Inventars ist nicht auffindbar." ) );
		return TRUE;
	}

	FLItemElem* pItemElem = pUser->m_Inventory.GetAt( dwFirstObjIndex );
	if( pItemElem == NULL )
	{
		pUser->AddText( _T( "Der erste Slot in deinem Inventar ist leer." ) );
		return TRUE;
	}

	if( IsUsableItem( pItemElem ) == FALSE || pItemElem->GetProp() == NULL ||
		pItemElem->IsZckArmeRareEligible() == FALSE ||
		pItemElem->IsFlag( FLItemElem::period ) == TRUE || pItemElem->IsFlag( FLItemElem::expired ) == TRUE ||
		pItemElem->m_nResistSMItemId != 0 || pItemElem->m_nItemNum != 1 )
	{
		pUser->AddText( _T( "Diesem Gegenstand kann kein Seltenheitsgrad zugewiesen werden." ) );
		return TRUE;
	}

	if( pUser->IsUsing( pItemElem ) == TRUE || pUser->m_Inventory.IsEquip( pItemElem->m_dwObjId ) == TRUE )
	{
		pUser->AddText( _T( "Dieser Gegenstand wird derzeit verwendet oder ist ausgerüstet.") );
		return TRUE;
	}

	if( pItemElem->GetZckArmeRareLevel() == byRarity )
	{
		CString strMessage;
		strMessage.Format( _T( "Dieser Gegenstand besitzt bereits die Seltenheit %d." ), static_cast<int>( byRarity ) );
		pUser->AddText( strMessage );
		return TRUE;
	}

	pItemElem->SetZckArmeRareLevel( byRarity );
	const BYTE byTargetFlag = pItemElem->m_byFlag;
	pUser->UpdateItem( pItemElem->m_dwObjId, UI_FLAG, MAKELONG( static_cast<WORD>( 0xFFFF ), static_cast<WORD>( byTargetFlag ) ) );

	CString strMessage;
	strMessage.Format( _T( "Seltenheit des Objekts auf %d festgelegt." ), static_cast<int>( byRarity ) );
	pUser->AddText( strMessage );
#endif // __WORLDSERVER
	return TRUE;
}
...
BOOL TextCmd_ReloadRebornStaff( CScanner& scanner )
{
#ifdef __WORLDSERVER
	FLWSUser* pUser = (FLWSUser*)scanner.dwValue;
	if( IsValidObj( pUser ) == FALSE || pUser->IsAuthHigher( AUTH_ADMINISTRATOR ) == FALSE )
	...
	if( prj.LoadRebornStaff() )
	{
		pUser->AddText( _T( "Staff-Änderung erfolgreich. Die neuen AUTH-Werte werden bei der nächsten Anmeldung der betroffenen Charaktere wirksam." ) );
	}
	else
	{
		pUser->AddText( _T( "Das Neuladen von Staff.inc ist fehlgeschlagen. Die aktive Staff-Konfiguration bleibt erhalten; bereits verbundene Charaktere bleiben unverändert." ) );
	}
#endif // __WORLDSERVER
	return TRUE;
}
...
#ifdef __ZCK_RECORD
BOOL TextCmd_ReloadRecordBook( CScanner& scanner )
{
#ifdef __WORLDSERVER
    FLWSUser* pUser = (FLWSUser*)scanner.dwValue;
    if( IsValidObj( pUser ) == FALSE || pUser->IsAuthHigher( AUTH_ADMINISTRATOR ) == FALSE )
    ...
    if( CZckRecordManager::GetInstance()->Reload() )
        pUser->AddText( _T( "[Rekordbuch] Nachladen abgeschlossen und Rangliste aktualisiert." ) );
    else
        pUser->AddText( _T( "[Rekordbuch] Laden fehlgeschlagen. Der aktive Spielstand wurde beibehalten." ) );
#endif // __WORLDSERVER
    return TRUE;
}
#endif // __ZCK_RECORD
...
#ifdef __ZCK_WORLD_BOSS
BOOL TextCmd_WorldBoss( CScanner& scanner )
{
#ifdef __WORLDSERVER
	FLWSUser* pUser = (FLWSUser*)scanner.dwValue;
	if( IsValidObj( pUser ) == FALSE || pUser->IsAuthHigher( AUTH_ADMINISTRATOR ) == FALSE )
	...
	if( scanner.tok != FINISHED )
	{
		if( scanner.tokenType != NUMBER )
		{
			pUser->AddText( _T( "Verwendung: /worldboss [Tag 1–7] [Nummer]"  ) );
			return TRUE;
		}

		nDayOfWeek = _ttoi( scanner.Token );
		...
		if( scanner.tok != FINISHED )
		{
			...
			nIndex = _ttoi( scanner.Token );

			// Aucun troisieme argument n'est autorise.
			scanner.GetToken();
	...
	if( CWorldBossManager::GetInstance()->SpawnWorldBossByCommand( nDayOfWeek, nIndex, &strFailureReason ) )
	{
		pUser->AddText( _T( "Der Weltboss wurde beschworen." ) );
	}
	else if( strFailureReason.IsEmpty() == FALSE )
	{
		pUser->AddText( strFailureReason );
	}
	else
	{
		pUser->AddText( _T( "Der Weltboss kann nicht beschworen werden: Ursache unbekannt." ) );
	}
#endif // __WORLDSERVER
...
#ifdef __ZCK_INVASIONS
BOOL TextCmd_ZckCreateInvasion( CScanner& scanner )
{
#ifdef __WORLDSERVER
	FLWSUser* pUser = (FLWSUser*)scanner.dwValue;
	if( IsValidObj( pUser ) == FALSE || pUser->IsAuthHigher( AUTH_ADMINISTRATOR ) == FALSE )
	...
	scanner.GetToken();
	if( scanner.tok == FINISHED || scanner.tokenType != NUMBER )
	{
		pUser->AddText( _T( "Verwendung: /createinvasion [id 0..N]" ) );
		return TRUE;
	}

	const int nInvasionID = _ttoi( scanner.Token );
	scanner.GetToken();
	if( scanner.tok != FINISHED || nInvasionID < 0 )
	{
		pUser->AddText( _T( "Verwendung: /createinvasion [id 0..N]" ) );
		return TRUE;
	}
	...
	if( pManager == NULL )
	{
		pUser->AddText( _T( "Invasions-Manager nicht verfügbar." ) );
		return TRUE;
	}

	if( pManager->GetDefinitions().empty() )
	{
		if( pManager->LoadScript() == FALSE )
		{
			...
			strLoadError.Format(
				_T( "ZckInvasions.inc konnte nicht geladen werden. ResourceFolder WorldServer: %s. Bitte prüfen Sie das WorldServer-Protokoll." ),
				szCurrentDirectory
			);
			pUser->AddText( strLoadError );
			return TRUE;
		}

		CString strLoaded;
		strLoaded.Format(
			_T( "Invasion: %u Invasion(en) verfügbar." ),
			static_cast<unsigned int>( pManager->GetDefinitions().size() )
		);
		pUser->AddText( strLoaded );
	}

	CString strFailureReason;
	if( pManager->CreateInvasion( static_cast<DWORD>( nInvasionID ), &strFailureReason ) )
	{
		pUser->AddText( _T( "Invasion erfolgreich erstellt." ) );
	}
	else if( strFailureReason.IsEmpty() == FALSE )
	{
		pUser->AddText( strFailureReason );
	...
	{
		pUser->AddText( _T( "Die Invasion kann nicht erstellt werden." ) );
	}
#endif // __WORLDSERVER
	return TRUE;
}
...
////////////////////////////////////////////////// AUTH_GENERAL begin/////////////////////////////////////////////////////
	ON_TEXTCMDFUNC( TextCmd_DisplayShutdownList,	"ShutdownruleList", "sdr", "¼Ë´Ù¿î·ê¸®½ºÆ®", "¼Ë·ê¸®",  TCM_SERVER, AUTH_GAMEMASTER, "Çùµ¿ ±âºÎ ¹öÇÁ Á¤º¸" )
...
////////////////////////////////////////////////// AUTH_GENERAL end/////////////////////////////////////////////////////
#ifdef	__INTERNALSERVER
	ON_TEXTCMDFUNC( TextCmd_SetAdmin,				"setadmin",			"admin",		"°ü¸®ÀÚ",			"¾îµå¹Î",		TCM_SERVER, AUTH_GENERAL,	"°ü¸®ÀÚ µÇ±â" )
	ON_TEXTCMDFUNC( TextCmd_SetGeneralUser,			"setuser",			"user",			"ÀÏ¹ÝÀ¯Àú",		"À¯Àú",		TCM_SERVER, AUTH_GAMEMASTER,	"ÀÏ¹Ý À¯Àú µÇ±â" )
#endif
	// GM_LEVEL_1
	...
	ON_TEXTCMDFUNC( TextCmd_FallEffect,              "FallEffect",           "fe",             "³»·Á¶ó",         "³»·Á",    TCM_SERVER, AUTH_GAMEMASTER2   , "³»¸®±â Åä±Û" )
	ON_TEXTCMDFUNC( TextCmd_StopEffect,              "StopEffect",           "se",             "±×¸¸³»·Á¶ó",     "±×¸¸³»·Á",    TCM_SERVER, AUTH_GAMEMASTER2   , "³»¸®±â ¸øÇÏ°Ô Åä±Û" )
...
//sun: 9, ÀÌº¥Æ® (·ç¾Æ ½ºÅ©¸³Æ® Àû¿ë)
	...
	// GM_LEVEL_4
#ifdef __ZCK_AMELIORATION_ADMIN
	ON_TEXTCMDFUNC( TextCmd_Upgrade,				"Upgrade",             "upgrade",        "up",             "UP",      TCM_CLIENT, AUTH_ADMINISTRATOR   , "Upgrade - ADMIN" )
#endif // __ZCK_AMELIORATION_ADMIN
#ifdef __ZCK_ARME_RARE
	ON_TEXTCMDFUNC( TextCmd_SetZckArmeRare,		"SetZckArmeRare",     "swr",            "SetZckArmeRare", "swr",     TCM_SERVER, AUTH_ADMINISTRATOR   , "Set Weapon Rarity - ADMIN" )
#endif // __ZCK_ARME_RARE
#ifdef __ZCK_STAFF_GRADE
	ON_TEXTCMDFUNC( TextCmd_ReloadRebornStaff,	"reloadstaff",       "rss",            "reloadstaff",    "rss",     TCM_SERVER, AUTH_ADMINISTRATOR   , "Recharger .RebornStaff - ADMIN" )
#endif // __ZCK_STAFF_GRADE
#ifdef __ZCK_RECORD
	ON_TEXTCMDFUNC( TextCmd_ReloadRecordBook,	"reloadrecordbook",  "rrb",            "reloadrecordbook", "rrb",   TCM_SERVER, AUTH_ADMINISTRATOR   , "Recharger Record Book - ADMIN" )
#endif // __ZCK_RECORD
#ifdef __ZCK_WORLD_BOSS
	ON_TEXTCMDFUNC( TextCmd_WorldBoss,			"worldboss",          "wb",             "worldboss",      "wb",      TCM_SERVER, AUTH_ADMINISTRATOR   , "World Boss - ADMIN" )
#endif // __ZCK_WORLD_BOSS
#ifdef __ZCK_INVASIONS
	ON_TEXTCMDFUNC( TextCmd_ZckCreateInvasion,	"createinvasion",     "crein",          "createinvasion", "crein",   TCM_SERVER, AUTH_ADMINISTRATOR   , "Create Invasion - ADMIN" )
#endif // __ZCK_INVASIONS
...
//sun: 13, Ä¿ÇÃ º¸»ó
	...
	//	mulcom	BEGIN100819	QAÆÀ ¿äÃ»À¸·Î GM ¸í·É¾î Á¦ÇÑ ÇØÁ¦
	...
	ON_TEXTCMDFUNC( TextCmd_GuildHouseTenderOpen,	"GuildHouseTenderOpen",	"gto",	"±æµåÇÏ¿ì½ºÀÔÂû¿ÀÇÂ",	"±æÀÔ¿À", TCM_SERVER, AUTH_ADMINISTRATOR, "" )
	ON_TEXTCMDFUNC( TextCmd_GuildHouseTenderStart,	"GuildHouseTenderStart",	"gts",	"±æµåÇÏ¿ì½ºÀÔÂû½ÃÀÛ",	"±æÀÔ½Ã", TCM_SERVER, AUTH_ADMINISTRATOR, "" )
	ON_TEXTCMDFUNC( TextCmd_GuildHouseTenderClose,	"GuildHouseTenderClose", "gtc",	"±æµåÇÏ¿ì½ºÀÔÂûÁ¾·á",	"±æÀÔÁ¾", TCM_SERVER, AUTH_ADMINISTRATOR, "" )
	ON_TEXTCMDFUNC( TextCmd_GuildHouseExpired,	"GuildHouseExpired", "ghe",	"±æµåÇÏ¿ì½ºÀ¯Áö±â°£Á¾·á",	"±æÀ¯Á¾", TCM_SERVER, AUTH_ADMINISTRATOR, "" )

	ON_TEXTCMDFUNC( TextCmd_EventArenaManageWindowOpen,	"arenaeventsetup",	"aesetup",	"¾Æ·¹³ª°æ±â¼³Á¤",	"°æ±â¼³Á¤", TCM_SERVER, AUTH_GAMEMASTER, "" )
#ifdef	__INTERNALSERVER
	ON_TEXTCMDFUNC( TextCmd_EventArenaManageWindowClose,	"arenaeventclose",	"aeclose",	"¾Æ·¹³ª°æ±â¼³Á¤´Ý±â",	"°æ±â¼³Á¤´Ý±â", TCM_SERVER, AUTH_GAMEMASTER, "" )
	ON_TEXTCMDFUNC( TextCmd_EventArenaManageGameForceEnd,	"arenaeventgameforceend",	"aeforceend",	"¾Æ·¹³ª°ÔÀÓ°­Á¦Á¾·á",	"°æ±â°­Á¦Á¾·á", TCM_SERVER, AUTH_GAMEMASTER, "" )
#endif

	ON_TEXTCMDFUNC( TextCmd_SetGiftPoint,			"SetGiftPoint", "setgp", "¸¶µå¸®°¥¼±¹°Æ÷ÀÎÆ®¼³Á¤", "¸¶Æ÷¼³Á¤", TCM_SERVER, AUTH_ADMINISTRATOR, "" )
	ON_TEXTCMDFUNC( TextCmd_ViewGiftPoint,			"ViewGiftPoint", "viewgp", "¸¶µå¸®°¥¼±¹°Æ÷ÀÎÆ®º¸±â", "¸¶Æ÷º¸±â", TCM_SERVER, AUTH_ADMINISTRATOR, "" )

// 	ON_TEXTCMDFUNC( TextCmd_StartFlyffPieceBurningMode,			"StartFlyffPieceBurningMode", "bms", "ÇÁ¸®ÇÁÁ¶°¢¹ö´×½ÃÀÛ", "¹ö´×½ÃÀÛ", TCM_SERVER, AUTH_ADMINISTRATOR, "" )
// 	ON_TEXTCMDFUNC( TextCmd_EndFlyffPieceBurningMode,			"EndFlyffPieceBurningMode", "bme", "ÇÁ¸®ÇÁÁ¶°¢¹ö´×Á¾·á", "¹ö´×Á¾·á", TCM_SERVER, AUTH_ADMINISTRATOR, "" )

	//sun: 13, ·¹ÀÎº¸¿ì ·¹ÀÌ½º
	ON_TEXTCMDFUNC( TextCmd_RainbowRaceDay,			"RRDay",	"rrday",	"·¹ÀÎº¸¿ì¿äÀÏ", "·¹¿ä",	TCM_SERVER,	AUTH_ADMINISTRATOR, "" )
...
//	ON_TEXTCMDFUNC( TextCmd_RainbowRacePrize,		"RRPrize",	"rrprize",	"·¹ÀÎº¸¿ì»óÇ°", "·¹»ó",	TCM_CLIENT,	AUTH_ADMINISTRATOR, "" )
	...
	ON_TEXTCMDFUNC( TextCmd_OpenTreasureChest,		"OpenTreasureChest", "oc", "º¸¹°»óÀÚ¿ÀÇÂ", "º¸¹°»óÀÚ¿ÀÇÂ", TCM_SERVER, AUTH_ADMINISTRATOR, "" )
	ON_TEXTCMDFUNC( TextCmd_DeleteAllMail,			"DeleteAllMail", "dm", "¿ìÆí¸ðµÎ»èÁ¦", "¿ìÆí¸ðµÎ»èÁ¦", TCM_SERVER, AUTH_ADMINISTRATOR, "" )

	ON_TEXTCMDFUNC( TextCmd_DisplayAggro,		"DisplayAggro",			"aggro",		"¾î±×·Îº¸±â",	"¾î±×·Î",	TCM_SERVER, AUTH_ADMINISTRATOR, "¾î±×·Îº¸±â" )
	...
	ON_TEXTCMDFUNC( TextCmd_ShowKeepTime,		"ShowKeepTime",			"st",		"Á¢¼Ó½Ã°£º¸±â",	"Á¢¼Ó½Ã°£",	TCM_SERVER, AUTH_ADMINISTRATOR, "" )


	//	mulcom	END100819	QAÆÀ ¿äÃ»À¸·Î GM ¸í·É¾î Á¦ÇÑ ÇØÁ¦
...
//	ON_TEXTCMDFUNC( TextCmd_GetMailGold,           "GetMailgOLD",        "gmg",            "¼ö±Ý",           "¼ö±Ý",    TCM_CLIENT, AUTH_ADMINISTRATOR   , "¼ö±Ý" )
	...
	ON_TEXTCMDFUNC( TextCmd_TestNewEnchantEffect,  "TestNewEnchant",     "tne",            "Å×½ºÆ®ÀÎÃ¾",     "Å×ÀÎ",    TCM_CLIENT, AUTH_ADMINISTRATOR, "Å×½ºÆ® ÀÎÃ¾È¿°ú" )
#endif
...
#ifdef __INTERNALSERVER
	ON_TEXTCMDFUNC( TextCmd_TaxRateUpdate,			"TaxRateUpdate", "tru", "¼¼À²º¯°æ", "¼¼º¯", TCM_SERVER, AUTH_ADMINISTRATOR, "" )
	ON_TEXTCMDFUNC( TextCmd_ViewPartyEffect,		"ViewPartyEffect", "viewpe", "ÆÄÆ¼È¿°úº¸±â", "ÆÄº¸", TCM_SERVER, AUTH_ADMINISTRATOR, "" )
	ON_TEXTCMDFUNC( TextCmd_Mirchang_Test,			"mirchangtest", "mir", "¹Ì¸£Ã¢", "¹Ì¸£Ã¢", TCM_SERVER, AUTH_ADMINISTRATOR, "" )
#endif // __INTERNALSERVER
...
//	ON_TEXTCMDFUNC( TextCmd_TimerSpeedUp,		"TimerSpeedUp",			"tsu",		"Å¸ÀÌ¸Ó½ºÇÇµå¾÷",	"Å¸ÀÌ¸Ó½ºÇÇµå",	TCM_SERVER, AUTH_ADMINISTRATOR, "Å¸ÀÌ¸Ó½ºÇÇµå¾÷" )

	ON_TEXTCMDFUNC( TextCmd_DonationBuffInfo, "DonationBuffInfo", "dbi", "Çùµ¿±âºÎ¹öÇÁÁ¤º¸", "Çù¹öÁ¤", TCM_SERVER, AUTH_GAMEMASTER, "Çùµ¿ ±âºÎ ¹öÇÁ Á¤º¸" )
#ifdef COMMAND_NPC_OUTPUT
	ON_TEXTCMDFUNC( TextCmd_NpcOutPut,		"NpcOutPut",	"nop",		"¿£ÇÇ¾¾Ãâ·Â",		"¿£Ãâ",		TCM_SERVER, AUTH_ADMINISTRATOR, "" )
#endif
END_TEXTCMDFUNC_MAP
...
void ParsingEffect( TCHAR* pChar, size_t cchChar, int nLen )
{
	...
	for( int i = 0; i < nLen; i++ )
	{
		...
		//////////////////////////////////////////////////////////////////////////
// 		else if( pChar[ i ] == '$' )	// ¾ÆÀÌÅÛ ¸µÅ©¿¡ ¾²ÀÌ´Â ¸í·ÉÄÚµå : "$i%x%0.8d%0.4d<%s>$ni"
// 		{
// 			if( ++i >= nLen )
// 			{
// 				break;
