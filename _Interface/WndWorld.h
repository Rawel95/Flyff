...
class CWndWorld : public CWndNeuz
{
	...
	CPoint		m_ptZckPingFpsHudPos;
	CPoint		m_ptZckPingFpsDragOffset;
	BOOL		m_bZckPingFpsDragging;
	BOOL		m_bZckPingFpsHudPosInitialized;
#endif // __ZCK_PING_FPS_AND_TOOLTIP
#ifdef __ZCK_WORLD_BOSS
	CPoint		m_ptWorldBossHudPos;
	CPoint		m_ptWorldBossDragOffset;
	BOOL		m_bWorldBossDragging;
	BOOL		m_bWorldBossHudPosInitialized;
...
public:
...
#ifdef __ZCK_PING_FPS_AND_TOOLTIP
	bool GetZckPingFpsRect( CRect& outRect );
	bool IsZckPingFpsRect( CPoint point );
	bool BeginZckPingFpsDrag( CPoint point );
	void UpdateZckPingFpsDrag( UINT nFlags, CPoint point );
	void EndZckPingFpsDrag();
	void ClampZckPingFpsHudPosition();
	void DrawZckPingFps( C2DRender* p2DRender );
#endif // __ZCK_PING_FPS_AND_TOOLTIP
#ifdef __ZCK_WORLD_BOSS
	bool GetWorldBossRect( CRect& outRect );
	bool IsWorldBossRect( CPoint point );
	bool BeginWorldBossDrag( CPoint point );
	void UpdateWorldBossDrag( UINT nFlags, CPoint point );
	void EndWorldBossDrag();
	void ClampWorldBossHudPosition();
	void DrawWorldBoss( C2DRender* p2DRender );
#endif // __ZCK_WORLD_BOSS
	int  GetBuffTimeGap();
	...
	std::vector< __GuildCombatJoin > m_vecGuildCombatJoin;		//sun: 10, __S_BUG_GC
