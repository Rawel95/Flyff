...
void CWndBagEx::InitItem( void )
{
	...
	{
		if( g_pPlayer->m_Pocket.IsAvailable( nPocket ) == TRUE )
		{
			m_wndItemCtrl[nPocket].InitItem( &( g_pPlayer->m_Pocket.m_kPocket[nPocket] ), APP_CANCEL_BLESSING );

			if( nPocket > 0 )
			{
				m_bUse[nPocket - 1] = TRUE;
			}
		}
		else
		{
...
{ 
	...
	for( int i = 1 ; i < 3 ; ++i )
	{
		if( m_bUse[i - 1] == FALSE )
		{
			...
			if(m_pTexNouse) 
				m_pTexNouse->Render( p2DRender, lpWndCtrl->rect.TopLeft(), 150 );

			lpWndCtrl = GetWndCtrl( nIco[i - 1] );
			if(m_pTexIco_empty) 
				m_pTexIco_empty->Render( p2DRender, lpWndCtrl->rect.TopLeft(), 200 );
		}
		else
		{
			LPWNDCTRL lpWndCtrl = GetWndCtrl( nIco[i - 1] );
			if(m_pTexIco) 
				m_pTexIco->Render( p2DRender, lpWndCtrl->rect.TopLeft(), 200 );
		}
	}
} 
...
{ 
	...
	if( g_WndMng.GetApplet( APP_TRADE ) ||
		g_WndMng.GetWndBase( APP_POST_READ ) || 
		g_WndMng.m_pWndShop || 
		g_WndMng.m_pWndShop_Cart ||
		g_WndMng.m_pWndGuildBank || 
		g_WndMng.GetWndVendorBase() ||
		g_WndMng.GetWndBase( APP_COSTUME_MIXMENU )||
		g_WndMng.GetWndBase( APP_TREASURECHEST_OPEN )||
	...
	m_wndItemCtrl[0].InitItem( &( g_pPlayer->m_Pocket.m_kPocket[0] ), APP_CANCEL_BLESSING );

	m_wndItemCtrl[1].Create( WLVS_ICON, CRect( 0, 0, 250, 250 ), pTab1, 11 );
	m_wndItemCtrl[1].InitItem( &( g_pPlayer->m_Pocket.m_kPocket[1] ), APP_CANCEL_BLESSING );
	
	m_wndItemCtrl[2].Create( WLVS_ICON, CRect( 0, 0, 250, 250 ), pTab2, 11 );
	m_wndItemCtrl[2].InitItem( &( g_pPlayer->m_Pocket.m_kPocket[2] ), APP_CANCEL_BLESSING );
	...
	m_pTexNouse = m_textureMng.AddTexture( m_pApp->m_pd3dDevice,  MakePath( DIR_THEME, g_xFlyffConfig->GetMainLanguage(),"InventoryBagEff.tga" ), 0xffff00ff );
	m_pTexIco	= m_textureMng.AddTexture( m_pApp->m_pd3dDevice,  MakePath( DIR_THEME, g_xFlyffConfig->GetMainLanguage(),"Itm_SysSysScrBagBag01.bmp" ), 0xffff00ff );;
	m_pTexIco_empty = m_textureMng.AddTexture( m_pApp->m_pd3dDevice,  MakePath( DIR_THEME, g_xFlyffConfig->GetMainLanguage(),"Itm_SysSysScrBagBag02.bmp" ), 0xffff00ff );;
...
void CWndBagEx::OnMouseWndSurface( CPoint point )
{
	...
	if( DrawRect.PtInRect( point ) )
	{
		...
		{
			// 사용하면 시간을 보여준다
			time_t nRemain = g_pPlayer->m_Pocket.GetExpirationDate( 1 ) - time( NULL );
//			time_t nRemain = (g_pPlayer->m_Pocket.GetPocket(1))->GetExpirationDate() - time(NULL);
	...
	if( DrawRect.PtInRect( point ) )
	{
		...
		{
			// 사용하면 시간을 보여준다
			time_t nRemain = g_pPlayer->m_Pocket.GetExpirationDate( 2 ) - time( NULL );
