...
HRESULT CWndCredit::InitDeviceObjects()
{
	switch( g_xFlyffConfig->GetMainLanguage() )
	{
		...
		case LANG_RUS:
			{
				LANG_DATA const * pData	= CLangMan::GetInstance()->GetLangData( g_xFlyffConfig->GetMainLanguage() );
				m_pFont = new CD3DFont( pData->stFont.lfCaption.szFontFirst, 9 );
				break;
			}
		default:
...
//	p2DRender->RenderTexture( CPoint( 4, 8 ), &m_aTexScreenShot[ m_nTexCount ] );
	
	CRect rect = GetClientRect();
	p2DRender->RenderTextureEx( rect.TopLeft() + CPoint( 10, 12 ), rect.BottomRight() - CPoint( 17, 14 ), &m_aTexScreenShot[ m_nTexCount ], 255, 1.0f, 1.0f );

	CRect rect2 = GetClientRect();
	rect2.top += 12;
	ClientToScreen( &rect2 );
	p2DRender->SetViewport( rect2 );
...
{ 
	...
	// 여기에 코딩하세요
	{
		...
		if( g_xFlyffConfig->GetMainLanguage() != LANG_KOR )
			strfile = MakePath( DIR_THEME , g_xFlyffConfig->GetMainLanguage(), _T( "Openning.inc" ) );
		else
			strfile = "credit.txt";
	...
	m_timerScreenShot.Set( SEC( 10 ) );
 	...
 	if( g_xFlyffConfig->GetMainLanguage() == LANG_JAP )
 	{
 		for( int i = 0; i < MAX_SCREENSHOT_JAP; i++ )
 		{
 			...
 			m_aTexScreenShot[ i ].LoadTexture( m_pApp->m_pd3dDevice, MakePath( DIR_THEME , g_xFlyffConfig->GetMainLanguage(), string ), 0xffff00ff );
	...
	if( g_xFlyffConfig->GetMainLanguage() == LANG_JAP )
	{
		...
		//	100805	CSTRING GETBUFFER
		g_SoundMng.Play( filename.GetString(), NULL, NULL, FALSE );
		m_pSound = g_SoundMng.GetSound( (LPTSTR)filename.GetString() );
...
{ 
	...
	if( lpWndApplet )
	{
		SetTexture( m_pApp->m_pd3dDevice, MakePath( DIR_THEME , g_xFlyffConfig->GetMainLanguage(), strName ), TRUE );
	}
