...
{ 
	...
	// 성전환이니깐 반대로 하자~
	int nMover = (g_pPlayer->GetSex() == SEX_MALE ? OBJECT_INDEX( 12, MI_FEMALE ) : OBJECT_INDEX( 11, MI_MALE ));
...
void CWndItemTransy::Init( FLItemElem* pItemElem, BOOL bMenu )
{
	m_bMenu = bMenu;
	for( int i = 0 ; i < 2 ; ++i )
		m_pItemElem[i] = NULL;
...
{ 
	...
	if( m_Rect[0].PtInRect( Point ) )
	{
		...
		if( m_pItemElem[0] )
			g_WndMng.PutToolTip_Item( (FLItemBase*)m_pItemElem[0], Point2, &HitRect );
		else
	...
	if( m_Rect[1].PtInRect( Point ) )
	{
		...
		ClientToScreen( &HitRect );
		if( m_pItemElem[0] )
			g_WndMng.PutToolTip_Item( (FLItemBase*)&m_pItemElemChange, Point2, &HitRect );
		else
...
BOOL CWndItemTransy::OnDropIcon( LPSHORTCUT pShortcut, CPoint point )
{
	...
	if( PtInRect( &m_Rect[0], point ) )
	{
		...
		if(!m_bMenu)
		{
			if( m_pItemElem[1]->GetProp()->dwID == ITEM_INDEX( 26204, II_CHR_SYS_SCR_ITEMTRANSY_A ) )
			{
				if( 61 <= pItemElem->GetProp()->nMinLimitLevel )
				{
					CString strMsg;
			...
			{
				if( pItemElem->GetProp()->nMinLimitLevel < 61 )
				{
					CString strMsg;
		...
		m_pItemElemChange.m_nHitPoint	= ( pItemPropChange->dwEndurance == -1 ) ? 0 : pItemPropChange->dwEndurance;//pItemPropChange->dwEndurance;
	}
