...
//			if( pItemElem->GetProp()->dwItemKind3 != IK3_ELECARD && pItemElem->GetProp()->dwItemKind3 != IK3_ENCHANT )
			...
			if( pItemElem->GetProp()->dwItemKind3 == IK3_ELECARD && m_pItemElem[0]->m_byItemResist != SAI79::NO_PROP && m_pItemElem[0]->m_byItemResist != pItemElem->GetProp()->eItemType )
			{
...
//			if( pItemElem->GetProp()->dwItemKind3 == IK3_ENCHANT )
			if( pItemElem->GetProp()->dwItemKind3 == IK3_SMELT_ULTIMATE_MATERIAL || pItemElem->GetProp()->dwItemKind3 == IK3_SMELT_GENERAL_MATERIAL )
			{
				//*
			...
			if( pItemElem->GetProp()->dwItemKind3 == IK3_ELECARD )
			{
				...
				{
					case SAI79::FIRE:
						m_dwReqItem[0]  = ITEM_INDEX( 3201, II_GEN_MAT_SUP_ERONS );
						m_dwReqItem[1]  = ITEM_INDEX( 3203, II_GEN_MAT_SUP_GURU );
						break;
					case SAI79::WATER:
						m_dwReqItem[0]  = ITEM_INDEX( 3202, II_GEN_MAT_SUP_KRASEC );
						m_dwReqItem[1]  = ITEM_INDEX( 3203, II_GEN_MAT_SUP_GURU );
						break;
					case SAI79::EARTH:
						m_dwReqItem[0]  = ITEM_INDEX( 3200, II_GEN_MAT_SUP_MINERAL );
						m_dwReqItem[1]  = ITEM_INDEX( 3203, II_GEN_MAT_SUP_GURU );
						break;
					case SAI79::ELECTRICITY:
						m_dwReqItem[0]  = ITEM_INDEX( 3200, II_GEN_MAT_SUP_MINERAL );
						m_dwReqItem[1]  = ITEM_INDEX( 3202, II_GEN_MAT_SUP_KRASEC );
						break;
					case SAI79::WIND:
						m_dwReqItem[0]  = ITEM_INDEX( 3201, II_GEN_MAT_SUP_ERONS );
						m_dwReqItem[1]  = ITEM_INDEX( 3202, II_GEN_MAT_SUP_KRASEC );
			...
			{
				m_dwReqItem[0]  = ITEM_INDEX( 3200, II_GEN_MAT_SUP_MINERAL );
				m_dwReqItem[1]  = ITEM_INDEX( 3201, II_GEN_MAT_SUP_ERONS );
			}
			...
			str.Format( "%s", g_xSpecManager->GetSpecItem(m_dwReqItem[0])->szName );
			pWndStatic->SetTitle(str);

			pWndStatic = (CWndStatic*)GetDlgItem(WIDC_STATIC11);
			str.Format( "%s", g_xSpecManager->GetSpecItem(m_dwReqItem[1])->szName );
			pWndStatic->SetTitle(str);
