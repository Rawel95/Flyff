...
void CWndSummonAngel::OnDestroy()
{
	...
	if(m_pWndInventory != NULL)
		m_pWndInventory->CISetDieFlag_ItemCtrl( FALSE );		//왜 TRUE?
...
{ 
	...
	for(int i=0; i<MAX_MATDIE; i++)
	{
		if(m_MatDie[i].isUse)
		{
			...
			{
				pItemProp = g_xSpecManager->GetSpecItem( m_MatDie[i].pItemElem->GetProp()->dwID );
				if(pItemProp != NULL)
				{
					pTexture = CWndBase::m_textureMng.AddTexture( g_Neuz.m_pd3dDevice, MakePath( DIR_ITEM, pItemProp->szIcon), 0xffff00ff );
					if(pTexture != NULL)
...
BOOL CWndSummonAngel::OnDropIcon( LPSHORTCUT pShortcut, CPoint point )
{
	...
	{
		if( m_nSelecCtrl < 10 && (pItemElem->GetProp()->dwID == ITEM_INDEX( 2035, II_GEN_MAT_ORICHALCUM01 ) || pItemElem->GetProp()->dwID == ITEM_INDEX( 2082, II_GEN_MAT_ORICHALCUM01_1 )))
		{
		...
		else if( m_nSelecCtrl > 9 && m_nSelecCtrl < 20 && (pItemElem->GetProp()->dwID == ITEM_INDEX( 2036, II_GEN_MAT_MOONSTONE ) || pItemElem->GetProp()->dwID == ITEM_INDEX( 2083, II_GEN_MAT_MOONSTONE_1 )))
		{
			if( pItemElem->m_nItemNum > pItemElem->GetExtra() )
				isValid = ITEM_VALID;
			else
...
void CWndSummonAngel::SetDie(FLItemElem* pItemElem)
{
