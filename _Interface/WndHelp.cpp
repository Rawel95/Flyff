...
void CWndHelp::OnDraw(C2DRender* p2DRender)
{
	...
	if( CS_VER1 == _GetContentState( CT_NEWUI_19 ) )
		p2DRender->TextOut( rect.left + 10, rect.top + 8, m_strKeyword, WNDFONTCOLOR_BLACK );
	if( CS_VER2 == _GetContentState( CT_NEWUI_19 ) )
		p2DRender->TextOut( rect.left + 10, rect.top + 8, m_strKeyword, WNDFONTCOLOR_WHITE );
}
...
{ 
	...
	m_nPosString = 0;
	pWndText->SetString( m_aString.GetAt( 0 ), m_dwColor );
} 
	...
	case WIDC_NEXT:
		{
			...
			if( m_nPosString == m_aString.GetSize() )
				m_nPosString = 0;
			CWndText* pWndText = (CWndText*)GetDlgItem( WIDC_TEXT );
			pWndText->SetString( m_aString.GetAt( m_nPosString ), m_dwColor);
		}
		break;
	case WIDC_CLOSE:
		Destroy();
