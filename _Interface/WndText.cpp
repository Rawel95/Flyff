...
{ 
	...
	if( pItemProp->dwQuestId == 0 || pItemElem->m_byItemResist == TRUE )
...
BOOL CWndTextBook::Initialize( CWndBase* pWndParent, FLItemBase* pItemBase )
{ 
...
*/
void CWndTextBook::SetItemBase( FLItemBase* pItemBase ) 
{
	m_pItemBase = pItemBase;
}
...
// 처음 이 함수를 부르면 윈도가 열린다.
BOOL CWndTextScroll::Initialize( CWndBase* pWndParent, FLItemBase* pItemBase )
{ 
	m_pItemBase = pItemBase;
	// Daisy에서 설정한 리소스로 윈도를 연다.
...
*/
void CWndTextScroll::SetItemBase( FLItemBase* pItemBase ) 
{
	m_pItemBase = pItemBase;
}
...
// 처음 이 함수를 부르면 윈도가 열린다.
BOOL CWndTextLetter::Initialize( CWndBase* pWndParent, FLItemBase* pItemBase )
{ 
	m_pItemBase = pItemBase;
	// Daisy에서 설정한 리소스로 윈도를 연다.
...
*/
void CWndTextLetter::SetItemBase( FLItemBase* pItemBase ) 
{
	m_pItemBase = pItemBase;
}
