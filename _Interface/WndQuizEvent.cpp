...
void CWndQuizEventQuestionOX::UpdateQuestion( const char* lpszQuestion )
{
	...
	if( lpszQuestion != NULL )
		pText->SetString( lpszQuestion, m_dwColor );
	else
		pText->SetString( CQuiz::GetInstance()->m_strQuestion.c_str(), m_dwColor );
}
...
{ 
	...
	m_bLoadTexMap = m_BtnTexture.LoadScript( g_Neuz.m_pd3dDevice, MakePath( DIR_THEME, g_xFlyffConfig->GetMainLanguage(),_T( "texMapQuizEventButton.inc" ) ) );
} 
