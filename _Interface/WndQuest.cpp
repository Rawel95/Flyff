...
void CWndQuest::Update( int nNewQuestId ) 
{
	...
	{
		for( size_t i = 0; i < g_pPlayer->m_nCompleteQuestSize; ++i )
		{
...
{
	for( size_t i = 0; i < g_pPlayer->m_nCheckedQuestSize; ++i )
	{
			...
			for( int i = 0; i < GetTreeItemArray()->GetSize(); ++i )
			{
				...
				if( GetMemberCheckingMode() == TRUE )
				{
					...
					if( pWndCheckBox && ( CWndButton* )pLResult == pWndCheckBox )
					{
						...
						{
							if( g_pPlayer->m_nCheckedQuestSize >= static_cast<size_t>( GetMaxCheckNumber() ) )
							{
...
void CWndQuestDetail::OnInitialUpdate( void )
{
	...
	if( pWndQuest )
	{
		CRect rectQuest = pWndQuest->GetWindowRect(TRUE);
...
void CWndQuestDetail::UpdateQuestDetailText( DWORD dwQuestID, LPQUEST lpQuest, BOOL bComplete, BOOL bClick )
{
	...
	if( bComplete )
	{
		for( int i = QS_END; i >= 0; --i )
		{
			...
			if( pQuestState )
			{
				pTextDesc->SetString( pQuestState->m_szDesc );
				break;
			}
		}
	}
	...
	CString strTemp = _T( "" );
	PFONTCOLOR_WNDQUESTDETAIL pFontColorWndQuestDetail = g_WndFontColorManager->GetWndQuestDetail();

	if( bComplete )
	{
		DWORD dwColor( pFontColorWndQuestDetail->m_stQuestConditionComplete.GetFontColor() );
		DWORD dwSelectColor( pFontColorWndQuestDetail->m_stQuestConditionComplete.GetFontColor() );
		
		pWndQuestConditionTree->InsertItem( NULL, GETTEXT( TID_QUEST_COMPLETED ), NULL, TRUE, FALSE, dwColor, dwSelectColor );
	}
	else
	{
		if( pFontColorWndQuestDetail )
		{
			DWORD dwStartColor( pFontColorWndQuestDetail->m_stQuestCondition.GetFontColor() );
			DWORD dwEndColor( pFontColorWndQuestDetail->m_stQuestCondition.GetFontColor() );
			DWORD dwSelectColor( pFontColorWndQuestDetail->m_stQuestConditionSelect.GetFontColor() );

			MakeQuestConditionItems( dwQuestID, pWndQuestConditionTree, bClick, NULL, dwStartColor, dwEndColor, dwSelectColor );
		}
	}
		...
		if( pEndRewardItem->m_nSex == -1 || pEndRewardItem->m_nSex == g_pPlayer->GetSex() )
		{
			if( pEndRewardItem->m_nType == 0 )
			{
				if( pEndRewardItem->m_nJobOrItem == -1 || pEndRewardItem->m_nJobOrItem == g_pPlayer->GetJob() )
				{
					...
					{
						PT_ITEM_SPEC pItemProp = g_xSpecManager->GetSpecItem( pEndRewardItem->m_nItemIdx );
	...
	if( pQuestProp->m_n64EndRewardExpMin )
	{
...
void MakeQuestConditionItems( DWORD dwQuestID, CWndTreeCtrl* pWndTreeCtrl, BOOL bClick, LPTREEELEM lpTreeElem, DWORD dwStartColor, DWORD dwEndColor, DWORD dwSelectColor )
{
	...
	if( NULL == lpQuest )
	{
		//gmpbigsun: 완료조건이 없는경우
		FLERROR_LOG( PROGRAM_NAME, _T( "lpQuest is NULL, dwQuestID %d" ), dwQuestID );
		return;
	}
	...
	if( NULL == pQuestProp )
	{
		FLERROR_LOG( PROGRAM_NAME, _T( "pQuestProp is NULL" ) );
		return;
	}
	...
	if( lpQuest->m_nState == ( i - 1 ) || lpQuest->m_nState == 14 )
	{
		...
		for( i = 0 ; i < MAX_QUEST_KILL_NPC_COUNT; ++i )
		{
			...
			if( pEndCondItem->m_nSex == -1 || pEndCondItem->m_nSex == g_pPlayer->GetSex() )
			{
				if( pEndCondItem->m_nType == 0 )
				{
					if( pEndCondItem->m_nJobOrItem == -1 || pEndCondItem->m_nJobOrItem == g_pPlayer->GetJob() )
					{
						...
						{
							PT_ITEM_SPEC pItemProp = g_xSpecManager->GetSpecItem( pEndCondItem->m_nItemIdx );
		...
		{
			SkillProp* pSkillProp = prj.GetSkillProp( pQuestProp->m_nEndCondSkillIdx );
		...
		if( pQuestProp->m_nEndCondGuild != 0 ) 
		{
			...
			if( pGuild != NULL )
			{
				...
				if( pGuild->IsMaster( g_pPlayer->m_idPlayer ) == TRUE )
				{
					bLeader = TRUE;
				}
			}
			...
			if( pEndCondOneItem->m_nSex == -1 || pEndCondOneItem->m_nSex == g_pPlayer->GetSex() )
			{
				if( pEndCondOneItem->m_nType == 0 )
				{
					if( pEndCondOneItem->m_nJobOrItem == -1 || pEndCondOneItem->m_nJobOrItem == g_pPlayer->GetJob() )
					{
						...
						{
							PT_ITEM_SPEC pItemProp = g_xSpecManager->GetSpecItem( pEndCondOneItem->m_nItemIdx );
...
DWORD SetQuestDestinationInformation( DWORD dwQuestID, DWORD dwGoalIndex )
{
	...
	for( i = 0; i < nEndCondItemNum; ++i )
	{
		...
		{
			// roQuestGoalData.m_nOwnMonster 가까운 놈 찾기
