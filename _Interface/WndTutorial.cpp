		...
		case WIDC_LIST1: // view ctrl
			{
				...
				std::map<int, TUTORIAL_STRING>::iterator iter = m_mapTutorial.find(pWndListBox->GetCurSel());
...
BOOL CWndTutorial::LoadTutorial(LPCTSTR lpszFileName)
{
	...
	while( s.tok != FINISHED )
	{
		...
		m_mapTutorial.insert(std::map<int, TUTORIAL_STRING>::value_type(nIndex, temp));
		s.GetToken();	// }
...
BOOL CWndTutorial::AddToList(int nIndex)
{
	...
	std::map<int, TUTORIAL_STRING>::iterator iter = m_mapTutorial.find(nIndex);
