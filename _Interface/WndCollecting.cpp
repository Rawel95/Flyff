...
//sun: 11, 채집 시스템
	...
	FLItemElem* pCollector = g_pPlayer->GetCollector();
	...
	if(pCollector)
	{
		...
		FLSPrintf(m_pbufText, _countof( m_pbufText ), "%d / %d (%d%%)", nBtryRemain, nMax, (int)(fRatio * 100));
...
//sun: 11, 채집 시스템
	...
	if(pCollector)
	{
		...
		m_pTexGauEmptyNormal = CWndBase::m_textureMng.AddTexture( g_Neuz.m_pd3dDevice, MakePath( DIR_THEME, g_xFlyffConfig->GetMainLanguage(),"GauEmptyNormal.bmp" ), 0xffff00ff );
		m_pTexGauFillNormal  = CWndBase::m_textureMng.AddTexture( g_Neuz.m_pd3dDevice, MakePath( DIR_THEME, g_xFlyffConfig->GetMainLanguage(),"GauEmptyNormal.bmp" ), 0xffff00ff );
		...
		*/
		FLSPrintf(m_pbufText, _countof( m_pbufText ), "%d / %d (%d%%)", nBtryRemain, nMax, nRatio * 100);
		...
		//mem_set(szTemp, 0, sizeof( szTemp ));
		char		szTemp[64] = { 0 };
		FLSPrintf(szTemp, _countof( szTemp ), "%d  /  5", pCollector->GetAbilityOption());
...
BOOL CCollectingWnd::SetButtonCaption(bool bIsStart)
{
	...
	FLERROR_ASSERT_LOG_RETURN( pWndButton, FALSE, _T("[윈도우 채집] 버튼 윈도우 포인터가 유효하지 않습니다.") );		
	...
	if( CS_VER1 == _GetContentState( CT_NEWUI_19 ) )
	{
		...
		else
		{
			if(g_xFlyffConfig->GetMainLanguage() == LANG_FRE)
				pWndButton->SetTexture(g_Neuz.m_pd3dDevice, MakePath( DIR_THEME, g_xFlyffConfig->GetMainLanguage(), _T( "ButOk2.bmp" ) ), TRUE);
			else
...
{ 
	...
	switch(nID)
	{
		case WIDC_BUTTON1 :
			...
			if(m_bIsCollecting)
			{
				if( g_pPlayer->m_pActMover->IsDie() == TRUE )
				{
					break;
				}
...
//sun: 11, 채집 시스템
	...
	if( DrawRect.PtInRect( point ) )
	{
		...
		strText = prj.GetText(TID_GAME_COLLECTOR_BETTERY_INFO);		//sun: 11,
	...
	if( DrawRect.PtInRect( point ) )
	{
		...
		strText = prj.GetText(TID_GAME_COLLECTOR_LEVEL_INFO);		//sun: 11,
