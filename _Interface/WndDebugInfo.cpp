...
extern FLLogPubLoggingDialog	g_cLogPubDialog;
#endif
...
{
    // Show frame rate
	p2DRender->TextOut( 2,  0 ,g_Neuz.m_strFrameStats , WNDFONTCOLOR_WHITE );
	p2DRender->TextOut( 2, 20 ,g_Neuz.m_strDeviceStats, WNDFONTCOLOR_WHITE );
	...
	if( g_pPlayer && g_pPlayer->GetWorld() )
	{
		...
		FLSPrintf( strDebug, _countof( strDebug ), _T("x:%6.3f y:%6.3f z:%6.3f %6.3f"), vPos.x, vPos.y, vPos.z,(float)g_pPlayer->GetAngle());
		p2DRender->TextOut( 2, 40, strDebug, WNDFONTCOLOR_WHITE );
		FLSPrintf( strDebug, _countof( strDebug ), _T("Objects:%d  Faces:%d   LandFace:%d %d" ), g_pPlayer->GetWorld()->m_nObjCullSize, g_nMaxTri,0, g_WndMng.m_pWndWorld->GetMouseMode() );
		p2DRender->TextOut( 2, 55, strDebug, WNDFONTCOLOR_WHITE );
	}
	else
	{
		FLSPrintf( strDebug, _countof( strDebug ), _T( "x:0.0 y:0.0 z:%0.0 0.0" ) );
		p2DRender->TextOut( 2, 40, strDebug, WNDFONTCOLOR_WHITE );
		FLSPrintf( strDebug, _countof( strDebug ), _T("Objects:0  Faces:0" ) );
		p2DRender->TextOut( 2, 55, strDebug, WNDFONTCOLOR_WHITE );
	}
	...
	if( g_pPlayer && g_pPlayer->IsAuthHigher( AUTH_GAMEMASTER ) )
	{
		...
		p2DRender->TextOut( 200, 260, string, WNDFONTCOLOR_WHITE );
	}
}
...
{ 
	...
	pWndButton = (CWndButton*)GetDlgItem( WIDC_DEBUG_DIALOG     );		pWndButton->SetCheck( g_WorldMng()->m_bViewDebugDialog 	  );
...
//		case 108: g_World.m_bViewName      = !g_World.m_bViewName      ; break;
		...
		case WIDC_ANIMATION: CObj::SetAnimate( !CObj::IsAnimate() ); break;
...
//			}
		...
		case WIDC_DEBUG_DIALOG:
			{
#ifdef _DEBUG
				g_WorldMng()->m_bViewDebugDialog = !g_WorldMng()->m_bViewDebugDialog;
				if( g_WorldMng()->m_bViewDebugDialog == TRUE )
				{
					g_cLogPubDialog.MoveUnderOfParent();
					g_cLogPubDialog.Show();
				}
				else
				{
					g_cLogPubDialog.Hide();
				}
#endif // _DEBUG
			}
			break;
