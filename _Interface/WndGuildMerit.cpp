...
{ 
	...
	FLIntToString( 0, szNumber, _countof( szNumber ), 10 );
	...
	case WIDC_CHECK2:	
		{
			...
			if( m_nItem )
			{
				if( g_pPlayer )
				{
					...
					for( DWORD i=0; i<g_pPlayer->m_Inventory.GetMax() ; i++ )
					{
...
void CWndGuildMeritCtrl::OnDraw( C2DRender* p2DRender )
{
	...
	PFONTCOLOR_WNDGUILDMETRICCTRL pFontColorWndGuildMetricCtrl = g_WndFontColorManager->GetWndGuildMetricCtrl();

	for( int i = pt.y; i < (int)( m_pItemContainer.size() ); i++ )
	{
		int x	= ( i - pt.x ) % 6;
		int y	= ( i - pt.y ) / 6;
		FLItemBase* pItemBase	= m_pItemContainer[i];
		if( pItemBase )
		{
			...
			if( pItemElem->GetProp()->dwPackMax > 1 )
			{
				...
				{
					p2DRender->TextOut( x * 32 + 32 - size.cx + 17,
										y * 32 + 32 - size.cy + 10, 
										szTemp, 
										pFontColorWndGuildMetricCtrl->m_stItemCount.GetFontColor(), 
										pFontColorWndGuildMetricCtrl->m_stItemCountShadow.GetFontColor() );
				}
			}
		}
	}
...
void CWndGuildMeritCtrl::AddItem(FLItemBase *pItemContainer)
{
...
void CWndGuildMeritCtrl::RestoreItem()
{
	...
	{
		pItemElem = (FLItemElem*)m_pItemContainer[i];
