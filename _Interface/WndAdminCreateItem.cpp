...
CString CWndAdminCreateItem::MakeName( PT_ITEM_SPEC pProp )
{
	CString szName;
	szName.Format( "%s (%s%d)", pProp->szName, prj.GetText(TID_SKILL_BUFFICONLEVEL01),pProp->nMinLimitLevel );
	return szName;
}
...
{ 
	...
	for( SpecItemIdItr it = g_xSpecManager->m_SpecItemIdMap.begin(); it != g_xSpecManager->m_SpecItemIdMap.end(); ++it )
	{
		...
		if( pItemProp )
		{
			if( g_xFlyffConfig->GetMainLanguage() != LANG_KOR && pItemProp->nVer >= 7 && pItemProp->bCharged == TRUE )
				continue;
...
{ 
	...
	if( nID == WIDC_ITEM_KIND || nID == WIDC_ITEM_SEX || nID == WIDC_ITEM_JOB || nID == WIDC_LEVEL )
	{
		if( message == WNM_SELCHANGE || message == EN_CHANGE )
		{
			...
			for( SpecItemIdItr it = g_xSpecManager->m_SpecItemIdMap.begin(); it != g_xSpecManager->m_SpecItemIdMap.end(); ++it )
			{
				...
				{
					if( pItemProp->nMinLimitLevel >= nLevel && ( pItemProp->dwItemJob == dwJob || dwJob == -1 ) )
					{
						if( g_xFlyffConfig->GetMainLanguage() != LANG_KOR && pItemProp->nVer >= 7 && pItemProp->bCharged == TRUE )
							continue;
						int nIndex = pListBox->AddString( MakeName( pItemProp ) );
	...
	if( nID == WIDC_OK || ( nID == WIDC_CONTROL1 && message == WNM_DBLCLK ) )
	{
		...
		else 
		{
			...
			if( nIndex != -1 ) 	
			{
				pItemProp = (PT_ITEM_SPEC)pListBox->GetItemDataPtr( nIndex );
				if( pItemProp )	
				{
