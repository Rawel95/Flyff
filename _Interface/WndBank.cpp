...
//sun: 11, 주머니
	...
	m_pTexture = m_textureMng.AddTexture( m_pApp->m_pd3dDevice,  MakePath( DIR_THEME, g_xFlyffConfig->GetMainLanguage(),"WndCommonBankNotUse.tga" ), 0xffff00ff );
} 

void CWndBank::ReSetBank( )
{
	const BYTE byProgramDataSlot	= GET_PLAYER_SLOT( g_pPlayer->m_nDBDataSlot );

	for( int i = 0 ; i < MAX_CHARACTER_SLOT ; ++i )
	{
		if( byProgramDataSlot == i )
		{
			bUse[i] = TRUE;
		}
		else
...
//---------------------------------------------------------------------------------------------
// 현재 선택 된 아이템 슬롯의 인덱스를 가져온다
// param	: void
// return	: void
// 설명		: 꼭 이렇게 포인터로 비교해야 하나? 그지같다... 바꿀 시간 없음 패스.
//---------------------------------------------------------------------------------------------
BYTE CWndBank::GetCurrentItemSlotIndex()
{
	if( m_pCurFocus == &m_wndItemCtrl[0] )
		return 0;
	else if( m_pCurFocus == &m_wndItemCtrl[1] )
		return 1;
	else if( m_pCurFocus == &m_wndItemCtrl[2] )
		return 2;
	else
		return 0;
...
//---------------------------------------------------------------------------------------------
// 현재 선택 된 돈 슬롯의 인덱스를 가져온다
// param	: void
// return	: void
// 설명		: 꼭 이렇게 포인터로 비교해야 하나? 그지같다... 바꿀 시간 없음 패스.
//---------------------------------------------------------------------------------------------
BYTE CWndBank::GetCurrentMoneySlotIndex()
{
	if( m_pCurFocus == &m_wndGold[0] )
		return 0;
	else if( m_pCurFocus == &m_wndGold[1] )
		return 1;
	else if( m_pCurFocus == &m_wndGold[2] )
		return 2;
	else
		return 0;
...
{ 
	if( nID == WIDC_OK || message == EN_RETURN )
	{
		...
		if( m_pPass )
		{
			...
			char szPass[10] ={0,};
			FLStrncpy( szPass, _countof( szPass ), m_pPass->GetString(), 10 );
			if( strlen( szPass ) == 4 )
			{
				BOOL bDigit = TRUE;
				for( int i = 0 ; i < 4 ; i++ )
...
{ 
	...
	if(	g_WndMng.GetWndBase( APP_BARUNA_NPC ))
	{
		Destroy();
		return;
	}
...
//			strncpy( szConfirmPass, m_pConfirmPass->GetString(), 10 );
			FLStrcpy( szLastPass, _countof( szLastPass ), m_pLastPass->GetString() );
			FLStrcpy( szNewPass, _countof( szNewPass ), m_pNewPass->GetString() );
			FLStrcpy( szConfirmPass, _countof( szConfirmPass ), m_pConfirmPass->GetString() );
