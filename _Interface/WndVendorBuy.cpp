...
CWndVendorBuy::CWndVendorBuy( FLItemBase* pItemBase, int iIndex )
{
...
void CWndVendorBuy::OnInitialUpdate()
{
	...
	char str[64]	= { 0, };
	FLSPrintf( str, _countof( str ), "%d", m_pItemBase->GetExtra() );
	...
	FLSPrintf( str, _countof( str ), "%d", m_pItemBase->m_nCost );
	pWndStatic->SetTitle( CString( str ) );
	pWndStatic->AddWndStyle(WSS_MONEY);

	pWndStatic	= (CWndStatic*)GetDlgItem( WIDC_STATIC5 );
	FLSPrintf( str, _countof( str ), "%d", m_pItemBase->m_nCost * m_pItemBase->GetExtra() );
	pWndStatic->SetTitle( CString( str ) );
	pWndStatic->AddWndStyle(WSS_MONEY);
...
BOOL CWndVendorBuy::OnChildNotify( UINT message, UINT nID, LRESULT* pLResult )
{
	...
	pWndStatic	= (CWndStatic*)GetDlgItem( WIDC_STATIC5 );
	FLSPrintf( strstr, _countof( strstr ), "%d", m_pItemBase->m_nCost * nNum );
...
//sun: 8, // __S8_VENDOR_REVISION
		...
		if( g_pPlayer->GetGold() < ((FLItemElem*)m_pItemBase)->m_nCost * nBufNum )
		{
