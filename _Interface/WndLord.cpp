...
void CWndLordSkill::OnInitialUpdate()
{
	...
	if( pLord )
	{
		...
		if( pLordSkill )
		{
			for(int i = 0; i < 8; ++i)
			{
				CLordSkillComponentExecutable* pComponent = pLordSkill->GetSkill(i);
				FLERROR_ASSERT_LOG_DO( pComponent, continue, _T("[윈도우 군주 스킬] 군주 스킬 컴포넌트 포인터가 유효하지 않습니다.") );
	...
	{
		FLERROR_LOG( PROGRAM_NAME, _T("[윈도우 군주 스킬] 군주 포인터가 유효하지 않습니다.") );
	}
...
void CWndLordSkill::OnMouseMove(UINT nFlags, CPoint point)
{
	...
	if(m_nCurSelect)
	{
		...
		CLordSkillComponentExecutable* pComponent = pLordSkill->GetSkill(m_nCurSelect - 1);
		...
		FLStrcpy( m_GlobalShortcut.m_szString, _countof( m_GlobalShortcut.m_szString ), pComponent->GetName());
	}
...
void CWndLordSkill::OnMouseWndSurface( CPoint point )
{
	...
	for(int i = 0; i < 8; ++i)
	{
		...
		{
			CCLord* pLord = CCLord::Instance();
			FLERROR_ASSERT_LOG_DO( pLord, continue, _T("[윈도우 군주 스킬] 군주 포인터가 유효하지 않습니다.") );

			CLordSkill* pLordSkill = pLord->GetSkills();
			FLERROR_ASSERT_LOG_DO( pLordSkill, continue, _T("[윈도우 군주 스킬] 군주 스킬 포인터가 유효하지 않습니다.") );
