...
CWndRepairItem::~CWndRepairItem() 
{
	for( int i = 0; i < MAX_REPAIRINGITEM; i++ )
	{
		if( m_adwIdRepair[i] != NULL_ID )
		{
			...
			pItemElem	= g_pPlayer->m_Inventory.GetAtId( m_adwIdRepair[i] );
			...
			if( pWndFrame->GetWndId() == APP_INVENTORY )
			{
				...
				if( pItemBase )
				{
					if( g_pPlayer->m_Inventory.IsEquip( pItemBase->m_dwObjId ) == FALSE )
					{
						...
						if( pItemProp )
						{
							if( pItemProp->dwItemKind2 >= IK2_WEAPON_HAND && pItemProp->dwItemKind2 <= IK2_ARMORETC && ((FLItemElem*)pItemBase)->m_nRepair < pItemProp->nMaxRepair )
							{
								...
								if( pItemElem->m_nHitPoint < (int)( pItemProp->dwEndurance ) )
								{
									...
									if( g_pPlayer->GetGold() >= (int)( dwSumCost ) )
									{
										...
										FLSPrintf( szCost, _countof( szCost ), "%-d", m_dwCost );
