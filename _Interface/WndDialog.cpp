...
{ 
	...
	{
		PFONTCOLOR_WNDDIALOG pFontColorWndDialog = g_WndFontColorManager->GetWndDialog();
		
		DWORD dwColor = ( i == m_nSelectKeyButton ) ? pFontColorWndDialog->m_stKeyButtonDetault.GetFontColor() : pFontColorWndDialog->m_stKeyButtonDetault.GetFontColor();
		WORDBUTTON* pKeyButton = &m_aKeyButton[ i ];
		if( pKeyButton->bStatus )
		{
			dwColor = pFontColorWndDialog->m_stKeyButtonRollOver.GetFontColor();
			CSize size = p2DRender->m_pFont->GetTextExtent( pKeyButton->szWord );
			p2DRender->RenderLine( CPoint( pKeyButton->rect.left, pKeyButton->rect.top + size.cy ),
			CPoint( pKeyButton->rect.left + size.cx, pKeyButton->rect.top + size.cy ), dwColor );
...
{ 
	...
	if( pWndGroupBox )
	{
		...
		pWndGroupBox->m_dwColor = pFontColorWndDialog->m_stGroupBoxTitle.GetFontColor();
		pWndGroupBox->SetTitle( prj.GetText( TID_GAME_DIALOG ) );
	}

	m_pNewQuestListIconTexture = CWndBase::m_textureMng.AddTexture( g_Neuz.m_pd3dDevice, MakePath( DIR_THEME, g_xFlyffConfig->GetMainLanguage(),_T( "QuestUiPaperGreen.tga" )), 0xffffffff );
	m_pExpectedQuestListIconTexture = CWndBase::m_textureMng.AddTexture( g_Neuz.m_pd3dDevice, MakePath( DIR_THEME,g_xFlyffConfig->GetMainLanguage(), _T( "QuestUiPaperRed.tga" )), 0xffffffff );
	m_pCurrentQuestListIconTexture = CWndBase::m_textureMng.AddTexture( g_Neuz.m_pd3dDevice, MakePath( DIR_THEME,g_xFlyffConfig->GetMainLanguage(), _T( "QuestUiPaperGray.tga" )), 0xffffffff );
	m_pCompleteQuestListIconTexture = CWndBase::m_textureMng.AddTexture( g_Neuz.m_pd3dDevice, MakePath( DIR_THEME,g_xFlyffConfig->GetMainLanguage(), _T( "QuestUiPaperYellow.tga" )), 0xffffffff );
	...
	for( int i = 0; i < (int)( dwLineCount ); i++)
	{
		...
		for( int j = 0; j < nLength; j++ )
		{
			...
			if( dwMark == 3 ) 
			{
				...
				FLStrcpy( m_aContextButton[ m_nContextButtonNum ].szWord, _countof( m_aContextButton[ m_nContextButtonNum ].szWord ), strTemp );
				FLStrcpy( m_aContextButton[ m_nContextButtonNum ].szKey, _countof( m_aContextButton[ m_nContextButtonNum ].szKey ), strKey );
				if( bLinkWord )
					FLStrcpy( m_aContextButton[ m_nContextButtonNum - 1 ].szKey, _countof( m_aContextButton[ m_nContextButtonNum - 1 ].szKey ), strKey );
				m_nContextButtonNum++;
				bLinkWord = FALSE;
				strWord.Empty();
			}
		...
		if( dwMark == 1 || dwMark == 2 ) //bKeyButton == TRUE )
		{
			...
			m_aContextButton[ m_nContextButtonNum ].dwParam2 = m_dwQuest;
			m_mapWordToOriginal.Lookup( strWord, strKey );
			FLStrcpy( m_aContextButton[ m_nContextButtonNum ].szWord, _countof( m_aContextButton[ m_nContextButtonNum ].szWord ), strWord );
			FLStrcpy( m_aContextButton[ m_nContextButtonNum ].szKey, _countof( m_aContextButton[ m_nContextButtonNum ].szKey ), strKey );
...
void CWndDialog::AddKeyButton( LPCTSTR lpszWord, LPCTSTR lpszKey, DWORD dwParam, DWORD dwQuest )
{
	...
	FLStrcpy( lpKeyButton->szWord, _countof( lpKeyButton->szWord ), lpszWord );
	FLStrcpy( lpKeyButton->szKey, _countof( lpKeyButton->szKey ), lpszKey );
...
void CWndDialog::AddAnswerButton( LPCTSTR lpszWord, LPCTSTR lpszKey, DWORD dwParam, DWORD dwQuest )
{
	...
	FLStrcpy( lpWordButton->szWord, _countof( lpWordButton->szWord ), lpszWord );
	FLStrcpy( lpWordButton->szKey, _countof( lpWordButton->szKey ), lpszKey );
...
void CWndDialog::MakeAnswerButton()
{
	...
	if( m_nCurArray == m_strArray.GetSize() - 1 )
	{
		...
		{
			WORDBUTTON* pWordButton = &m_aWordButton[ i ];
			FLERROR_ASSERT_LOG_DO( pWordButton, continue, _T("[윈도우 다이얼로그] 버튼 문자 포인터가 유효하지 않습니다.") );

			if( CS_VER1 == _GetContentState( CT_NEWUI_19 ) )
				rect = CRect( x, lpWndCtrl->rect.bottom - 20, x + 70, lpWndCtrl->rect.bottom + 10 );
			if( CS_VER2 == _GetContentState( CT_NEWUI_19 ) )
				rect = CRect( x, lpWndCtrl->rect.bottom - 22, x + 70, lpWndCtrl->rect.bottom + 2 );
			...
			if( strcmp( pWordButton->szWord, "__YES__" ) == 0 )
			{
				...
				if( CS_VER2 == _GetContentState( CT_NEWUI_19 ) )
					strTitle = prj.GetText( TID_MMI_UIBUTTON_BUTTON02 );
			...
			if( strcmp( pWordButton->szWord, "__NO__" ) == 0 )
			{
				...
				if( CS_VER2 == _GetContentState( CT_NEWUI_19 ) )
					strTitle = prj.GetText( TID_MMI_UIBUTTON_BUTTON03 );
			}
			else
			if( strcmp( pWordButton->szWord, "__CANCEL__" ) == 0 )
			{
			...
			if(	nWndId )
			{
				...
				if( CS_VER1 == _GetContentState( CT_NEWUI_19 ) )
					m_apWndAnswer[ j ]->SetTexture( D3DDEVICE, MakePath( DIR_THEME, g_xFlyffConfig->GetMainLanguage(),strTexture ), 1 );
				if( CS_VER2 == _GetContentState( CT_NEWUI_19 ) )
				{
					m_apWndAnswer[ j ]->SetTexture( D3DDEVICE, MakePath( DIR_THEME, g_xFlyffConfig->GetMainLanguage(),_T("ButtNormal00.tga") ), 1 );
					m_apWndAnswer[ j ]->SetTitle( strTitle );
				}
...
void CWndDialog::MakeQuestKeyButton( const CString& rstrKeyButton )
{
	...
	lpKeyButton = &m_aKeyButton[ m_nKeyButtonNum ];
	FLStrcpy( lpKeyButton->szWord, _countof( lpKeyButton->szWord ), rstrKeyButton );
	FLStrcpy( lpKeyButton->szKey, _countof( lpKeyButton->szKey ), rstrKeyButton );
	lpKeyButton->bStatus = FALSE;
