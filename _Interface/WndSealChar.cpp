...
{ 
	...
	strTemp.Format( _T( prj.GetText(TID_DIAG_SEAL_CHAR_SELECT_TEXT01) ) ); 

	CWndText* pWndText( (CWndText*)GetDlgItem( WIDC_TEXT1 ) );
	FLERROR_ASSERT_LOG_RETURN_VOID( pWndText, _T("[캐릭터 봉인 선택] 윈도우 텍스트의 포인터가 유효하지 않습니다.") );

	pWndText->SetString( strTemp );

	CWndButton* pWndButtonChar_1st( NULL );
	CWndButton* pWndButtonChar_2nd( NULL );
	
	pWndButtonChar_1st = (CWndButton*)GetDlgItem( WIDC_RADIO1 );
	FLERROR_ASSERT_LOG_RETURN_VOID( pWndButtonChar_1st, _T("[캐릭터 봉인 선택] 윈도우 텍스트의 포인터가 유효하지 않습니다.") );

	pWndButtonChar_1st->SetGroup( TRUE );
	pWndButtonChar_1st->SetCheck( TRUE );

	pWndButtonChar_2nd = (CWndButton*)GetDlgItem( WIDC_RADIO2 );
	FLERROR_ASSERT_LOG_RETURN_VOID( pWndButtonChar_2nd, _T("[캐릭터 봉인 선택] 윈도우 텍스트의 포인터가 유효하지 않습니다.") );
...
{
	FLStrcpy( m_szSrc1, _countof( m_szSrc1 ), szName1 );
	FLStrcpy( m_szSrc2, _countof( m_szSrc2 ), szName2 );
