...
void CWndRegVend::SetItem( int iIndex, FLItemBase* pItemBase )
{
	CWndStatic* pStatic = (CWndStatic*)GetDlgItem(WIDC_SELLNUM);

	char str[32]	= { 0, };
	FLSPrintf( str, _countof( str ), "%d", ( (FLItemElem*)pItemBase )->m_nItemNum );
...
//sun: 8, // __S8_VENDOR_REVISION
		FLSPrintf( str, _countof( str ), "%d", 0 );
//		sprintf( str, "%d", pItemProp->dwCost );

		pStatic->SetTitle( str );

		if(( (FLItemElem*)pItemBase )->m_nItemNum <= 1 )
			m_dwFocus = 1;
	}
...
void CWndRegVend::OnInitialUpdate()
{
	...
	m_pTex = m_textureMng.AddTexture( m_pApp->m_pd3dDevice,  MakePath( DIR_THEME,g_xFlyffConfig->GetMainLanguage(), "WndVenderArrowEx.tga" ), 0xffff00ff );
	...
	case WIDC_VENEQUA:
		{
			if( m_bIsFirst )
			{
				...
				if(i < 0)
					FLIntToString( 2100000000, buffer, _countof( buffer ), 10 );
				else
					FLIntToString( i, buffer, _countof( buffer ), 10 );
	...
	case WIDC_VENMAX:
		{
			if(m_pItemBase)
			{
				...
				int nValue = 0;
//sun: 8, // __S8_VENDOR_REVISION
				nValue = ( m_dwFocus == 0 ) ? ( (FLItemElem*)m_pItemBase )->m_nItemNum: 2100000000;
//				nValue = ( m_dwFocus == 0 ) ? ( (FLItemElem*)m_pItemBase )->m_nItemNum: pItemProp->dwCost;
	...
	if( nID == WIDC_OK || message ==  EN_RETURN )
	{
		...
		int nNum	= 1;
		nNum	= atoi( str );
		FLItemElem* pItemElem	= (FLItemElem*)m_pItemBase;
...
//		}
		...
		if( n64Sum <= 0 || 2100000000 <= n64Sum || (INT64)nOldGold >= n64Sum )
		{
