...
BOOL CWndQuestQuickInfo::Process( void )
{
	...
	//gmpbigsun ( On mouse 검사 루틴 수정 )
	ClientToScreen( &ptMouse );
	if( this == IsOnWndBase2( ptMouse ) )
		m_bOnSurface = TRUE;
	else m_bOnSurface = FALSE;
...
void CWndQuestQuickInfo::PaintFrame( C2DRender* p2DRender )
{
	...
	if( IsWndStyle( WBS_CAPTION ) )
	{
		...
		if( CS_VER2 == _GetContentState( CT_NEWUI_19 ) )
			p2DRender->TextOut( TitleStringPosition.x, TitleStringPosition.y, m_strTitle, ( ( m_dwColor & 0x00FFFFFF ) | ( m_nAlphaCount << 24 ) ) );
