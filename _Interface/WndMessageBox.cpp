		...
		case MB_CANCEL:
			{
				...
				if( CS_VER1 == _GetContentState( CT_NEWUI_19 ) )
					m_wndButton1.SetTexture( m_pApp->m_pd3dDevice, MakePath( DIR_THEME,g_xFlyffConfig->GetMainLanguage(), "ButtCancel.tga" ) );
				if( CS_VER2 == _GetContentState( CT_NEWUI_19 ) )
					m_wndButton1.SetTexture( m_pApp->m_pd3dDevice, MakePath( DIR_THEME,g_xFlyffConfig->GetMainLanguage(), "ButtNormal00.tga" ) );
...
BOOL CWndMessageBox::OnChildNotify( UINT message, UINT nID, LRESULT* pLResult )
{
	switch(nID)
	{
		...
		case IDOK:
		case IDYES:
		case WTBID_CLOSE:		// gmpbigsun( 20110224 ): x버튼 처리
			{
				if( _tcscmp( m_wndText.m_string, _T( prj.GetText(TID_DIAG_0035) ) ) == 0 )
				{
					::PostMessage( g_Neuz.GetSafeHwnd(), WM_CLOSE, TRUE, 0 );
					FLERROR_LOG( PROGRAM_NAME, _T( "Old Client PostMessage" ) );
				}

				if( nID == IDOK && getCode() == ERROR_WEBZEN_ACCOUNT_ID_CONVERTING_FAIL ) {
					ShellExecute( NULL, _T("open"),  _T("http://www.webzen.com/account-merge?cd=SVR014"), NULL, NULL, SW_SHOW );
				}
				else if( nID == IDOK && getCode() == ERROR_WEBZEN_GDPR_AGREEMENT ) {
					char url[128] = { 0 };
					FLStrcpy( url, _countof( url ), "https://login.webzen.com" );
					ShellExecute( NULL, _T("open"),  url, NULL, NULL, SW_SHOW );
				}
				else if( nID == IDOK && getCode() == ERROR_WEBZEN_TERMS_AGREEMENT ) {
					char url[128] = { 0 };
					FLStrcpy( url, _countof( url ), "http://www.webzen.com/Rules/TermsOfUse?SvcCd=SVR014&LangCd=" );
					if( g_xFlyffConfig->GetMainLanguage() == LANG_USA ) {
						FLStrcat( url, _countof( url ), "en" );
					}
					else if( g_xFlyffConfig->GetMainLanguage() == LANG_GER ) {
						FLStrcat( url, _countof( url ), "de" );
					}
					else if( g_xFlyffConfig->GetMainLanguage() == LANG_FRE ) {
						FLStrcat( url, _countof( url ), "fr" );
					}
					ShellExecute( NULL, _T("open"),  url, NULL, NULL, SW_SHOW );
				}
				else if( nID == IDOK && getCode() == ERROR_WEBZEN_TRY_CONNECT_BLOCKING_AREA ) {
					char url[128] = { 0 };
					FLStrcpy( url, _countof( url ), "https://member.webzen.com/SecurityCheck" );
					ShellExecute( NULL, _T("open"),  url, NULL, NULL, SW_SHOW );
				}
...
BOOL CWndMessageBox::Create( LPCTSTR lpszMessage, UINT nType, const RECT& rect, UINT nID, CWndBase* pWndParent )
{
	...
	m_wndText.SetString( lpszMessage );
