...
{ 
	...
	if( idEnemyGuild != NULL_ID )
	{
		...
		{
			FLERROR_LOG( PROGRAM_NAME, _T( "적길드를 찾을수 없음." ) );
			return;
		}
	} else
	{
...
{ 
	...
	if(pWndButton)
	{
		if(g_xFlyffConfig->GetMainLanguage() == LANG_USA || g_xFlyffConfig->GetMainLanguage() == LANG_VTN)
			pWndButton->SetTexture( m_pApp->m_pd3dDevice, MakePath( DIR_THEME , g_xFlyffConfig->GetMainLanguage(), "ButtGuildDuel.bmp" ), 0xffff00ff );
	}
