...
{ 
	...
	{
		FLStrcpy( m_sParty, _countof( m_sParty ), prj.GetText(TID_GAME_PARTY_NAME) );
	}
	else
	{
		FLStrcpy( m_sParty, _countof( m_sParty ), g_pPlayer->m_szPartyName );
	}
...
{ 
	if( nID == WIDC_OK )
	{
		...
		if( PartyName.IsEmpty() == FALSE )
		{
			...
			if( PartyName.GetLength() < 4 || PartyName.GetLength() > 16 )	//sun: 10, __RULE_0615
			{
				g_WndMng.OpenMessageBox( _T( prj.GetText(TID_DIAG_RULE_0) ) );	//sun: 10, __RULE_0615
				return TRUE;
			}
			else
...
//							g_WndMng.OpenMessageBox( _T( "명칭에 콘트롤이나 스페이스, 특수 문자를 사용할 수 없습니다." ) );
			...
			if( prj.IsInvalidName( PartyName ) 
				|| prj.IsAllowedLetter( PartyName ) == FALSE	//sun: 10, __RULE_0615
				)
			{
			...
			FLStrcpy( m_sParty, _countof( m_sParty ), PartyName );
