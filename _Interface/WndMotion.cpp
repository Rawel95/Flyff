...
BOOL CWndMotion1::Process( void )
{
	...
	for( int i = 0; i < m_motionArray.GetSize(); i++ )
	{
		...
		if( pMotionProp  )
		{
			...
			TCHAR szDesc [ 1024 ] = { 0 };
			FLStrcpy( szDesc, _countof( szDesc ), pMotionProp->szDesc );
			
			if( g_pPlayer )
			{
				TCHAR szChar[ 256 ] = { 0 };
				FLSPrintf( szChar, _countof( szChar ), prj.GetText(TID_CHEER_MESSAGE1), g_pPlayer->m_nCheerPoint );
				FLStrcat( szDesc, _countof( szDesc ), "\n" );
				FLStrcat( szDesc, _countof( szDesc ), szChar );
				...
				if( g_pPlayer->m_nCheerPoint < MAX_CHEERPOINT )
				{
					...
					if( ct.GetMinutes() <= 0 )
					{
						FLSPrintf( szChar2, _countof( szChar2 ), prj.GetText(TID_PK_LIMIT_SECOND), ct.GetSeconds() );
					}
					else
					{
						FLSPrintf( szChar2, _countof( szChar2 ), prj.GetText(TID_PK_LIMIT_MINUTE), ct.GetMinutes() );
					}
					
					FLStrcat( szDesc, _countof( szDesc ), "\n" );
					FLStrcat( szDesc, _countof( szDesc ), prj.GetText(TID_CHEER_MESSAGE2) );	
					FLStrcat( szDesc, _countof( szDesc ), szChar2 );	
				}
	...
	for( int i = 0; i < (int)( g_DialogMsg.m_texEmoticonUser.GetNumber() ) ; i++ )
	{
		...
		if( pTexture )
		{
			...
			if( PtInRect( &rect, pt2 ) )
			{
				...
				FLStrcat( szCmd, _countof( szCmd ), g_DialogMsg.m_EmiticonCmd[i].m_szCommand );
...
void CWndEmoticon::OnMouseMove( UINT nFlags, CPoint point )
{
	if( m_pSelectTexture && IsPush())
	{
		...
		FLStrcat( szCmd, _countof( szCmd ), "/" );
		FLStrcat( szCmd, _countof( szCmd ), g_DialogMsg.m_EmiticonCmd[m_nSelect].m_szCommand );
		
		FLStrcpy( m_GlobalShortcut.m_szString, _countof( m_GlobalShortcut.m_szString ), szCmd  );
	}
}
