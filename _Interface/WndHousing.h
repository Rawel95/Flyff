...
struct GHBidData	//길드하우스 입찰 데이터
{
	...
	std::vector< DWORD > _cGuildList;	//입찰한 길드목록
	int _nBidMinPenya;				//최소 입찰금액
};

typedef std::vector<GHBidData>				GHBidDataContainer;
...
public:
	...
	void UpdateData_HouseInfo( DWORD houseID, const int nMinPenya, __int64 nTnederPenya, std::vector< DWORD >& guildIDs );
