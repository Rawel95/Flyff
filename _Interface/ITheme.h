...
public:
	...
	CD3DFont*	GetFont( const CString& strFontName );
	...
	void 		RenderWndBaseTitleBar   ( C2DRender* p2DRender, CRect* pRect, int nX, int nY, LPCTSTR lpszTitle, DWORD dwColor = 0xFF000000 );
	void 		RenderWndBaseTitleBar   ( C2DRender* p2DRender, int nX, int nY, LPCTSTR lpszTitle, DWORD dwColor = 0xFF000000 );
...
private:
	...
	bool		InitFont( int nMainLanguage );

	bool		CreateFont( CString		strFontName, 
							UINT		nFontSize, 
							CD3DFont*&	pFont,
							CString		strFontID,
							UINT		nOutLine, 
...
public:
	...
	CD3DFont* 							m_pFontSRMyGiuld;			//sun: 12, 비밀의 방
