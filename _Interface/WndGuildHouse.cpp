...
GH_Fntr_Info* CWndGHTeleporter::GetInstalledItem( int& serverSlot )
{
	for( int i = 0; i < GuildHouse->GetFurnitureListSize( ); ++i )
	{
		...
		PT_ITEM_SPEC pProp = g_xSpecManager->GetSpecItem( pItem->dwItemId );
		FLASSERT( pProp );
...
void CWndGHTeleporter::RefreshItems( )
{
	...
	if( pItem )
	{
		...
		pProp = g_xSpecManager->GetSpecItem( pItem->dwItemId );
		FLASSERT( pProp );
...
{ 
	...
	FLASSERT( MAX_UPKEEP_DAY_CLIENT > 0 );
} 
...
void CWndGHUpkeep::OnOK()
{
	...
	FLASSERT( nBuy > 0 && nBuy <= MAX_UPKEEP_DAY_CLIENT );
	GH_Fntr_Info data;
