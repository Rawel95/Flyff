...
//		SetDefender( -1 );
	...
	for(int i=nIndex; i<nListCount; i++)
	{
		...
		CGuild* pGuild = g_pPlayer->GetGuild();
		std::map<u_long, CGuildMember*>::iterator iter = pGuild->m_mapPMember.find( m_vecSelectPlayer[i] );
...
*/		
		...
		CGuild* pGuild = g_pPlayer->GetGuild();
		std::map<u_long, CGuildMember*>::iterator i = pGuild->m_mapPMember.find( m_vecSelectPlayer[nCurSel] );
		CGuildMember* pMember = i->second;
//sun: 11, 캐릭터 정보 통합
		PlayerData* pPlayerData		= CPlayerDataCenter::GetInstance()->GetPlayerData( pMember->m_idPlayer );
