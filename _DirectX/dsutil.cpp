...
BOOL CSoundManager::Play( int nId, const D3DXVECTOR3* pvPosition, D3DXVECTOR3* pvVelocity, BOOL bIsAutoplay, BOOL bIsVoice ) 
{ 
	if( !m_bSuccess )
		return FALSE;
	...
	if( lpszFileName )
	{
		...
		MakePath( szFileName, _countof( szFileName ), DIR_SOUND, GetFileName( nId ) ); 
//		FLERROR_LOG( PROGRAM_NAME, _T( "%s" ), szFileName );
...
BOOL CSoundManager::Play( LPTSTR pszFileName, const D3DXVECTOR3* pvPosition, D3DXVECTOR3* pvVelocity, BOOL bIsAutoplay, BOOL bIsVoice )
{
	if( !m_bSuccess )
		return FALSE;
	...
	if( pSound == NULL )
	{
//		FLERROR_LOG( PROGRAM_NAME, _T( "file not found: %s" ), pszFileName );
		return FALSE;
	}
	...
	if( pvPosition )
	{
		...
		for( int i = 0; i < m_world.m_arySelect.size(); i++)
		{
			...
			if( pt.y )
			{
				...
				if( vlength )
				{
					FLTRACE_LOG( PROGRAM_NAME, _T( "a %f" ), a );
	...
	//Radian *= 2.0f;
		...
		//gmpbigsun( 20111114 ) : #20583 스킬사용 효과음 
		if( vlength > 0.0f )
			Line2 = Line2 / vlength;
		// 라디안 구하기 
		FLOAT a = acos( D3DXVec3Dot( &Line1, &Line2 ) );
