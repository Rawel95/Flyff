...
HRESULT DXUtil_GetDXSDKMediaPathCch( TCHAR* strDest, int cchDest )
{
    ...
    if( lstrlen(strDest) + lstrlen(strMedia) < cchDest )
        FLStrcat( strDest, cchDest, strMedia );
    else
        return E_INVALIDARG;
...
HRESULT DXUtil_FindMediaFileCch( TCHAR* strDestPath, int cchDest, TCHAR* strFilename )
{
    ...
    TCHAR strShortName[MAX_PATH] = TEXT("");
    int cchPath;

    if( NULL==strFilename || NULL==strDestPath || cchDest < 1 )
        return E_INVALIDARG;

    FLStrcpy( strDestPath, cchDest, TEXT("") );
    ...
    if( INVALID_HANDLE_VALUE != file )
    {
        FLStrncpy( strDestPath, cchDest, strShortName, cchDest );
    ...
    if( lstrlen(strDestPath) + lstrlen(strShortName) < cchDest )
        FLStrcat( strDestPath, cchDest, strShortName );
    else
        return E_INVALIDARG;
    ...
    FLStrncpy( strDestPath, cchDest, strFilename, cchDest );
    strDestPath[cchDest-1] = 0; // _tcsncpy doesn't NULL term if it runs out of space
                                    ...
                                    DWORD cchDest, TCHAR* strDefault )
{
    ...
    if( ERROR_SUCCESS != RegQueryValueEx( hKey, strRegName, 0, &dwType, 
                                          (BYTE*)strDest, &cbDest ) )
    {
        FLStrncpy( strDest, cchDest, strDefault, cchDest );
        strDest[cchDest-1] = 0;
...
#else
    FLStrncpy( strDestination, cchDestChar, tstrSource, cchDestChar );
    return S_OK;
#endif   
}
...
#ifdef _UNICODE
    FLStrncpyW( wstrDestination, cchDestChar, tstrSource, cchDestChar );
...
#else
    FLStrncpy( tstrDestination, cchDestChar, strSource, cchDestChar );
    return S_OK;
#endif    
}
...
#ifdef _UNICODE
    FLStrncpyW( tstrDestination, cchDestChar, wstrSource, cchDestChar );
...
VOID DXUtil_LaunchReadme( HWND hWnd, TCHAR* strLoc )
{
    ...
    bool bSuccess = false;
    bool bFound = false;
    TCHAR strReadmePath[1024] = TEXT("");
    TCHAR strExeName[MAX_PATH] = TEXT("");
    TCHAR strExePath[MAX_PATH] = TEXT("");
    TCHAR strSamplePath[MAX_PATH] = TEXT("");
    ...
    if( strLoc )
    {
        ...
        if( ERROR_SUCCESS == lResult )
        {
            ...
            if( ERROR_SUCCESS == lResult )
            {
                FLSPrintf( strReadmePath, _countof( strReadmePath ), TEXT("%s\\C++\\%s\\readme.txt"), 
    ...
    if( strLastSlash )
    {
        FLStrncpy( strExeName, _countof( strExeName ), &strLastSlash[1], MAX_PATH );
    ...
    if( !bFound )
    {
        ...
        FLStrcpy( strReadmePath, _countof( strReadmePath ), strExePath );

        strLastSlash = _tcsrchr( strReadmePath, TEXT('\\') );
        if( strLastSlash )
            *strLastSlash = 0;
        FLStrcat( strReadmePath, _countof( strReadmePath ), TEXT("\\") );
        FLStrcat( strReadmePath, _countof( strReadmePath ), strExeName );
        FLStrcat( strReadmePath, _countof( strReadmePath ), TEXT("\\readme.txt") );
        if( GetFileAttributes( strReadmePath ) != 0xFFFFFFFF )
            bFound = TRUE;
    }
...
VOID DXUtil_Trace( TCHAR* strMsg, ... )
{
    ...
    FLVSPrintf( strBuffer, _countof( strBuffer ), strMsg, args );
    va_end(args);
...
{
    int nResult = FLSPrintf( strDest, cchDestChar, TEXT("{%0.8X-%0.4X-%0.4X-%0.2X%0.2X-%0.2X%0.2X%0.2X%0.2X%0.2X%0.2X}"),
