...
void CD3DSettingsDialog::WindowedFullscreenChanged( void )
{
    ...
    if( IsDlgButtonChecked( m_hDlg, IDC_WINDOW ) )
    {
        ...
        FLSPrintf( strResolution, _countof( strResolution ), TEXT("%d by %d"), m_d3dSettings.Windowed_DisplayMode.Width, 
            m_d3dSettings.Windowed_DisplayMode.Height );
        strResolution[49] = 0;
        ...
        if( m_d3dSettings.Windowed_DisplayMode.RefreshRate == 0 )
            FLStrcpy( strRefreshRate, _countof( strRefreshRate ), TEXT("Default Rate") );
        else
            FLSPrintf( strRefreshRate, _countof( strRefreshRate ), TEXT("%d Hz"), m_d3dSettings.Windowed_DisplayMode.RefreshRate );
        strRefreshRate[49] = 0;
    ...
    if( !IsDlgButtonChecked( m_hDlg, IDC_WINDOW ) )
    {
        ...
        for( UINT idm = 0; idm < pAdapterInfo->pDisplayModeList->Count(); idm++ )
        {
            ...
            if (displayMode.Format == adapterFormat)
            {
                ...
                FLSPrintf( strResolution, _countof( strResolution ), TEXT("%d by %d"), displayMode.Width, displayMode.Height );
                strResolution[49] = 0;
    ...
    for( UINT idm = 0; idm < pAdapterInfo->pDisplayModeList->Count(); idm++ )
    {
            ...
            displayMode.Height == height)
        {
            ...
            if( displayMode.RefreshRate == 0 )
                ...
                FLSPrintf( strRefreshRate, _countof( strRefreshRate ), TEXT("%d Hz"), displayMode.RefreshRate );
            strRefreshRate[49] = 0;
...
void CD3DSettingsDialog::MultisampleTypeChanged( void )
{
    ...
    {
        TCHAR str[100];
        FLSPrintf( str, _countof( str ), TEXT("%d"), msq );
