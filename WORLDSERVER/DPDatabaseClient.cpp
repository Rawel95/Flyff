...
#ifdef __ZCK_WORLD_BOSS
	CWorldBossManager::GetInstance()->SendWorldBossInfo( pUser );
#endif // __ZCK_WORLD_BOSS
#ifdef __ON_ERROR
	}
	catch( ... )
...
void CDPDatabaseClient::OnAllGuildData( CAr & ar, DPID, DPID )
{
	...
	const DWORD dwZckGuildDataStart = timeGetTime();
#endif // __ZCK_TICK_FIX
	g_GuildMng.Serialize( ar, FALSE );
	g_GuildWarMng.Serialize( ar );
#ifdef __ZCK_TICK_FIX
	const DWORD dwZckGuildDataMs = timeGetTime() - dwZckGuildDataStart;
	FLINFO_LOG( PROGRAM_NAME,
		_T( "[ZCK_TICK_FIX] OnAllGuildData deserialize - tick:(%u ms)" ),
		dwZckGuildDataMs );
#endif // __ZCK_TICK_FIX
