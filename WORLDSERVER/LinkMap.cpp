...
, m_iMPU( 0 )
{
	...
	//memset( m_apfMask, 0, sizeof( m_apfMask ) );
	//memset( m_arrVisibilityRangeNum, 0, sizeof( m_arrVisibilityRangeNum ) );
}

void CLinkMap::Init( int nLandWidth, int nLandHeight, int nView, int nMPU )
{
	if( nLandWidth <= 0 || nLandHeight <= 0 || nMPU <= 0)
	{
		FLERROR_LOG( PROGRAM_NAME, _T("[ INVALID SIZE : Width(%d) Height(%d) MPU(%d)  ]"), nLandWidth, nLandHeight, nMPU );
		return;
	}
	...
	//{
	//	// m_arrVisibilityRangeNum[i] = { 2, 1, 0, 0, 0, 0 };
	//	m_arrVisibilityRangeNum[i]	= (int)( 2 * nView / pow( 2., i ) );		
	//	int nSizeOfMask	= ( 2 * m_arrVisibilityRangeNum[i] + 1 ) * ( 2 * m_arrVisibilityRangeNum[i] + 1 );
	//	m_apfMask[i]	= new BOOL[nSizeOfMask];
	//	memset( m_apfMask[i], 0, sizeof(BOOL) * nSizeOfMask );
	//}

	const size_t uWidthMeter	= m_nLandWidth * m_iMPU * MAP_SIZE;
	const size_t uHeightMeter	= m_nLandHeight * m_iMPU * MAP_SIZE;
	const int nWidthTileCount	= uWidthMeter / DEFAULT_TILE_SIZE + ( ( uWidthMeter % DEFAULT_TILE_SIZE ) != 0 ? 1 : 0 );
	const int nHeightTileCount	= uHeightMeter / DEFAULT_TILE_SIZE + ( ( uHeightMeter % DEFAULT_TILE_SIZE ) != 0 ? 1 : 0 );

	const size_t uViewRangeCount		= DEFAULT_VIEW_RANGE_COUNT * nView + ( ( DEFAULT_VIEW_RANGE_COUNT * nView % 2 ) == 0 ? 1 : 0 );
	m_kMapGrid.Create( nWidthTileCount, nHeightTileCount, DEFAULT_TILE_SIZE, uViewRangeCount );
}
...
//		FLERROR_LOG( PROGRAM_NAME, _T( "ML//%s//%d//%d" ), ( (CMover*)pObj )->GetName(), ( (CMover*)pObj )->m_idPlayer, pObj->GetLinkLevel() );
//	
...
//			{
//				FLERROR_LOG( PROGRAM_NAME, _T( "헉 InsertObjWorld 새로 추가된 오브젝트가 Next와 Prev에 값이 있네?? Type = %d" ), pObj->GetType());
//			}
...
//	else
//	{
//		FLERROR_LOG( PROGRAM_NAME, _T( "헉 InsObjInFld에서 범위를 벗어난게 추가되려고 하네?" ) );
//		return FALSE;
//	}
//
//	if( pObj->GetType() == OT_MOVER )
//		m_kMapGrid.Enter( static_cast< CMover * >( pObj ), pObj->GetPos() );
//
//	return TRUE;
//}
//
...
//{
//	FLERROR_LOG( PROGRAM_NAME, _T( "ROL//CALLED" ) );
//
...
//			pObj->DelNode();
//			FLERROR_LOG( PROGRAM_NAME, _T( "ROL//FIXED//%d, %d" ), pObj->GetType(), pObj->GetIndex() );
//			
//			if( pObj->GetType() == OT_MOVER )
//				m_kMapGrid.Leave( static_cast< CMover * >( pObj ), pObj->GetPos() );
//			return TRUE;
//		}
//	}
//	FLERROR_LOG( PROGRAM_NAME, _T( "ROL//NOT FIXED//%d, %d" ), pObj->GetType(), pObj->GetIndex() );
//	return FALSE;
//}
//
...
//{
//	if( pObj->GetType() == OT_MOVER && ( (CMover*)pObj )->IsPlayer() && pObj->GetLinkLevel() != 0 )
//		FLERROR_LOG( PROGRAM_NAME, _T( "ROL//%s//%d//%d" ), ( (CMover*)pObj )->GetName(), ( (CMover*)pObj )->m_idPlayer, pObj->GetLinkLevel() );
//
//	CObj** ppObj = GetObjPtr( pObj->GetLinkPos(), pObj->GetLinkType(), pObj->GetLinkLevel() );
//	if( ppObj )
//	{
...
//	FLERROR_LOG( PROGRAM_NAME, _T( "ROL//POS OVERFLOW//%d, %d, %f, %f, %f, %f, %f, %f" ), 
...
//			nVisibilityRange	= m_arrVisibilityRangeNum[i];
...
//					pUsertmp	= (FLWSUser*)apObj[j * nMaxWidth + k];
//					while( pUsertmp )
//					{
...
//						}
//						pUsertmp	= (FLWSUser*)pUsertmp->GetNextNode();
//					}
//				}
//			}
//
...
//			}
//	
//			nVisibilityRange	= m_arrVisibilityRangeNum[nLinkLevel];
...
//	{
//		//FLASSERT( 0 );
//		FLTRACE_LOG( PROGRAM_NAME, _T( "pObj->m_dwIndex:%d fObjWidth: %f linkType:%d" ), pObj->m_dwIndex, fObjWidth, dwLinkType ); 
//	}
//	
