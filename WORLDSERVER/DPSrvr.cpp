...
// 	ON_MSG( PACKETTYPE_GETPOS, OnGetPos );
	...
	ON_MSG( PACKETTYPE_SEND_UPGRADE, OnUpgrade );
#endif // __ZCK_AMELIORATION_ADMIN
#ifdef __ZCK_ARME_RARE
	ON_MSG( PACKETTYPE_ZCK_ARME_RARE, OnZckArmeRare );
#endif // __ZCK_ARME_RARE
#ifdef __ZCK_WORLD_BOSS
	ON_MSG( PACKETTYPE_WORLDBOSS_TELEPORT, OnWorldBossTeleport );
#endif // __ZCK_WORLD_BOSS
#ifdef __ZCK_INVASIONS
	ON_MSG( PACKETTYPE_ZCK_INVASIONS_REFRESH, OnZckInvasionsRefresh );
#endif // __ZCK_INVASIONS
#ifdef __ZCK_WIKI
	ON_MSG( PACKETTYPE_WIKI_CREATE, OnWikiCreate );
#endif // __ZCK_WIKI
#ifdef __ZCK_TELEPORTEUR
	ON_MSG( PACKETTYPE_ZCK_TELEPORTEUR, OnZckTeleporteur );
#endif // __ZCK_TELEPORTEUR
#ifdef __ZCK_RECORD
	ON_MSG( PACKETTYPE_ZCK_RECORD_REQUEST, OnZckRecordRequest );
#endif // __ZCK_RECORD
...
#ifdef __TIMER
	ON_MSG( PACKETTYPE_TIMER, OnDungeonTimer );
#endif
...
#ifdef __ZCK_AMELIORATION_ADMIN
void CDPSrvr::OnUpgrade(CAr& ar, DPID dpidCache, DPID dpidUser, LPBYTE, u_long)
{
	OBJID objItem = NULL_ID;
	SERIALNUMBER serialItem = 0;
	BYTE byElement = SAI79::NO_PROP;
	short nElementLevel = 0;
	short nAbilityLevel = 0;
	short nPiercing = 0;
	...
	if (pUser->m_dwLastUpgradeAdminTick != 0 &&
		dwNow - pUser->m_dwLastUpgradeAdminTick < 250)
	{
		return;
	}

	pUser->m_dwLastUpgradeAdminTick = dwNow;
	...
	if (pUser->m_vtInfo.GetOther() != NULL ||
		pUser->m_vtInfo.VendorIsVendor() == TRUE)
	{
		return;
	}
	...
	FLItemElem* pItem = pUser->m_Inventory.GetAtId(objItem);
	if (pItem == NULL)
		return;

	if (pItem->GetSerialNumber() != serialItem)
		return;

	if (IsUsableItem(pItem) == FALSE)
		return;
	...
	if (pUser->IsUsing(pItem) == TRUE ||
		pUser->m_Inventory.IsEquip(objItem) == TRUE)
	{
		return;
	}

	if (pItem->IsFlag(FLItemElem::period) == TRUE ||
		pItem->IsFlag(FLItemElem::expired) == TRUE)
	{
		return;
	}

	if (pItem->m_nResistSMItemId != 0 ||
		pItem->m_nItemNum <= 0)
	{
		return;
	}

	const DWORD dwKind3 = pItemSpec->dwItemKind3;

	const BOOL bMainEligible =
		dwKind3 == IK3_HELMET ||
		dwKind3 == IK3_SUIT ||
		dwKind3 == IK3_GAUNTLET ||
		dwKind3 == IK3_BOOTS ||
		dwKind3 == IK3_SWD ||
		dwKind3 == IK3_AXE ||
		dwKind3 == IK3_KNUCKLEHAMMER ||
		dwKind3 == IK3_CHEERSTICK ||
		dwKind3 == IK3_WAND ||
		dwKind3 == IK3_STAFF ||
		dwKind3 == IK3_BOW ||
		dwKind3 == IK3_YOYO ||
		...
		|| dwKind3 == IK3_CROSSBOW
#endif // __ADDWEAPON_CROSSBOW16
		|| pItemSpec->IsBarunaWeapon() == TRUE
		|| pItemSpec->IsBarunaArmor() == TRUE;

	const BOOL bGeneralOnly =
		pItemSpec->dwItemKind2 == IK2_JEWELRY ||
		pItem->IsCollector(TRUE) == TRUE;

	if (bMainEligible == FALSE && bGeneralOnly == FALSE)
	{
		pUser->AddText(_T("UPGRADE ADMIN : cet objet ne peut pas etre modifie."));
		return;
	}

	if (byElement >= SAI79::END_PROP ||
		nElementLevel < 0 ||
		nElementLevel > 20 ||
		nAbilityLevel < 0 ||
		nAbilityLevel > 20 ||
		nPiercing < 0)
	{
		pUser->AddText(_T("UPGRADE ADMIN : parametres invalides."));
		return;
	}

	if (byElement == SAI79::NO_PROP)
	{
		if (nElementLevel != 0)
			return;
	}
	...
	{
		if (bMainEligible == FALSE)
			return;
	}

	const BOOL bPiercingEligible =
		pItemSpec->dwParts == PARTS_RWEAPON ||
		pItemSpec->dwParts == PARTS_UPPER_BODY ||
		pItemSpec->dwParts == PARTS_SHIELD;

	if (bGeneralOnly == TRUE && bMainEligible == FALSE)
	{
		if (byElement != SAI79::NO_PROP ||
			nElementLevel != 0 ||
			nPiercing != 0)
		...
		pUser->UpdateItem(
			objItem,
			UI_AO,
			static_cast<DWORD>(nAbilityLevel));
	}
	else
	{
		if (bPiercingEligible == TRUE)
		{
			const int nMaxPiercing =
				dwKind3 == IK3_SUIT
				? static_cast<int>(MAX_PIERCING_SUIT)
				: static_cast<int>(MAX_PIERCING_WEAPON);
		...
		pUser->UpdateItem(
			objItem,
			UI_IR,
			static_cast<DWORD>(byElement));

		pUser->UpdateItem(
			objItem,
			UI_RAO,
			static_cast<DWORD>(nElementLevel));
		...
		if (pItemSpec->dwReferStat1 == WEAPON_ULTIMATE)
		{
			int nUltimatePiercingSize = nAbilityLevel - 5;

			if (nUltimatePiercingSize < 0)
				nUltimatePiercingSize = 0;

			if (nUltimatePiercingSize > static_cast<int>(MAX_PIERCING_ULTIMATE))
				nUltimatePiercingSize = static_cast<int>(MAX_PIERCING_ULTIMATE);

			pUser->UpdateItem(
				objItem,
				UI_ULTIMATE_PIERCING_SIZE,
				static_cast<DWORD>(nUltimatePiercingSize));
		}
#ifdef BARUNA_UPGRADE_ENHANCEMENT_GEM
		else if (pItemSpec->IsBarunaWeapon() == TRUE)
		{
			int nBarunaPiercingSize = 0;

			if (nAbilityLevel >= 11)
				nBarunaPiercingSize = (nAbilityLevel - 9) / 2;

			if (nBarunaPiercingSize > static_cast<int>(MAX_PIERCING_ULTIMATE))
				nBarunaPiercingSize = static_cast<int>(MAX_PIERCING_ULTIMATE);

			pUser->UpdateItem(
				objItem,
				UI_ULTIMATE_PIERCING_SIZE,
				static_cast<DWORD>(nBarunaPiercingSize));
		}
	...
	if (pUser->IsMode(TRANSPARENT_MODE) == 0)
	{
		g_xWSUserManager->AddCreateSfxObj(
			static_cast<CMover*>(pUser),
			XI_INDEX(1714, XI_INT_SUCCESS),
			pUser->GetPos().x,
			pUser->GetPos().y,
			pUser->GetPos().z);
	}
...
#ifdef __ZCK_ARME_RARE
void CDPSrvr::OnZckArmeRare( CAr & ar, DPID dpidCache, DPID dpidUser, LPBYTE, u_long )
{
	OBJID objTarget = NULL_ID;
	OBJID objMaterial = NULL_ID;
	OBJID objScroll = NULL_ID;
	SERIALNUMBER serialTarget = 0;
	SERIALNUMBER serialMaterial = 0;
	SERIALNUMBER serialScroll = 0;
	...
	FLWSUser* pUser = g_xWSUserManager->GetUser( dpidCache, dpidUser );
	if( IsValidObj( pUser ) == FALSE || pUser->IsDie() == TRUE || pUser->m_bAllAction == FALSE )
		return;
	...
	if( objTarget == NULL_ID || objMaterial == NULL_ID || objScroll == NULL_ID ||
		objTarget == objMaterial || objTarget == objScroll || objMaterial == objScroll )
		return;

	FLItemElem* pTarget = pUser->m_Inventory.GetAtId( objTarget );
	FLItemElem* pMaterial = pUser->m_Inventory.GetAtId( objMaterial );
	FLItemElem* pScroll = pUser->m_Inventory.GetAtId( objScroll );
	if( pTarget == NULL || pMaterial == NULL || pScroll == NULL )
		return;

	if( pTarget->GetSerialNumber() != serialTarget ||
		pMaterial->GetSerialNumber() != serialMaterial ||
		pScroll->GetSerialNumber() != serialScroll )
		return;

	if( IsUsableItem( pTarget ) == FALSE || IsUsableItem( pMaterial ) == FALSE || IsUsableItem( pScroll ) == FALSE )
		return;

	if( pTarget->GetProp() == NULL || pMaterial->GetProp() == NULL || pScroll->GetProp() == NULL )
		return;

	if( pUser->IsUsing( pTarget ) == TRUE || pUser->IsUsing( pMaterial ) == TRUE || pUser->IsUsing( pScroll ) == TRUE )
		return;

	if( pUser->m_Inventory.IsEquip( objTarget ) == TRUE ||
		pUser->m_Inventory.IsEquip( objMaterial ) == TRUE ||
		pUser->m_Inventory.IsEquip( objScroll ) == TRUE )
		return;

	if( pTarget->IsFlag( FLItemElem::period ) == TRUE || pTarget->IsFlag( FLItemElem::expired ) == TRUE ||
		pMaterial->IsFlag( FLItemElem::period ) == TRUE || pMaterial->IsFlag( FLItemElem::expired ) == TRUE ||
		pScroll->IsFlag( FLItemElem::period ) == TRUE || pScroll->IsFlag( FLItemElem::expired ) == TRUE )
		return;

	if( pTarget->m_nResistSMItemId != 0 || pMaterial->m_nResistSMItemId != 0 )
		return;

	if( pTarget->m_nItemNum != 1 || pMaterial->m_nItemNum != 1 || pScroll->m_nItemNum <= 0 )
		return;

	if( pScroll->m_dwItemId != II_PARCHEMIN_RARETER )
	{
		pUser->AddText( _T( "Parchemin de rarete invalide." ) );
		return;
	}

	if( pTarget->IsZckArmeRareEligible() == FALSE || pMaterial->IsZckArmeRareEligible() == FALSE )
	{
		pUser->AddText( _T( "Objet incompatible avec ZCK ARME RARE." ) );
		return;
	}

	if( pTarget->m_dwItemId != pMaterial->m_dwItemId )
	{
		pUser->AddText( _T( "Les deux objets doivent etre identiques." ) );
		return;
	}
	...
	if( prj.CheckZckArmeRare( nTargetRarity ) == FALSE || prj.CheckZckArmeRare( nMaterialRarity ) == FALSE )
		return;

	if( nTargetRarity != nMaterialRarity )
	{
		pUser->AddText( _T( "Les deux objets doivent avoir la meme rarete." ) );
		return;
	}

	if( nTargetRarity >= prj.m_nHighZckArmeRareLevel )
	{
		pUser->AddText( _T( "Cet objet possede deja la rarete maximale." ) );
		return;
	}

	const ZckArmeRareProp* pRarity = prj.GetZckArmeRare( nTargetRarity );
	if( pRarity == NULL || pRarity->m_nUpgradeRate < 0 || pRarity->m_nUpgradeRate > 100 )
		return;

	const BOOL bSuccess = xRandom( 100 ) < pRarity->m_nUpgradeRate;
	const DWORD dwScrollRemain = static_cast<DWORD>( pScroll->m_nItemNum - 1 );
	BYTE byTargetFlag = pTarget->m_byFlag;

	if( bSuccess )
	{
		pTarget->SetZckArmeRareLevel( static_cast<BYTE>( nTargetRarity + 1 ) );
		byTargetFlag = pTarget->m_byFlag;
	}

	pUser->UpdateItem( objScroll, UI_NUM, dwScrollRemain );

	if( bSuccess )
	{
		pUser->UpdateItem( objMaterial, UI_NUM, 0 );
		pUser->UpdateItem( objTarget, UI_FLAG, MAKELONG( static_cast<WORD>( 0xFFFF ), static_cast<WORD>( byTargetFlag ) ) );
		pUser->AddText( _T( "ZCK ARME RARE : amelioration reussie." ) );
		pUser->AddPlaySound( SND_INF_UPGRADESUCCESS );
		g_xWSUserManager->AddCreateSfxObj( static_cast<CMover*>( pUser ), XI_INDEX( 1714, XI_INT_SUCCESS ), pUser->GetPos().x, pUser->GetPos().y, pUser->GetPos().z );
	}
	else
	{
		pUser->UpdateItem( objMaterial, UI_NUM, 0 );
		pUser->UpdateItem( objTarget, UI_NUM, 0 );
		pUser->AddText( _T( "ZCK ARME RARE : echec, les deux objets ont ete detruits." ) );
		pUser->AddPlaySound( SND_INF_UPGRADEFAIL );
		g_xWSUserManager->AddCreateSfxObj( static_cast<CMover*>( pUser ), XI_INDEX( 1715, XI_INT_FAIL ), pUser->GetPos().x, pUser->GetPos().y, pUser->GetPos().z );
	}
}
#endif // __ZCK_ARME_RARE
...
#ifdef __ZCK_WORLD_BOSS
void CDPSrvr::OnWorldBossTeleport( CAr&, DPID dpidCache, DPID dpidUser, LPBYTE, u_long )
{
	FLWSUser* pUser = g_xWSUserManager->GetUser( dpidCache, dpidUser );
	if( IsValidObj( pUser ) == FALSE )
		return;

	CWorldBossManager::GetInstance()->TeleportUserToWorldBoss( pUser );
}
#endif // __ZCK_WORLD_BOSS

#ifdef __ZCK_INVASIONS
void CDPSrvr::OnZckInvasionsRefresh( CAr&, DPID dpidCache, DPID dpidUser, LPBYTE, u_long )
{
	FLWSUser* pUser = g_xWSUserManager->GetUser( dpidCache, dpidUser );
	if( IsValidObj( pUser ) == FALSE )
		return;

	CZckInvasions::GetInstance()->SendRefreshAll( pUser );
}
#endif // __ZCK_INVASIONS
...
void CDPSrvr::OnWikiCreate( CAr & ar, DPID dpidCache, DPID dpidUser, LPBYTE, u_long )
{
	...
	PT_ITEM_SPEC pItemSpec = g_xSpecManager != NULL ? g_xSpecManager->GetSpecItem( dwItemId ) : NULL;
	if( pItemSpec == NULL )
		return;

	if( nNum < 1 )
		nNum = 1;
	if( pItemSpec->dwPackMax > 0 && nNum > static_cast<int>( pItemSpec->dwPackMax ) )
		nNum = static_cast<int>( pItemSpec->dwPackMax );

	FLWSUser* pReceiver = pUser;
	if( szName[0] != _T( '\0' ) )
	{
		u_long idReceiver = CPlayerDataCenter::GetInstance()->GetPlayerId( szName );
		if( idReceiver == 0 )
		{
			pUser->AddText( _T( "Joueur introuvable." ) );
			return;
		}

		pReceiver = g_xWSUserManager->GetUserByPlayerID( idReceiver );
		if( IsValidObj( pReceiver ) == FALSE )
		{
			pUser->AddText( _T( "Le joueur doit etre connecte." ) );
			return;
		}
	}
	...
	if( pReceiver->CreateItem( &itemElem ) == TRUE )
	{
		CString strMessage;
		strMessage.Format( _T( "Objet cree : %s x%d pour %s." ), pItemSpec->szName, nNum, pReceiver->GetName() );
		pUser->AddText( strMessage );
	}
	else
	{
		pUser->AddText( _T( "Impossible de creer l'objet : inventaire plein ou objet refuse." ) );
	}
}
#endif // __ZCK_WIKI

#ifdef __ZCK_TELEPORTEUR
void CDPSrvr::OnZckTeleporteur( CAr& ar, DPID dpidCache, DPID dpidUser, LPBYTE, u_long )
{
    int nKey = -1;
    ar >> nKey;
    ...
    CZckTeleporteur::GetInstance()->DoTeleport( nKey, pUser );
}
#endif // __ZCK_TELEPORTEUR

#ifdef __ZCK_RECORD
void CDPSrvr::OnZckRecordRequest( CAr&, DPID dpidCache, DPID dpidUser, LPBYTE, u_long )
{
	FLWSUser* pUser = g_xWSUserManager->GetUser( dpidCache, dpidUser );
	if( IsValidObj( pUser ) == FALSE )
		return;

	CZckRecordManager::GetInstance()->SendSnapshot( pUser );
}
#endif // __ZCK_RECORD
...
void CDPSrvr::OnTeleportByAPP(CAr & ar, DPID dpidCache, DPID dpidUser, LPBYTE, u_long)
{
	...
	if (IsValidObj(pUser) && (pWorld = pUser->GetWorld()))
	{
		...
		switch (nCurSel)
		{
		case 0: vPos = D3DXVECTOR3(6968.471F, 100.0F, 3329.339F); dwWorldID = 1; break; //Flaris
		case 1: vPos = D3DXVECTOR3(6941.0F, 100.0F, 3101.0F); dwWorldID = 1; break; //Collectorfeld
		case 2: vPos = D3DXVECTOR3(8315.0F, 100.0F, 3724.0F); dwWorldID = 1; break; //SaintMorning
		case 3: vPos = D3DXVECTOR3(9112.0F, 66.0F, 4172.0F); dwWorldID = 1; break; //Garten of Rhisis
		case 4: vPos = D3DXVECTOR3(3808.0F, 59.0F, 4455.0F); dwWorldID = 1; break; //Darkon 2
		case 5: vPos = D3DXVECTOR3(5583.0F, 75.0F, 3891.0F); dwWorldID = 1; break; //Darkon 1
		case 6: vPos = D3DXVECTOR3(3222.0F, 10.0F, 3428.0F); dwWorldID = 1; break; //Darkon 3
		case 7: vPos = D3DXVECTOR3(5947.0F, 100.0F, 5406.0F); dwWorldID = 1; break; //Khaldera
		case 8: vPos = D3DXVECTOR3(7065.0F, 82.0F, 6427.0F); dwWorldID = 1; break; //Knochenpfad
		case 9: vPos = D3DXVECTOR3(7263.0F, 66.0f, 9125.0F); dwWorldID = 1; break; //Kailun Grasland
		case 10: vPos = D3DXVECTOR3(6006.0F, 72.0F, 9253.0F); dwWorldID = 1; break; //Bahara Wuste
		case 11: vPos = D3DXVECTOR3(736.0F, 100.0F, 825.0F); dwWorldID = 234; break; // Flaris Market
		case 12: vPos = D3DXVECTOR3(5270.0F, 100.0F, 4723.0F); dwWorldID = 1; break; //Tramnuk-Abbild
		case 13: vPos = D3DXVECTOR3(4899.0F, 100.0F, 3458.0F); dwWorldID = 1; break; //Tramnuk
		case 14: vPos = D3DXVECTOR3(6644.0F, 58.0F, 6454.0F); dwWorldID = 1; break; //Aminuskrypta
		case 15: vPos = D3DXVECTOR3(6804.0F, 100.0F, 6065.0F); dwWorldID = 1; break; //Verfluchte Aminuskrypta
		case 16: vPos = D3DXVECTOR3(6374.0F, 100.0F, 8040.0F); dwWorldID = 1; break; //Chrystal Labyrinth
		case 17: vPos = D3DXVECTOR3(5734.0F, 87.0F, 8145.0F); dwWorldID = 1; break; //Kalgas Hohle
		case 18: vPos = D3DXVECTOR3(8243.0F, 70.0F, 9343.0F); dwWorldID = 1; break; //Tempel des Behemoths)
		case 19: vPos = D3DXVECTOR3(1396.0F, 86.0F, 569.0F); dwWorldID = 203; break; // //Volcane
		case 20: vPos = D3DXVECTOR3(2686.0F, 150.0F, 5223.0F); dwWorldID = 1; break; //Euphresia
		case 21: vPos = D3DXVECTOR3(2814.0F, 150.0F, 5244.0F); dwWorldID = 1; break; //Euphresia M
		case 22: vPos = D3DXVECTOR3(2064.0F, 100.0F, 2280.0F); dwWorldID = 1; break; //Herneos
		case 23: vPos = D3DXVECTOR3(1853.9F, 100.0F, 2755.3F); dwWorldID = 1; break; //Herneos M
		case 24: vPos = D3DXVECTOR3(5177.1F, 100.0F, 5269.2F); dwWorldID = 1; break; //Sanpres M
		case 25: vPos = D3DXVECTOR3(6570.9F, 100.0F, 5082.0F); dwWorldID = 1; break; //Sanpres M
		case 26: vPos = D3DXVECTOR3(7616.7F, 155.2F, 4275.3F); dwWorldID = 1; break; //Mas Dungeon M

		default: FLERROR_LOG(PROGRAM_NAME, _T("이벤트 아레나에서는 듀얼이 불가능 합니다. User: %s")); return;
		}
		...
		else if (pUser->GetWorld()->GetID() == WI_WORLD_ARENA)
		{
			return;
		}
		else
