...
// nProbability * GetItemDropRateFactor( pAttacker );/* ¿Ã∫•∆Æ */
...
float FLDoDrop::GetItemDropRateFactor( CMover * pMonster, FLWSUser & kAttacker, const FLDropSet::E_DROP_SET_KIND eSetKind )
{
	...
	{
		// º¯»∏±ÿ¥‹¿Ã∏Á ∏µ≈©æÓ≈√¡ﬂ¿Ã∏Á ¥‹¿Â∏∏ µ•πÃ¡ˆ∏¶ ≥Ùø©¡ÿ¥Ÿ
...
float FLDoDrop::GetGoldDropRateFactor( CMover & kMonster, FLWSUser & kAttacker )
{
	...
	//@@@@@@@@ ∫∏¡§ƒ°
	//	int	nSeedID = 0;
	//	int nNumGold = lpDropItem->dwNumber + xRandom( lpDropItem->dwNumber2 - lpDropItem->dwNumber );	// Number ~ Number2 ªÁ¿Ã¿« ∑£¥˝∞™.
	//nNumGold	= nNumGold * nPenyaRate / 100;		//∫∏¡§ƒ°	
	...
	nNumGold	= (int)( nNumGold * prj.m_EventLua.GetGoldDropFactor() );	//sun: 9, ¿Ã∫•∆Æ (∑Áæ∆ Ω∫≈©∏≥∆Æ ¿˚øÎ)
	*/
...
void FLDoDrop::DropItem( CMover & kMonster, FLWSUser & kAttacker, const DWORD dwOwnerPlayerID, const DWORD dwItemID, const int nItemCount )
{
	...
	if( !addItemToFlyingAttacker( kMonster, kAttacker, dwItemID, nItemCount ) ) {
		...
		if( dwEnhanceProb > 2100000000 && ( pItemProp->dwItemKind1 == IK1_WEAPON || pItemProp->dwItemKind1 == IK1_ARMOR ) )
		{
			...
			if ( !pItemProp->IsBaruna() && !pItemProp->IsUltimate() )	pItemElem->SetAbilityOption( dwEnhanceRandom+1 );		// AÉAüA°∆A¢Æ A¢•AâA°§AÇAÑA¢Æ +1, +2 A°∆A°∆AÄA®¨A°∆AÖ.
			else if ( dwEnhanceProb > 2700000000)						pItemElem->SetAbilityOption( 1 );		// AÉAüA°∆A¢Æ A¢•AâA°§AÇAÑA¢Æ +1, +2 A°∆A°∆AÄA®¨A°∆AÖ.
		...
		for ( int i = 0; i < nSize; i++ )
		{
			if( ( lpDropItem = lpMoverProp->m_DropItemGenerator.GetAt( i, FALSE, 100.0f ) ) != NULL )
			{
				if ( lpDropItem->dwIndex == dwItemID )
					pItemElem->SetAbilityOption( lpDropItem->dwLevel );		// AÉAüA°∆A¢Æ A¢•AâA°§AÇAÑA¢Æ +1, +2 A°∆A°∆AÄA®¨A°∆AÖ.
			}
		}
		// end
		...
		pItem->m_idOwn			= dwOwnerPlayerID;		// AÄAå A®˙AÜAÄAåAÖAõAÄAá A®˘AíAÄA?A°∆A¢Æ pAttacker(A®˙A¢ÁAÖAÇAÑA¢Ø)A©˜A°ßA¢“A•ÏA°∆Aâ AáA°ÕA®ˆAÉ.
		pItem->m_dwDropTime		= ::GetTickCount();		// A•ÏA°ÕA¢“A¢¨ AáAüAÄA°ÌA¢•A°◊A®ˆAÉAÄAá A®ˆAÉA°∆A°ÃAÄA°Ì A°æA°ÀA°§AèAáAî.
		pItem->m_bDropMob		= TRUE;		// A¢¨A°§AÄAå AÅAóA®˙A¢ÁA®˘A°© A¢“A©¯A°æA®¨ A•ÏA°§AÄA®¨ AáA°ÕA®ˆAÉA¢¨A| AáAòA•ÏAí.
...
// 		vPos.y					+= 0.1f;									//A¢¨A©¯A®ˆA®¨AÖAç yA®¨AéAÖAç AÉA®˘AÖA®œAáAèA¢¨A®œ A©ˆAôA¢•AöAÄA¢¨A°§Aé A¢“A©¯A®˙A¢ÁAÅA¢¨A¢•Aô.vPos.y
// 		vPos.y					= pWorld->GetUnderHeight( vPos );
		...
		if( pItemProp->dwItemKind1 == IK1_WEAPON || pItemProp->dwItemKind1 == IK1_ARMOR || ( pItemProp->dwItemKind1 == IK1_GENERAL && pItemProp->dwItemKind2 == IK2_JEWELRY ) )
		{
			switch( pItemProp->nLog )
			{
			case 1:		g_dpDBClient.SendLogUniqueItem( &kAttacker, pItem, pItemElem->GetAbilityOption() );	// AÄAèA©ˆAù A®˙AÜAÄAåAÖAõ +A®˙AÜAÄAåAÖAõ
				break;
			case 2:		g_dpDBClient.SendLogUniqueItem( &kAttacker, pItem, 200 );	// AÄAåAÜA°ÕAÜA¢Á A®˙AÜAÄAåAÖAõ
				break;
			case 3:		g_dpDBClient.SendLogUniqueItem( &kAttacker, pItem, 100 );	// AÄA?A¢•AèAÖA®œ A®˙AÜAÄAåAÖAõ
				break;
			}
		}
	}
...
void FLDoDrop::DropGold( CMover & kMonster, FLWSUser & kAttacker, const DWORD dwOwnerPlayerID, const int nGold )
{
	...
	if( !addGoldToFlyingAttacker( kMonster, kAttacker, nGold ) ) {
		FLItemElem* pItemElem	= new FLItemElem;
		pItemElem->m_dwItemId	= dwItemID;
		pItemElem->m_nItemNum	= nGold;	// µ∑æ◊ºˆ
		pItemElem->m_nHitPoint	= nGold;

		CItem* pItem			= new CItem;
		pItem->m_pItemBase		= pItemElem;
		pItem->m_idOwn			= dwOwnerPlayerID;	// ¿Ã æ∆¿Ã≈€¿« º“¿Ø∞° pAttacker(æÓ≈¬ƒø)≤®∂ı∞… «•Ω√.
		pItem->m_dwDropTime		= ::GetTickCount();	// µÂ∂¯ «ﬂ¿ª¥ÁΩ√¿« Ω√∞£¿ª ±‚∑œ«‘.
		pItem->m_bDropMob = TRUE;		// ∏˜¿Ã ¡◊æÓº≠ ∂≥±∫ µ∑¿∫ «•Ω√∏¶ «ÿµ“.
		pItem->SetIndex( D3DDEVICE, pItem->m_pItemBase->m_dwItemId );

		D3DVECTOR vPos			= kMonster.GetPos();
		vPos.x					+= ( xRandomF( 2.0f ) - 1.0f );
		vPos.z					+= ( xRandomF( 2.0f ) - 1.0f );
// 		vPos.y					+= 0.1f;									//∏ÛΩ∫≈Õ y∫Œ≈Õ √º≈©«œ∏È πŸ¥⁄¿∏∑Œ ∂≥æÓ¡¯¥Ÿ.vPos.y
// 		vPos.y					= pWorld->GetUnderHeight( vPos );
		pItem->SetPos( vPos );
		if( pWorld->ADDOBJ( pItem, TRUE, kAttacker.GetLayer() ) == FALSE )
		{
...
//	}
//	pItemElem->SetAbilityOption( lpDropItem->dwLevel );		// √ﬂ∞° ¥…∑¬ƒ° +1, +2 ∞∞¿∫∞≈.
//	pItemElem->SetSerialNumber();
//	CItem* pItem	= new CItem;
//	pItem->m_pItemBase	= pItemElem;
//	BOOL bJJim = TRUE;		// ¡÷¿Œ¿Ã ¿÷¥Ÿ¥¬∞… «•Ω√«ÿæﬂ«œ¥¬ ≈€¿Œ∞°.
//	if( lpMoverProp->dwClass == RANK_SUPER )		// ∫∏Ω∫∏˜¿Ã µÂ∑”«— æ∆¿Ã≈€¿∫ æ∆π´≥™ ∏‘¿ªºˆ ¿÷¥Ÿ.
//		bJJim = FALSE;
//	if( GetIndex() == OBJECT_INDEX( 196, MI_DEMIAN5 ) || GetIndex() == OBJECT_INDEX( 197, MI_KEAKOON5 ) || GetIndex() == OBJECT_INDEX( 198, MI_MUFFRIN5 ) )
//		bJJim = FALSE;		// æÍµÈ¿∫ ¿Ã∫•∆Æ∏˜¿Ãπ«∑Œ ¬Úæ»«ÿ≥ˆµµ µ»¥Ÿ. æ∆π´≥™ ∏‘¿ªºˆ ¿÷¿Ω
//	if( bJJim )
//	{
//		pItem->m_idOwn = pAttacker->GetId();	// ¿Ã æ∆¿Ã≈€¿« º“¿Ø∞° pAttacker(æÓ≈¬ƒø)≤®∂ı∞… «•Ω√.
//		pItem->m_dwDropTime = timeGetTime();	// µÂ∂¯ «ﬂ¿ª¥ÁΩ√¿« Ω√∞£¿ª ±‚∑œ«‘.
//	}
//	pItem->m_bDropMob = TRUE;		// ∏˜¿Ã ¡◊æÓº≠ ∂≥±∫ µ∑¿∫ «•Ω√∏¶ «ÿµ“.
...
//	// ¿Ã∫•∆Æ ∏ÛΩ∫≈Õ∞° µÂ∂¯«— æ∆¿Ã≈€¿∫ ∏ÛΩ∫≈Õ¿« ID∏¶ ±‚æÔ«—¥Ÿ(∆Í¿Ã ∏¯ ¡›∞‘...)
...
//		switch( pItemProp->nLog )
//		{
//		case 1:		g_dpDBClient.SendLogUniqueItem( pAttacker, pItem, pItemElem->GetAbilityOption() );	// ¿œπ› æ∆¿Ã≈€ +æ∆¿Ã≈€
//			break;
//		case 2:		g_dpDBClient.SendLogUniqueItem( pAttacker, pItem, 200 );	// ¿Ã∆Â∆Æ æ∆¿Ã≈€
//			break;
//		case 3:		g_dpDBClient.SendLogUniqueItem( pAttacker, pItem, 100 );	// ¿Ø¥œ≈© æ∆¿Ã≈€
//			break;
//		}
//	}
//
...
//	else
//	{
//		// µ∑æ◊ºˆø° µ˚∂Û æÓ∂≤∏æÁ¿« Ω√µÂ∏¶ ªÁøÎ«“¡ˆ ∞·¡§«—¥Ÿ.
...
//		pItemElem->m_nItemNum	= nNumGold;	// µ∑æ◊ºˆ
...
//	if( lpQuest && lpQuest->m_nState == pQuestItem->dwState )				//ƒ˘Ω∫∆Æ∞° ¿÷∞Ì .  ªÛ≈¬∞° ∞∞¥Ÿ∏È.
//	{
...
//				FLERROR_LOG( PROGRAM_NAME, _T( "%s¿« quest item drop %dπ¯¬∞¿« dwNumber∞° 0" ), GetName(), i );
...
//
//< ∏˜¿Ã ¡◊æ˙¿ª∂ß æ∆¿Ã≈€ ìb±‚ >
//if( IsNPC() )	// ∏˜¿Ã ¡◊æ˙¿ª∂ß.. // ¡÷¿∫ æ∆¿Ã≈€ ìb±‚ ¿ŒµÌ??????
//{
//	MoverProp* pMoverProp	= GetProp();
//	if( pMoverProp->m_nLoot == 2 )	// 2æ∆ dø…º«.  æ∆¿Ã≈€ ∏‘∞Ì πÒ±‚±Ó¡ˆ «œ¥¬ ø…º«.
//	{
