...
//	case OBJMSG_KFORWARD:
	case OBJMSG_FORWARD:
		...
		if( GetDmgState() == OBJSTA_DMG )	// 피격 중 명령이면 피격 상태를 클리어 시킨다.
			...
			return -3;
		if( dwMsg == OBJMSG_FORWARD && GetMoveState() == OBJSTA_FMOVE )				return 0;	// 이미 전진상태면 추가로 처리 하지 않음
...
//sun: 10차 전승시스템	Neuz, World, Trans
		...
		// 키보드이동 변경
#ifdef KEYBOARD_SET		// 여긴 뒷걸음질치는 모션임.
...
//			if( pMover->IsPlayer() )	몬스터도 어택스피드를 가지고 오자..
			{
			...
			switch( dwMsg )
			{
				...
				case OBJMSG_ATK5:	
					...
					pMover->SetMotion( MTI_ATK5, ANILOOP_1PLAY, dwOption );		// 최초 메시지 입력시 동작 설정
					break;
			}
	...
	case OBJMSG_MELEESKILL:
		{
			...
			if( GetState() & OBJSTA_ATK_ALL )
			{
				//	BEGIN20120425 공격 상태일때 스킬 사용 들어오면 리턴 하던 부분 제거
//				return -2;	// 이미 공격중이면 리턴. 서버에선 씹히지 않게 하자.
				//	END20120425
			}
#endif
			pMover->m_dwMotion = static_cast< DWORD >( -1 );	// 나머지 경우는 간단하게 생각하기 위해 모두 클리어 시키고 스킬만 실행하자.
...
#ifdef __ADDWEAPON_CROSSBOW16
			//gmpbigsun(20100908) : 크로스 보우면 모션+200, 모션에 대한 offset값을 데이터로부터 뽑아와야하는데 없음
			...
			if( pProp )
			{
				...
				if( pProp->dwSourceMaterial != NULL_ID || pProp->dwClass == RANK_MATERIAL )	// 자원 몹이면..
				{
					if( pProp->dwHoldingTime == NULL_ID )
						FLERROR_LOG( PROGRAM_NAME, _T( "%s 의 MoverProp::dwHoldingTime 값이 -1" ), pMover->GetName() );
...
//sun: 10차 전승시스템	Neuz, World, Trans
	...
	case OBJMSG_SLEEPING:
		...
		if( GetState() == OBJSTA_STAND )
		{
			pMover->SetMotion( MTI_GROGGY, ANILOOP_LOOP, MOP_FIXED ); // 일단은 스턴과 모션 동일...
		}
		break;
