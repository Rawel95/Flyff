...
void CAIMeteonyker::AddDamage( OBJID idAttacker, int nDamage )
{
	if( nDamage == 0 )
		return;
	std::map<OBJID, int>::iterator	i = m_mapDamage.find( idAttacker );
	if( i == m_mapDamage.end() )
		m_mapDamage.insert( std::map<OBJID, int>::value_type( idAttacker, nDamage ) );
	else
		i->second	+= nDamage;
}
...
OBJID CAIMeteonyker::GetDealer( void )
{
	int nDamage	= 0;
	OBJID objid	= NULL_ID;
	for( std::map<OBJID, int>::iterator i = m_mapDamage.begin(); i != m_mapDamage.end(); ++i )
	{
		if( i->second >= nDamage )
		{
