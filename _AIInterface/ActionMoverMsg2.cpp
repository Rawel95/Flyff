...
//	빗자루 비행용
//
int		CActionMover::ProcessActMsgFly(  CMover* pMover, OBJMSG dwMsg, int nParam1, int nParam2, int nParam3, int nParam4, int nParam5 )
{
	...
	case OBJMSG_STAND:
		{
			...
			if( pItemProp == NULL )
			{
				FLERROR_LOG( PROGRAM_NAME, _T( "ItemProp is NULL in ProcessActMsgFly() %d" ), m_pMover->m_dwRideItemIdx );
			}
	...
	case OBJMSG_FORWARD:
		...
		if( pItemProp == NULL )
		{
			FLERROR_LOG( PROGRAM_NAME, _T( "ItemProp is NULL in ProcessActMsgFly()" ) );
		}
	...
	case OBJMSG_LTURN:
		{
			...
			else if( pItemProp != NULL && fTurnAngle > 1.0f )
			{
	...
	case OBJMSG_DAMAGE:
		{
			...
			pMover->PostAIMsg( AIMSG_DAMAGE, pAttacker->GetId(), nDamage );	//sun: __METEONYKER_0608
			...
			if( pAttacker )
			{
				if( pMover->IsPlayer() && pAttacker->IsPlayer() )	// 죽은놈과 죽인놈이 플레이어고
				{
					if( pMover->m_nDuel == 1 && pMover->m_idDuelOther == pAttacker->GetId() )	// 둘이 듀얼중이었으면
					{
						...
						FLTRACE_LOG( PROGRAM_NAME, _T( "m_nDuel		= 0		// OBJMSG_DIE	// ProcessActMsgFly" ) );
...
//			float f		= 2.0f * m_fDistance / 0.10f;
			float f		= m_fDistance * 0.2f;
