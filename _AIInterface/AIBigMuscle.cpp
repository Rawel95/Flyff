...
static const TCHAR LOG_BOSS_AI_BIGMUSCLE[] = _T( "Boss(BigMuscle)" );
#endif
...
//
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
MEMPOOLER_IMPLEMENT( CAIBigMuscle, 4 );
...
#ifdef __INTERNALSERVER
	FLERROR_LOG( LOG_BOSS_AI_BIGMUSCLE, _T( "ClockWorks start" ) );
#endif
}

BOOL CAIBigMuscle::IsIdleMode()
{
...
//	CModelObject *pModel = (CModelObject *)pMover->GetModel();
	...
	{		
		...
		if( fDistSq >= fRadiusSq * 10.0f )				// 라스트어태커가 내 반지름의 10배이상 떨어져있으면
		{
			// 타겟 포기
#ifdef __INTERNALSERVER
			FLERROR_LOG( LOG_BOSS_AI_BIGMUSCLE, _T( "LastAtker(%s)가 너무 멀어서(%5.2f) 타겟포기함" ), pLastAttacker->GetName(), sqrt(fDistSq) );
#endif
...
#ifdef __INTERNALSERVER
				FLERROR_LOG( LOG_BOSS_AI_BIGMUSCLE, _T( "scanTarget:%s" ), pTarget->GetName() );
#endif
	...
	// 날 때린 쉐리가 있다.
	{
		...
		if( dwNum < 75 )
		{
			...
			if( pTarget )
			{
				...
				if( pMover->IsFlyingNPC() || pTarget->m_pActMover->IsFly() == FALSE )	
				{
					m_idTarget = pTarget->GetId();
#ifdef __INTERNALSERVER
					FLERROR_LOG( LOG_BOSS_AI_BIGMUSCLE, _T( "Leader:%s %d" ), pTarget->GetName(), pTarget->GetLevel() );
#endif
				}
				else
		...
		if( dwNum < 100 )
		{
			...
			if( pTarget )
			{
				...
				if( pMover->IsFlyingNPC() || pTarget->m_pActMover->IsFly() == FALSE )	
				{
					m_idTarget = pTarget->GetId();
#ifdef __INTERNALSERVER
					FLERROR_LOG( LOG_BOSS_AI_BIGMUSCLE, _T( "OverHealer:%s" ), pTarget->GetName() );
#endif
				}
				else
					m_idTarget = m_idLastAttacker;		// 타겟이 공격하기가 여의치 않으면 마지막으로 때린쉐리 공격하자.
...
#ifdef __INTERNALSERVER
		{
			...
			{
				if( pTarget )
					FLERROR_LOG( LOG_BOSS_AI_BIGMUSCLE, _T( "rand:%d LastAttacker:%s SelectTarget:%s" ), dwNum, pMover->GetName(), pTarget->GetName() );
				else
					FLERROR_LOG( LOG_BOSS_AI_BIGMUSCLE, _T( "rand:%d LastAttacker:%s SelectTarget:%s" ), dwNum, pMover->GetName(), _T( "없음" ) );
			}
			else
			{
				if( pTarget )
					FLERROR_LOG( LOG_BOSS_AI_BIGMUSCLE, _T( "rand:%d LastAttacker:%s SelectTarget:%s" ), dwNum, _T( "없음." ), pTarget->GetName() );
				else
					FLERROR_LOG( LOG_BOSS_AI_BIGMUSCLE, _T( "rand:%d LastAttacker:%s SelectTarget:%s" ), dwNum, _T( "없음." ), _T( "없음" ) );
			}
...
#ifdef __INTERNALSERVER
//		FLERROR_LOG( LOG_BOSS_AI_BIGMUSCLE, _T( "보스의 방어도가 증가되었음. 엔터를 치시고 계속 즐겜을 하시오" ) );
#endif
	}
*/
	...
	if( m_bEventFlag[0] == FALSE )
	{
		...
		if( pTarget )
		{
			...
			g_DPCoreClient.SendCaption( prj.GetText(TID_GAME_BOSS_BIGMUSCLE_MSG_01), pTarget->GetWorld()->GetID(), FALSE, pTarget->m_idparty );	
			
			g_xWSUserManager->AddWorldShout( pMover->GetName(), prj.GetText(TID_GAME_BOSS_BIGMUSCLE_MSG_02),
				pTarget->GetPos(), pTarget->GetWorld() );
			g_xWSUserManager->AddWorldShout( pMover->GetName(), prj.GetText(TID_GAME_BOSS_BIGMUSCLE_MSG_03),
					pTarget->GetPos(), pTarget->GetWorld() );
			
			g_xWSUserManager->AddCreateSfxObj( pTarget, XI_INDEX( 1745, XI_SKILL_DROP_DUST_RAIN ) );
			g_xWSUserManager->AddCommonPlace( pTarget, COMMONPLACE_QUAKE );

			SkillProp *pSkillProp;
			AddSkillProp *pAddSkillProp;
			BOOL bSuccess = pMover->GetSkillProp( &pSkillProp, &pAddSkillProp, SKILL_INDEX( 228, SI_GEN_BLEEDING ), 10, "CAIBigMuscle::MoveProcessRage" );
			if( bSuccess )
				g_cSkillSystem->ApplySkillAround( pMover, pMover, OBJTYPE_PLAYER, pSkillProp, pAddSkillProp , false, FALSE, 255.0f );										
		}
	}
...
//				FLERROR_LOG( PROGRAM_NAME, _T( "타겟이 없다" ) );
...
#ifdef __INTERNALSERVER
			{
				...
				FLERROR_LOG( LOG_BOSS_AI_BIGMUSCLE, _T( "AtkType:%s Dist:%5.2f" ), strType, sqrt(fDistSq) );
			}
#endif
