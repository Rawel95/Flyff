...
//	CModelObject *pModel = (CModelObject *)pMover->GetModel();
	...
	{		
		...
		if( fDistSq >= fRadiusSq * 10.0f )				// 라스트어태커가 내 반지름의 10배이상 떨어져있으면
		{
			// 타겟 포기
#ifdef __INTERNALSERVER
			FLERROR_LOG( LOG_BOSS_AI_BEAR, _T( "LastAtker(%s)가 너무 멀어서(%5.2f) 타겟포기함" ), pLastAttacker->GetName(), sqrt(fDistSq) );
#endif
...
#ifdef __INTERNALSERVER
		{
			...
			if( pMover )
			{
				if( pTarget )
					...
					FLERROR_LOG( LOG_BOSS_AI_BEAR, _T( "rand:%d LastAttacker:%s SelectTarget:%s" ), dwNum, pMover->GetName(), _T( "없음" ) );
			}
			else
			{
				if( pTarget )
					FLERROR_LOG( LOG_BOSS_AI_BEAR, _T( "rand:%d LastAttacker:%s SelectTarget:%s" ), dwNum, _T( "없음." ), pTarget->GetName() );
				else
					FLERROR_LOG( LOG_BOSS_AI_BEAR, _T( "rand:%d LastAttacker:%s SelectTarget:%s" ), dwNum, _T( "없음." ), _T( "없음" ) );
			}
