	...
	if( m_bMove )	
	{
		...
		if( pMover == g_pPlayer )		//주인공일경우 
		{
			...
			DWORD dwMoveState = GetMoveState( );
				...
				{
					if ( (dwMoveState == OBJSTA_FMOVE)
					|| (dwMoveState == OBJSTA_BMOVE) 
					|| (dwMoveState == OBJSTA_LEFT)
					|| (dwMoveState == OBJSTA_RIGHT)
					|| (dwMoveState == OBJSTA_LMOVE)
					|| (dwMoveState == OBJSTA_RMOVE) ) 
					{
						if( !IsStateFlag( OBJSTAF_WALK ) ) 
							COLL_TEST_DIR = 8.0f;
					}
		...
		BOOL bColl	= TRUE;
		if( pMover->IsNPC() && (pMover->GetIndex() >= OBJECT_INDEX( 720, MI_PET_LAWOLF ) && pMover->GetIndex() <= OBJECT_INDEX( 749, MI_PET_LASTINDEX )) )
		{
...
//sun: 9,	//__AI_0509
...
#ifdef __CLIENT
		if( pMover->GetId() == NULL_ID )	//sun: 9, 9-10차 펫 // 클라이언트에서만 생성한 객체면,
			bColl	= FALSE;
#endif	// __CLIENT

		bColl = TRUE;

#if defined (__NPCPORTBUG) || defined (__JUST_TEST01)
		if (pMover->IsPlayer() || (pMover->GetWorld()->GetID() >= WI_DUNGEON_SECRET_0 && pMover->GetWorld()->GetID() <= WI_DUNGEON_SECRET_16) && pMover->IsPeaceful() == FALSE && pMover->m_dwAIInterface != AII_PET && pMover->m_dwAIInterface != AII_EGG)
#else // __NPCPORTBUG 
	...
	if( vDelta.y < 0.0f )		// 좌표가 내려갔음
	{
		...
		else	
		{
			...
			{
#ifdef __ARROW_FIX
				if (vDelta.y < -1.5f)		// 떨어지는 속도가 0.x보다 클때만 하강동작.  살짝 떨어지는건 걍 좌표만 내려감.
#else //__ARROW_FIX
				if (vDelta.y < -0.1f)
#endif //__ARROW_FIX
				{
