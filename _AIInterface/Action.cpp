...
//	ResetState( OBJSTA_MOVE_ALL | OBJSTA_TURN_ALL | OBJSTA_LOOK_ALL | OBJSTA_JUMP_ALL | OBJSTA_ATK_ALL | OBJSTA_DMG_ALL );
	...
	if( m_dwState )
	{
		FLERROR_LOG( PROGRAM_NAME, _T( "아직도 뭔가 클리어되지 않은 값이 있다. %08x, %08x" ), dwState, m_dwState );
		m_dwState = 0; 
	}
}
...
void CAction::SetState( DWORD dwType, DWORD dwState )
{
	ResetState(dwType);	m_dwState |= dwState;
}

void CAction::__ForceSetState( DWORD dwState )
{
	m_dwState = dwState;
}

void CAction::SetMoveState( DWORD dwState )
{
	ResetState(OBJSTA_MOVE_ALL);	m_dwState |= dwState;
}
