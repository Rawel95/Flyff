...
//sun: 10차 전승시스템	Neuz, World, Trans
	...
	case OBJSTA_SIT:
		...
		else if( pMover->m_dwMotion == MTI_GETUP )		// 일어나는중
		{
			if( pModel->IsEndFrame() )
			{
	...
	case OBJSTA_COLLECT:
		...
		if( pMover->IsNPC() )
		{
			FLERROR_LOG( PROGRAM_NAME, _T( "OBJSTA_COLLECT : 웬 몬스터가 채집으로 들어왔다 %s" ), pMover->GetName() );
			break;
		}
 #ifdef __WORLDSERVER
		if( pMover->OnActCollecting() == TRUE || IsMove() )		// 채집동작이 끝나면 / 채집하다 움직이면.
		{
			...
			g_xWSUserManager->AddHdr( (FLWSUser *)pMover, SNAPSHOTTYPE_STOP_COLLECT );	// 채집끝남.
		}
...
//		}
	...
	case OBJSTA_SJUMP4:
		...
		if( pModel->IsEndFrame() || ( GetState() & OBJSTA_DMG_ALL ) )	// 착지동작 다 끝났거나, 데미지 중이었으면 착지 상태 해제시키고 끝냄.
		{
			ResetState( OBJSTA_JUMP_ALL );
#ifdef __CLIENT
			// OnActLanding() 으로부터 이전 // chipi_2012/09/12
...
void	CActionMover::ProcessStateMeleeSkill( DWORD /*dwState*/, int /*nParam*/ )
{
	...
	if( nSkill == SKILL_INDEX( 204, SI_ACR_YOYO_COUNTER ) )	// 싸이런 스트라이크는 예외다.
	{
	}
	else if( nSkill == SKILL_INDEX( 138, SI_BLD_DOUBLESW_SILENTSTRIKE ) )	// 싸이런 스트라이크는 예외다.
	{
		if( pModel->IsEndFrame() )
		{
			ResetState( OBJSTA_ATK_ALL );
	...
	if( (g_nProcessCnt & 3) == 0 )
	{
		...
		{
			g_cSkillSystem->OnMeleeSkill( pMover, 0 );	// 다시 발사시키고
	...
	else
	{
		...
		{
			if( nSkill == SKILL_INDEX( 138, SI_BLD_DOUBLESW_SILENTSTRIKE ) )	// 싸이런 스트라이크는 예외다.
			{
				D3DXVECTOR3 vForce;
				AngleToVectorXZ( &vForce, pMover->GetAngle(), 0.75f );		// 앞쪽으로 돌진하는 힘 설정.
				SetExternalForce( vForce );
			} 
			else if( nSkill == SKILL_INDEX( 152, SI_BIL_KNU_BELIALSMESHING ) )	// 베리알 스매싱도 앞으로 전진.
			{
				D3DXVECTOR3 vForce;
			...
			g_cSkillSystem->OnMeleeSkill( pMover, 0, m_nMotionHitCount );	// 무버의 핸들러를 호출.
			m_nMotionHitCount++;
...
void	CActionMover::ProcessStateMagicSkill( DWORD /*dwState*/, int /*nParam*/ )
{
	...
	if( (g_nProcessCnt & 3) == 0 )
	{
		if( m_nShootCnt > 0 )		// 연속발사가 세팅되면
		{
			g_cSkillSystem->OnMagicSkill( pMover, 0 );	// 다시 발사시키고
			m_nShootCnt--;		// 개수 하나씩 뺌.
		}
	}
	...
	{
		g_cSkillSystem->OnMagicSkill( pMover, 0, m_nMotionHitCount );	// 무버의 핸들러를 호출.
		m_nMotionHitCount++;
				...
				if( GetMover()->IsActiveMover() )
				{
					if( (pMover->GetLevel() >= nReqLevel) && ((m_dwCtrlMsg & CTRLMSG_LDOWN) || ((CWndWorld *)g_WndMng.m_pWndWorld)->m_bAutoAttack) )	// 직업레벨 X이상이면 다음연타가능
					{
						...
						if( IsValidObj( (CObj*)pHitObj ) &&		// 다음 스킬이 예약되어있으면 여기서 끝낸다.
							...
							&& pMover->IsRangeObj( pHitObj, pMover->GetAttackRange( pItemProp != NULL ? pItemProp->dwAttackRange : AR_SHORT ) ) == TRUE )	// 공격 가능 거리 내에 있는지 검사.
						{
