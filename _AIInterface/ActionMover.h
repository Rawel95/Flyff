...
//sun: 10차 전승시스템	Neuz, World, Trans
	...
	void			ProcessStateAttackMelee( DWORD dwState, int nParam = 0 );	// 공격부분은 코드가 길어서 따로 빼냄
	void			ProcessStateAttackMeleeFly( DWORD dwState, int nParam = 0 );
	...
	int				ProcessActMsg( CMover* pMover, OBJMSG dwMsg, int nParam1, int nParam2, int nParam3, int nParam4, int nParam5 );
	int				ProcessActMsgFly( CMover* pMover, OBJMSG dwMsg, int nParam1, int nParam2, int nparam3, int nParam4, int nParam5 );
	void			ProcessState( CMover* pMover, DWORD dwState, float fSpeed );
	void			ProcessStateFly( CMover* pMover, DWORD dwState, float fSpeed );
	void			ProcessFlyMove();
	...
	MEMPOOLER_DECLARE( CActionMover );
