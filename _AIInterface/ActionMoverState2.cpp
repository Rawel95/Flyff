...
void CActionMover::ProcessFlyParticle( float fLenSq )
{
	...
	{
		PT_ITEM_SPEC pRideProp = g_xSpecManager->GetSpecItem( m_pMover->m_dwRideItemIdx );		// 현재 타고있는 탈것의 프로퍼티.
		// 비행 파티클 처리.
		if( fLenSq > 0.01f )
		{
		...
		if( fLenSq > 0.001f  )
		{
			if( (pMover->IsActiveMover() && g_Neuz.m_camera.m_fZoom > 1.0f) || pMover->IsActiveMover() == FALSE )
			{
				if( pRideProp && pRideProp->dwItemKind3 == IK3_BOARD )		// 보드만 꼬리가 나온다.
				{
					if( m_pTail )
					{
						if( pRideProp->dwID == ITEM_INDEX( 5800, II_RID_RID_BOR_RIDINGCLOUD ) ) // 근두운일때
						{
					...
					{
						if( pRideProp->dwID == ITEM_INDEX( 5800, II_RID_RID_BOR_RIDINGCLOUD ) ) // 근두운...
						{
...
void	CActionMover::ProcessFlyMove( void )
{
	...
	// gmpbigsun( 20120802 ) : 최고속도에서 마찰력때문에 감속 현상 수정
