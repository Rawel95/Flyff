...
#ifdef __POSI_BUG_FIX
	m_MovementStage = MOVEMENT_INIT;
	m_tsMovementUpdate = TIMEGETTIME;
#endif
}
...
#ifdef __POSI_BUG_FIX
void CAIMonster::FF_MoveToVarSpot(DWORD const& MovementStage, OBJID const& dwIdTarget, float const& fRange, bool const& bKeepRange, const D3DXVECTOR3& vDst)
{
	CMover* pMover = GetMover();
#if __VER >= 9
	pMover->SetSpeedFactor(2.0F);
#endif //__VER >= 9

	CMover* pTarget = prj.GetMover(dwIdTarget);
	if (IsValidObj(pTarget)) {
		pMover->ClearDest();
		pMover->SetDestObj(dwIdTarget, fRange);
		pMover->m_nCorr = -1;

		SetTarget(pTarget->GetId(), pTarget->m_idparty);
		if (m_MovementStage == MovementStage)
		{
			if (m_tsMovementUpdate < TIMEGETTIME) {
				m_tsMovementUpdate = TIMEGETTIME + 500;
				g_xWSUserManager->FF_MoveToVarSpot(pMover, dwIdTarget, fRange, bKeepRange, vDst);
			}
		}
		else
		{
			m_MovementStage = MovementStage;
			m_tsMovementUpdate = TIMEGETTIME + 500;
			g_xWSUserManager->FF_MoveToVarSpot(pMover, dwIdTarget, fRange, bKeepRange, vDst);
		}
	}
...
#ifdef __POSI_BUG_FIX_2
			if (pMover->IsRangeObj(pTarget, fRange))
#else //__POSI_BUG_FIX_2
...
void CAIMonster::SubSummonProcess( CMover * /*pTarget*/ )
{
	...
	{
		for( std::vector<OBJID>::iterator it = m_vecIdSummon.begin(); it!=m_vecIdSummon.end(); )
		{
...
//		int nAttackFirstRange	= pProp->m_nAttackFirstRange;
	...
	if( TIMEGETTIME > m_tmAttack )	// 공격 못한지 20초 지났냐?
	{
		...
		vDist.y	= 0;	
#ifndef __NPCPORTBUG// 옆으로 살짝움직였는데 산위로 확 올라갔을땐 거리가 엄청 멀수 있다. 그래서 이렇게 한것.
	...
	else
	{
		...
		if( m_bRangeAttack )	// 원거리 공격으로 결정됐음.
		{
			...
			{
				FLERROR_LOG(PROGRAM_NAME, _T("CAIMonster::MoverProcessRage : 범위침범 %d"), nTargetJob);
				nTargetJob = 0;
			}
			...
			// Reduce Range for movement rounding error
			fRange -= 0.5f;

			if (pMover->IsRangeObj(pTarget,fRange) == FALSE)
				FF_MoveToVarSpot(MOVEMENT_FAR_TO_FAR,pTarget->GetId(), fRange, false, pTarget->GetPos());
			else
				m_MovementStage = MOVEMENT_FAR;
		}
		else
		{
			if (pMover->IsRangeObj(pTarget, 1.0f) == TRUE)
				m_MovementStage = MOVEMENT_MELEE;
			else
				FF_MoveToVarSpot(MOVEMENT_MELEE_TO_FAR, pTarget->GetId(), 0.0f, true, pTarget->GetPos());
		}
#else //__POSI_BUG_FIX
		if( m_bRangeAttack )	// 원거리 공격으로 결정됐음.
		{
