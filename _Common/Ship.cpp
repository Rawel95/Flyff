...
{	
	...
	if( bSuccess == FALSE )
		FLERROR_LOG( PROGRAM_NAME, _T( "Add실패." ) );
}
...
void CShip::Control( void )
{
	...
	BOOL bAcc	= g_bKeyTable[ pShortcutKey_Control->KeyJump	];
	BOOL bLeft	= g_bKeyTable[ pShortcutKey_Control->KeyLeft	];
	BOOL bDown	= g_bKeyTable[ pShortcutKey_Control->KeyUp		];
	BOOL bRight = g_bKeyTable[ pShortcutKey_Control->KeyRight	];
	BOOL bUp	= g_bKeyTable[ pShortcutKey_Control->KeyDown	];
...
void CShipLoop::Process()
{
	...
	if( (m_nCount & 127) == 0 )
	{
		...
		g_xWSUserManager->AddSetPosAngle( this, GetPos(), GetAngle() );		// 먼저 this(Ship)의 위치를 sync시킴.
		// 링크되어 있는 모든 ctrl의 위치를 다시 sync시킴.
		for( int i = 0; i < MAX_LINKCTRL; i ++ )		
		{
			...
			if( IsValidObj( pCtrl ) )
			{
				if( pCtrl->GetIAObjLink() == this )
					g_xWSUserManager->AddSetPosAngle( pCtrl, pCtrl->GetPos(), pCtrl->GetAngle() );		// this(Ship)의 위치를 sync시킴.
				else
