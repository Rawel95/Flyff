...
#if defined(__ZCK_STAFF_GRADE) && defined(__WORLDSERVER)
#include "authorization.h"
#include <mutex>
#endif // __ZCK_STAFF_GRADE && __WORLDSERVER
...
#include "RainbowRace.h"	//sun: 13, ·¹ÀÎº¸¿ì ·¹ÀÌ½º
#include "Quiz.h"
...
DWORD	CProject::m_dwLegendSP = 4;				// 3Â÷Á÷¾÷ ½ºÅ³ÀÇ ·¹º§¾÷¶§ ÇÊ¿äÇÑ SPÆ÷ÀÎÆ®
...
CProject::CProject()
#ifdef __WORLDSERVER
//sun: 12, A†A¨ÏA…A¢´A¢¬A¢çA¨úA©ø A¡ÆA©øA¨ùA¡¾
:
m_nMaxSequence( 0 )
#endif	// __WORLDSERVER
{
	...
	while(pos)
	{
		...
		for( j = 0; j < MAX_VENDOR_INVENTORY_TAB; j++ )
		{
			for( i = 0; i < lpCharacter->m_venderItemAry3[ j ].GetSize(); i++)
				SAFE_DELETE_CONST( (LPVENDOR_ITEM)lpCharacter->m_venderItemAry3[ j ].GetAt(i) );
		}
#endif // __ADDSHOPITEM		
...
namespace
{
	std::mutex& GetZckRebornStaffMutex()
	{
...
DWORD CProject::GetRebornStaffAuthValue( const CString& strAuth ) const
{
	if( strAuth.Compare( _T( "AUTH_OBSERVER" ) ) == 0 )
		return AUTH_OBSERVER;
	if( strAuth.Compare( _T( "AUTH_GENERAL" ) ) == 0 )
		return AUTH_GENERAL;
	if( strAuth.Compare( _T( "AUTH_LOGCHATTING" ) ) == 0 )
		return AUTH_LOGCHATTING;
	if( strAuth.Compare( _T( "AUTH_JOURNALIST" ) ) == 0 )
		return AUTH_JOURNALIST;
	if( strAuth.Compare( _T( "AUTH_HELPER" ) ) == 0 )
		return AUTH_HELPER;
	if( strAuth.Compare( _T( "AUTH_GAMEMASTER" ) ) == 0 )
		return AUTH_GAMEMASTER;
	if( strAuth.Compare( _T( "AUTH_GAMEMASTER2" ) ) == 0 )
		return AUTH_GAMEMASTER2;
	if( strAuth.Compare( _T( "AUTH_GAMEMASTER3" ) ) == 0 )
		return AUTH_GAMEMASTER3;
	if( strAuth.Compare( _T( "AUTH_OPERATOR" ) ) == 0 )
		return AUTH_OPERATOR;
	if( strAuth.Compare( _T( "AUTH_ADMINISTRATOR" ) ) == 0 )
		return AUTH_ADMINISTRATOR;

	return 0;
}

BOOL CProject::LoadRebornStaff()
{
	TCHAR szModulePath[MAX_PATH] = { 0 };
	const DWORD dwLength = GetModuleFileName( NULL, szModulePath, _countof( szModulePath ) );
	if( dwLength == 0 || dwLength >= _countof( szModulePath ) )
	{
		FLERROR_LOG( PROGRAM_NAME, _T( "LoadRebornStaff - impossible de determiner le chemin du WorldServer. La configuration staff active est conservee." ) );
		return FALSE;
	}

	CString strFilePath( szModulePath );
	const int nLastSlash = strFilePath.ReverseFind( _T( '\\' ) );
	if( nLastSlash < 0 )
	{
		FLERROR_LOG( PROGRAM_NAME, _T( "LoadRebornStaff - chemin du WorldServer invalide : %s. La configuration staff active est conservee." ), (LPCTSTR)strFilePath );
		return FALSE;
	}

	strFilePath = strFilePath.Left( nLastSlash + 1 );
	strFilePath += _T( ".RebornStaff.ini" );

	CStdioFile file;
	CFileException fileError;
	if( file.Open( strFilePath, CFile::modeRead | CFile::shareDenyWrite | CFile::typeText, &fileError ) == FALSE )
	{
		FLERROR_LOG( PROGRAM_NAME, _T( "LoadRebornStaff - impossible d'ouvrir %s. La configuration staff active est conservee." ), (LPCTSTR)strFilePath );
		return FALSE;
	}

	std::map<CString, DWORD> mapLoaded;
	CString strLine;
	int nLine = 0;
	...
	while( file.ReadString( strLine ) )
	{
		...
		const int nHash = strLine.Find( _T( '#' ) );
		const int nSemicolon = strLine.Find( _T( ';' ) );

		if( nHash >= 0 && ( nComment < 0 || nHash < nComment ) )
			nComment = nHash;
		if( nSemicolon >= 0 && ( nComment < 0 || nSemicolon < nComment ) )
			nComment = nSemicolon;

		if( nComment >= 0 )
		{
			strLine = strLine.Left( nComment );
			strLine.Trim();
			if( strLine.IsEmpty() )
				continue;
		}
		...
		int nPos = 0;
		CString strKeyword = strLine.Tokenize( _T( " \t" ), nPos );
		CString strCharacterName = strLine.Tokenize( _T( " \t" ), nPos );
		CString strAuth = strLine.Tokenize( _T( " \t" ), nPos );
		CString strExtra = strLine.Tokenize( _T( " \t" ), nPos );

		if( strKeyword.Compare( _T( "IGN" ) ) != 0 || strCharacterName.IsEmpty() || strAuth.IsEmpty() || strExtra.IsEmpty() == FALSE )
		{
			FLERROR_LOG( PROGRAM_NAME, _T( "LoadRebornStaff - ligne %d invalide et ignoree." ), nLine );
			continue;
		}

		const DWORD dwAuthorization = GetRebornStaffAuthValue( strAuth );
		if( dwAuthorization == 0 )
		{
			FLERROR_LOG( PROGRAM_NAME, _T( "LoadRebornStaff - ligne %d : AUTH inconnu '%s', ligne ignoree." ), nLine, (LPCTSTR)strAuth );
			continue;
		}

		if( mapLoaded.find( strCharacterName ) != mapLoaded.end() )
		{
			FLINFO_LOG( PROGRAM_NAME, _T( "LoadRebornStaff - ligne %d : doublon pour '%s', la premiere valeur valide est conservee." ), nLine, (LPCTSTR)strCharacterName );
			continue;
		}

		mapLoaded.insert( std::map<CString, DWORD>::value_type( strCharacterName, dwAuthorization ) );
	}

	file.Close();
	const unsigned int nLoadedCount = static_cast<unsigned int>( mapLoaded.size() );

	{
		std::lock_guard<std::mutex> lock( GetZckRebornStaffMutex() );
		m_mapRebornStaff.swap( mapLoaded );
	}

	FLINFO_LOG( PROGRAM_NAME, _T( "LoadRebornStaff - %u entree(s) chargee(s) depuis %s." ), nLoadedCount, (LPCTSTR)strFilePath );
	return TRUE;
}

BOOL CProject::TryGetRebornStaffAuthorization( LPCTSTR lpszCharacterName, DWORD& dwAuthorization ) const
{
	if( lpszCharacterName == NULL || lpszCharacterName[0] == _T( '\0' ) )
		return FALSE;

	CString strCharacterName( lpszCharacterName );
	strCharacterName.Trim();
	if( strCharacterName.IsEmpty() )
		return FALSE;

	std::lock_guard<std::mutex> lock( GetZckRebornStaffMutex() );
	std::map<CString, DWORD>::const_iterator it = m_mapRebornStaff.find( strCharacterName );
	if( it == m_mapRebornStaff.end() )
		return FALSE;

	dwAuthorization = it->second;
	return TRUE;
}
#endif // __ZCK_STAFF_GRADE && __WORLDSERVER
...
BOOL CProject::OpenProject( LPCTSTR lpszFileName )
{
	...
	CItemUpgrade::GetInstance();		//preloading A¡Ë¢ç¢®¡¿IoAŒA¢®E¡ËcE¢®E¡Ëc¡§I¢®¨ÏA¢®E¡ËcEc A€AA¢®E¡Ëc¡Ë¢ç¢®¢¯uA¢®E¡ËcEc data A¡Ë¢çE¢®Ec¡Ë¢ç¢®¡¿¢®E¡ËcA‹AA¢®E¡ËcIi.
	...
	//sun: AA|A¡Ë¢çE¢®Ec¡Ë¢çE¢®E¢®IAƒ A€AŒA†A‘A†A¢®E¡ËcEc A‡A¡Ë¢çE¢®EcEA¡Ë¢çE¢®EciA¡Ë¢ç¢®¡¿I¢®¡×I¢®¡×¢®I A¡Ë¢ç¢®¡¿IoA¢®E¡ËcEOA€A¢®E¡Ëc¡Ë¢ç¢®¢¯u
...
#ifdef __ZCK_ARME_RARE
	if( LoadZckArmeRare( _T( "ZckArmeRare.inc" ) ) == FALSE )
		return FALSE;
#endif // __ZCK_ARME_RARE
...
#ifdef __CLIENT	// A…A¡Ë¢çE¢®Ec¢®¡×I¡Ë¢ç¡§IA¢®E¡ËcEOA¡Ë¢ç¢®¡¿I¢®¡×I¢®¡×¢®IA€AŒA¢®E¡Ëc¡Ë¢ç¢®¢¯uA¡Ë¢çE¢®Ec¡Ë¢ç¢®¡¿¢®E¡ËcA†A¢®E¡ËcEcA¢®E¡ËcE¢®E¡Ëc¡§I¢®¨ÏA¢®E¡ËcE¢®E¡Ëc¡§I¢®¨Ï A¡Ë¢çE¢®Ec¡Ë¢çE¢®E¢®IAŽA¢®E¡ËcIiA¡Ë¢çE¢®EcI
	CEventGeneric::GetInstance()->LoadScript( "propEvent.inc" );
	...
	CPetProperty::GetInstance()->LoadScript( "pet.inc" );		//sun: 9, 9-10A‚A¡Ë¢çE¢®Ec¡Ë¢çE¢®E¢®I A†A¢®E¡Ëc¡Ë¢ç¢®¢¯¢®E¡ËcI

	//sun: 11, AƒA¢®E¡ËcE¡Ë¢çE¡Ë¢çIAA¢®E¡Ëc¡Ë¢ç¢®¢¯o A¢®E¡Ëc¡Ë¢ç¢®¢¯oAƒA¢®E¡Ëc¡Ë¢ç¢®¢¯oA¢®E¡Ëc¡Ë¢ç¢®¢¯¢®E¡Ëc¡§I¢®¨ÏA…A›
	...
	LoadUltimateWeapon();		//sun: 9,10A‚A¡Ë¢çE¢®Ec¡Ë¢çE¢®E¢®I AA|A¡Ë¢çE¢®Ec¡Ë¢çE¢®E¢®IAƒ __ULTIMATE
	...
	//sun: 11, A€AA¢®E¡ËcE¢®E¡ËcIA¡Ë¢çE¢®EciA€A A¡Ë¢çE¢®Ec¡Ë¢ç¢®¡¿uA|A¢®E¡ËcIiA¡Ë¢çE¢®EcI A¢®E¡ËcE¢®E¡ËcIA¡Ë¢çE¢®EciA€A¢®E¡Ëc¡Ë¢ç¢®¢¯u
#ifdef __WORLDSERVER
...
#ifdef __WORLDSERVER
	CSecretRoomMng::GetInstance()->LoadScript();		//sun: 12, A¢®E¡Ëc¡Ë¢ç¢®¢¯¢®E¡Ëc¡§I¢®¨ÏA¡Ë¢çE¢®Ec¡Ë¢ç¢®¡¿uA¡Ë¢ç¢®¡¿IoAA€A‡ A¡Ë¢ç¢®¡¿IoA|
...
//	g_pItemUpgradeManager->Load();
	...
	//sun: 12, A¡Ë¢çE¢®Ec¡Ë¢ç¢®¡¿uA¢®E¡Ëc¡Ë¢ç¢®¢¯¢®E¡Ëc¡§I¢®¨ÏAA–
#ifdef __WORLDSERVER
...
#ifdef __WORLDSERVER
	LoadGuideMaxSeq();	//sun: 12, A†A¢®E¡Ëc¡Ë¢ç¢®¢¯IA…A¢®E¡ËcE¡Ë¢çE¡Ë¢çIA¢®E¡ËcE¢®E¡Ëc¡§I¢®¨ÏA¢®E¡ËcEcA¢®E¡Ëc¡Ë¢ç¢®¢¯uA¡Ë¢ç¢®¡¿I¢®¡×I¢®¡×¢®I A¡Ë¢çE¢®Ec¡Ë¢ç¢®¡¿¢®E¡ËcA¡Ë¢ç¢®¡¿I¢®¡×I¢®¡×¢®IA¢®E¡Ëc¡Ë¢ç¢®¢¯uA¡Ë¢çE¢®Ec¡Ë¢ç¢®¡¿u

//	CRainbowRaceMng::GetInstance()->LoadScript();	//sun: 13, A¡Ë¢çE¢®Ec¡Ë¢çE¢®E¢®IA¡Ë¢ç¢®¡¿IoA€AŽA¢®E¡Ëc¡Ë¢ç¢®¢¯¢®E¡Ëc¡§I¢®¨ÏA¢®E¡ËcE¢®E¡Ëc¡§I¢®¨ÏA¢®E¡ËcE?A¡Ë¢çE¢®Ec¢®¡×I¡Ë¢ç¡§I A¡Ë¢çE¢®Ec¡Ë¢çE¢®E¢®IA¡Ë¢ç¢®¡¿IoA€AŒA¢®E¡Ëc¡Ë¢ç¢®¢¯oA¢®E¡Ëc¡Ë¢ç¢®¢¯¢®E¡Ëc¡§I¢®¨Ï
	CRainbowRaceMng::GetInstance()->Load();
#ifdef __ZCK_WORLD_BOSS
	CWorldBossManager::GetInstance()->LoadScript();
#endif // __ZCK_WORLD_BOSS

#endif	// __WORLDSERVER

#if defined( __ZCK_INVASIONS ) && ( defined( __CLIENT ) || defined( __WORLDSERVER ) )
	if( CZckInvasions::GetInstance()->LoadScript() == FALSE )
	{
		FLERROR_LOG( PROGRAM_NAME, _T( "CProject::OpenProject() - Echec du chargement obligatoire de ZckInvasions.inc." ) );
		return FALSE;
	}
#endif // __ZCK_INVASIONS && (__CLIENT || __WORLDSERVER)

#if defined( __ZCK_TELEPORTEUR ) && ( defined( __CLIENT ) || defined( __WORLDSERVER ) )
	if( CZckTeleporteur::GetInstance()->LoadScript( _T( "ZckTeleporteur.inc" ) ) == FALSE )
	{
		FLERROR_LOG( PROGRAM_NAME, _T( "CProject::OpenProject() - Echec du chargement obligatoire de ZckTeleporteur.inc." ) );
		return FALSE;
	}
#endif // __ZCK_TELEPORTEUR && (__CLIENT || __WORLDSERVER)
	//	OutputDropItem();
	//	return FALSE;
	...
	//sun: 13, A„A¢®E¡ËcE?A‡Aƒ A¢®E¡Ëc¡Ë¢ç¢®¢¯oAƒA¢®E¡Ëc¡Ë¢ç¢®¢¯oA¢®E¡Ëc¡Ë¢ç¢®¢¯¢®E¡Ëc¡§I¢®¨ÏA…A›
#ifdef __WORLDSERVER
...
#if defined(__APP_TELEPORTER) && defined(__CLIENT)
	LoadTeleportItem();
#endif
...
AddSkillProp* CProject::GetAddSkillProp( DWORD dwSubDefine, DWORD dwLevel )
{
	...
	{
		FLERROR_LOG( PROGRAM_NAME, _T( "¹üÀ§Ä§¹ü. %d" ), nskillIndex );
		return 0;
	}
...
BOOL CProject::LoadMotionProp( LPCTSTR lpszFileName )
{
	...
	while( script.tok != FINISHED )
	{
		...
		if( strlen(script.m_mszToken) >= sizeof(MotionProp.szIconName) )
			FLERROR_LOG( PROGRAM_NAME, _T( "¸Þ¸ð¸®Ä§¹ü. %s ¹öÆÛÅ©±â(%d), ½ºÆ®¸µ±æÀÌ(%d)" ), script.m_mszToken, sizeof(MotionProp.szIconName), strlen(script.m_mszToken) );
...
BOOL CProject::LoadPropQuest( LPCTSTR lpszFileName, BOOL bOptimize )
{
	...
	while( script.tok != FINISHED )
	{
		...
		propQuest.m_nBeginCondTutorialState		= -1;		//sun: 12, Æ©Åä¸®¾ó °³¼±
		...
		while( nBlock && script.tok != FINISHED )
		{
			...
			else if( script.Token == "SetMultiCharacter" )
			{
				...
				do
				{
					...
					{
						FLERROR_LOG( PROGRAM_NAME, _T( "%s(%d) : %s°¡ Á¸ÀçÇÏÁö ¾ÊÀ½" ), lpszFileName, script.GetLineNum(), szLinkChar[ szLinkCharNum ] );
					}
					szLinkCharNum++;
					script.GetToken(); // , or )
				...
				else if( script.Token == "SetBeginCondNotItem" )
				{
					...
					{
						nJobOrItem = -1;
						FLERROR_LOG( PROGRAM_NAME, _T( "%s(%d) SetBeginCondNotItem ¹üÀ§ ÃÊ°ú(0~%d)" ), lpszFileName, script.GetLineNum(), MAX_QUESTCONDITEM ); 
					}
					script.GetToken(); // ,
				...
				{
					script.GetToken(); // (
					propQuest.m_nBeginCondPreviousQuestType = static_cast< char >( script.GetNumber() ); // 0 ½ÃÀÛÁß, 1 ÀÌÀü Äù½ºÆ® ¿Ï·á
					script.GetToken(); // ,
					int nCnt = 0;
					do
					{
				...
				else if( script.Token == "SetBeginCondItem" )
				{
					...
					{
						nJobOrItem = -1;
						FLERROR_LOG( PROGRAM_NAME, _T( "%s(%d) SetBeginCondItem ¹üÀ§ ÃÊ°ú(0~%d)" ), lpszFileName, script.GetLineNum(), MAX_QUESTCONDITEM ); 
					}
					script.GetToken(); // ,
				...
				else if( script.Token == "SetBeginSetAddItem" )
				{
					...
					{
						nIdx = 0;
						FLERROR_LOG( PROGRAM_NAME, _T( "%s(%d) SetBeginSetAddItem Idx °ª ¹üÀ§ ÃÊ°ú(0~3)" ), lpszFileName, script.GetLineNum() ); 
					}
					script.GetToken(); // ,
				...
				else if( script.Token == "SetEndCondOneItem" )
				{
					...
					{
						nJobOrItem = -1;
						FLERROR_LOG( PROGRAM_NAME, _T( "%s(%d) SetEndCondOneItem ¹üÀ§ ÃÊ°ú(0~%d)" ), lpszFileName, script.GetLineNum(), MAX_QUESTCONDITEM ); 
					}
					script.GetToken(); // ,
				...
				else if( script.Token == "SetEndCondItem" )
				{
					...
					{
						nJobOrItem = -1;
						FLERROR_LOG( PROGRAM_NAME, _T( "%s(%d) SetEndCondItem ¹üÀ§ ÃÊ°ú(0~%d)" ), lpszFileName, script.GetLineNum(), MAX_QUESTCONDITEM ); 
					}
					script.GetToken(); // ,
					...
					if( script.Token ==  "," )
					{
						...
						script.GetToken(); // , or )
						if( script.Token == "," )
							aEndCondItem[ nEndCondItemNum ].m_ItemGoalData.m_nOwnMonster = script.GetNumber();	// ¾ÆÀÌÅÛ ¼ÒÀ¯ ¸ó½ºÅÍ Index
					}
				...
				else if( script.Token == "SetEndCondKillNPC" )
				{
					...
					{
						nIdx = 0;
						FLERROR_LOG( PROGRAM_NAME, _T( "%s(%d) SetEndCondKillNPC¿¡¼­ Idx °ª ¹üÀ§ ÃÊ°ú:[%d], MAX:[%d]" )
				...
				{
					script.GetToken(); // (
					script.GetToken(); // "key"	-> ¾È¾²ÀÓ, À§¿¡ SetCharacter¿¡¼­ ÀÌ¹Ì ÀúÀåÇÔ.
					script.GetToken(); // , or )
					if( script.Token ==  "," )
					{
				...
				else if( script.Token == "SetParam" )
				{
					...
					{
						nIdx = 0;
						FLERROR_LOG( PROGRAM_NAME, _T( "%s(%d) SetParam Idx °ª ¹üÀ§ ÃÊ°ú(0~3)" ), lpszFileName, script.GetLineNum() ); 
					}
					script.GetToken(); // ,
				...
				else if( script.Token == "SetDlgRewardItem" )
				{
					...
					{
						nIdx = 0;
						FLERROR_LOG( PROGRAM_NAME, _T( "%s(%d) SetDlgRewardItem Idx °ª ¹üÀ§ ÃÊ°ú(0~3)" ), lpszFileName, script.GetLineNum() ); 
					}
					script.GetToken(); // ,
				...
				else if( script.Token == "SetEndRewardItem" )
				{
					...
					script.GetToken(); // ,
					int nJobOrItem = script.GetNumber();
					if( nType == 0 && nJobOrItem >= MAX_JOB )	// Job °Ë»çÀÎµ¥ ¿Ö MAX_QUESTCONDITEM À¸·Î °Ë»ç¸¦ ÇßÀ»±î..
					{
				...
				else if( script.Token == "SetEndRemoveItem" )
				{
					...
					{
						nIdx = 0;
						FLERROR_LOG( PROGRAM_NAME, _T( "%s(%d) SetEndRemoveItem Idx °ª ¹üÀ§ ÃÊ°ú(0~3)" ), lpszFileName, script.GetLineNum() ); 
					}
					script.GetToken(); // ,
				...
				if( script.Token == "SetPatrolZoneName" )
				{
					...
					if( string.GetLength() < 64 )
						...
						FLERROR_LOG( PROGRAM_NAME, _T( "%s(%d) SetPatrolZoneName ½ºÆ®¸µ 64¹ÙÀÌÆ® ÃÊ°ú" ), lpszFileName, script.GetLineNum() ); 
				}
					...
					while( script.Token[0] != '}' )
					{
						...
						else if( script.Token == "SetDesc" )
						{
							...
							if( string.GetLength() < 512 ) // null ¶§¹®¿¡ 255
								...
								FLERROR_LOG( PROGRAM_NAME, _T( "%s(%d) LoadPropQuestÀÇ SetDesc ±æÀÌ ÃÊ°ú" ), lpszFileName, script.GetLineNum() ); 
...
BOOL CProject::LoadPropAddSkill( LPCTSTR lpszFileName )
{
	...
	while ( script.tok != FINISHED )
	{
		...
		pAddSkillProp.dwAbilityMinPVP	= script.GetNumber( TRUE );		// ´ëÀÎ È¿°ú ÃÖ¼Ò
		pAddSkillProp.dwAbilityMaxPVP	= script.GetNumber( TRUE );		// ´ëÀÎ È¿°ú ÃÖ´ë
...
//	int		nBlock = 0;
	...
	do 
	{
		...
		if(pProp == NULL)
		{
			FLTRACE_LOG( PROGRAM_NAME, _T( "MoverAddProp¿¡¼­ Á¸ÀçÇØ¾ßµÉ base Property %s°¡ ¾ø´Ù. %s" ), script.Token );
			continue;
		}
		...
		while( *script.token != '}' )
		{
			...
			if( _strcmpi( script.Token, "AI" ) == 0 )		// AIºí·°
			{
				BOOL bRet = LoadPropMoverEx_AI( szFileName, script, nVal );
				if ( bRet == FALSE )
				{
					FLERROR_LOG( PROGRAM_NAME, _T( "%s : MoverID %s(%d)ÀÇ AI{} ºí·° ÀÐ±â ½ÇÆÐ" ), szFileName, pProp->szName, nVal );
					return FALSE;
				}
			}
			...
			if( script.Token == "AddSummonMonster" )
			{
				...
				{
					FLERROR_LOG( PROGRAM_NAME, _T( "[ SUB MONSTER HAS SUMMON MONSTER LIST : AddSubMonster ][ MI(%u) Sub_MI(%u) ]" ), nVal, dwMoverIndex );	//Àç±ÍÀû
					RELEASE_BREAK;
				}
				...
				pProp->m_mapSummonMonsters.insert
					( MoverProp::SummonMonstersMap::value_type( key, dwMoverIndex ) );		//@ ³ªÁß¿¡ °ËÁõ ÇØ¾ßÇÔ.
				script.GetToken();			//")"
				script.GetToken();			//";"
			...
			else if(script.Token == "m_nAttackFirstRange")
			{
				...
				if( pProp->m_nAttackFirstRange > 100 || pProp->m_nAttackFirstRange <= 0 )
					FLERROR_LOG( PROGRAM_NAME, _T( "LoadPropMoverEx::m_nAttackFirstRange ÀÌ»ó:%d" ), pProp->m_nAttackFirstRange );
			}
			...
			else if( script.Token == "DropItem" )
			{
				...
				if( di.dwIndex == 0 )
					FLERROR_LOG( PROGRAM_NAME, _T( "%s : %s°¡ defineItem.h¿¡ Á¤ÀÇµÇÁö ¾Ê¾ÒÀ½" ), szFileName, script.token );
...
#ifdef __WORLDSERVER
			//	if( _GetContentState( CT_DROP_ITEM_REFORM ) != CS_VER1 )	// ½ºÅ©¸³Æ®¿¡¼­ Drop¾ÆÀÌÅÛÀÌ ¼ÂÆÃµÉ ¼ö ÀÖµµ·Ï ¼öÁ¤
				{
...
BOOL CProject::LoadCharacter( LPCTSTR szFileName )
{
	...
	while( script.tok != FINISHED)
	{
		...
		lpCharacter->m_nVenderType = VENDOR_TYPE_PENYA;		//sun: 11, Ä¨À¸·Î »óÁ¡ ÀÌ¿ë ÇÏ±â (±æµå ´ëÀü °ü·Ã)
		...
		{
			FLERROR_LOG( PROGRAM_NAME, _T( "%s¿¡¼­ µ¿ÀÏÇÑ %s ¹ß°ß" ), szFileName, strName );
		}
#endif
		...
		{ 
			...
			else if(script.Token == "m_szChar") // "SetImage"¶û °°´Ù Â÷ÈÄ »èÁ¦ ¿¹Á¤ 
			{ 
				script.GetToken();
			...
			else if(script.Token == "AddMenu" )
			{
				...
				if( ( g_xFlyffConfig->GetMainLanguage() != LANG_TWN && g_xFlyffConfig->GetMainLanguage() != LANG_HK ) || ( nMMI != MMI_BEAUTYSHOP && nMMI != MMI_BEAUTYSHOP_SKIN ) )
					//				if( g_xFlyffConfig->GetMainLanguage() != LANG_TWN || nMMI != MMI_BEAUTYSHOP )		// sun: 8Â÷ Çì¾î¼¥°ü·Ã ¼öÁ¤ ¹× ¼ºÇü¼ö¼ú ±â´É Ãß°¡.( < 8 )
					lpCharacter->m_abMoverMenu[ nMMI ] = TRUE;
			}
			...
			else if(script.Token == "AddVenderSlot" ) // "AddVendorSlot"¶û °°´Ù »èÁ¦ ¿¹Á¤ 
			{
				script.GetToken(); // (
...
{	
	...
	while( script.tok != FINISHED )
	{
		...
		else if( script.Token == _T( "expUpitem" ) )
		{
			...
			if( j != 6 )
#endif
				FLERROR_LOG( PROGRAM_NAME, _T( "expUpitem ºí·°ÀÇ µ¥ÀÌÅ¸ ¿­°³¼ö°¡ ¸ÂÁö ¾Ê´Ù. %d" ), j );
			if( i != _MAX_ENCHANT_LEVEL + 1 )
				FLERROR_LOG( PROGRAM_NAME, _T( "expUpitem ºí·°ÀÇ µ¥ÀÌÅ¸ Çà°³¼ö°¡ ¸ÂÁö ¾Ê´Ù. %d" ), j );
...
void CSetItemFinder::AddSetItem( CSetItem* pSetItem )
{
	...
	if( !bResult )
	{
		FLTRACE_LOG( PROGRAM_NAME, _T( "[¼¼Æ® ¾ÆÀÌÅÛ] ¼¼Æ® ¾ÆÀÌÅÛ µî·Ï ½ÇÆÐ. ¾ÆÀÌµð Áßº¹ÀÔ´Ï´Ù ID - [%u]" ), pSetItem->m_nId );
		SAFE_DELETE( pSetItem );
		return;
	}
	for( int i = 0; i < pSetItem->m_nElemSize; i++ )
	{
		...
		if( !bResult )
			FLTRACE_LOG( PROGRAM_NAME, _T( "[¼¼Æ® ¾ÆÀÌÅÛ] ¼¼Æ® ¾ÆÀÌÅÛ(ÆÄÆ®) µî·Ï ½ÇÆÐ. ÀÌ¹Ì µî·ÏµÇ¾î ÀÖ´Â ¾ÆÀÌÅÛÀÌ ÀÖ½À´Ï´Ù. ID - [%u], ¼¼Æ®ÀÌ¸§ - [%s]" ), pSetItem->m_adwItemId[i], pSetItem->m_pszString );
	}
}
...
JobProp* CProject::GetJobProp( int nIndex )
{
	...
	{
		FLERROR_LOG( PROGRAM_NAME, _T( "¹üÀ§Ä§¹ü. %d" ), nIndex );
		return NULL;
	}
	return &m_aPropJob[nIndex];
}

//sun:9, Á¦·Ã ÀÌÆÑÆ® Çâ»ó ¹öÀü
BOOL CProject::LoadPropEnchant( LPCTSTR lpszFileName )
{
	...
	if( scanner.Load( lpszFileName, FALSE ) == FALSE )
	{
		...
		FLERROR_LOG( PROGRAM_NAME, _T( "ÇÁ·ÎÆÛÆ¼ ÀÐ±â½ÇÆÐ : Enchant" ) );
#endif //__CLIENT
		return FALSE;
	}
...
BOOL CProject::LoadPropJob( LPCTSTR lpszFileName )
{
	...
	if( scanner.Load( lpszFileName, FALSE ) == FALSE )
	{
		...
		FLERROR_LOG( PROGRAM_NAME, _T( "ÇÁ·ÎÆÛÆ¼ ÀÐ±â½ÇÆÐ : Job" ) );
#endif //__CLIENT
		return FALSE;
	}
	...
	while( 1 )
	{
		...
		//gmpbigsun( 20111223 ) : Å¬·¡½º ¸¶Å© ÆÄÀÏ¸í Ãß°¡µÊ.
		scanner.GetToken();
...
BOOL CProject::LoadPropCtrl( LPCTSTR lpszFileName, CIndexArray<CtrlProp>*	apObjProp)
{
	...
	if( scanner.Load( lpszFileName, FALSE ) == FALSE )
	{
		...
		FLERROR_LOG( PROGRAM_NAME, _T( "ÇÁ·ÎÆÛÆ¼ ÀÐ±â½ÇÆÐ : Ctrl" ) );
#endif //__CLIENT
		return FALSE;
	}
	...
	while( scanner.tok != FINISHED )
	{
		ctrlProp.dwID = i;    
		if( i <= 0 )
			FLERROR_LOG( PROGRAM_NAME, _T( "%s ÀÐ´Ù°¡ ID°¡ %dÀÌ³ª¿Ô´Ù. ¸¶Áö¸·À¸·Î ÀÐÀº°Í %s" ), lpszFileName, i, szTemp );
		scanner.GetToken();
...
{ 
	...
	case IK3_YOYO: 
	case IK3_BOW:
	case IK3_COLLECTER:	//sun: 11, Ã¤Áý ½Ã½ºÅÛ
	...
	FLERROR_LOG( PROGRAM_NAME, _T( "itemkind¿À·ù %d, %d" ), pItemProp->dwItemKind3, nOption );
	return 0;
}
...
void CProject::LoadTeleportItem()
{
	...
	{
		FLERROR_LOG(PROGRAM_NAME, _T("Could not load TeleportItem.inc"));
		return;
	}
	DWORD dwWorld = 0;
	dwWorld = s.GetNumber();
	while (s.tok != FINISHED)
	{
		...
		dwColor = 0xFF000000;
		dwItem = s.GetNumber();
		while (s.Token != "}")//read  id items
		{

			if (!g_xSpecManager->GetSpecItem(dwItem))
			{
				FLERROR_LOG(PROGRAM_NAME, _T("::LoadTeleportItem Error invalid dwItem in World %d", dwWorld));
				continue;
			}
...
std::vector<DWORD> * CProject::GetTeleportItem(DWORD dwWorldId)
{
