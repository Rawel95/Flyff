...
//	for(i=0;i<m_nStar;i++) m_vStar[i]=D3DXVECTOR3(0,0,0);
	...
	for( int i = 0; i < SNOW_TEX_MAX; ++i )
		m_pSnowTexture[ i ] = NULL;
}
CSkyBox::~CSkyBox()
{
	Free();
...
{
	//운영자일경우 날씨가 잠겨있더라도 변경가능
	if( g_pPlayer && FALSE == g_pPlayer->IsAuthHigher( AUTH_ADMINISTRATOR ) )
	{
...
void CSkyBox::InitFall()
{
	...
	if( m_nWeather == WEATHER_SAKURA ) {
		FLStrcpy( fileName, _countof( fileName ), _T( "sakura.bmp" ) );
	}
	else if( m_nWeather == WEATHER_LEAF ) {
		FLStrcpy( fileName, _countof( fileName ), _T( "leaf.bmp" ) );
	}
	else {
		FLStrcpy( fileName, _countof( fileName ), _T( "sundisk.bmp" ) );
	}

	if( FAILED( LoadTextureFromRes( g_Neuz.m_pd3dDevice, MakePath( DIR_WEATHER, fileName ), &m_pSunTexture ) ) ) {
		FLERROR_LOG( PROGRAM_NAME, _T( "LoadTextureFromRes Default" ) );
	}
...
void CSkyBox::Process()
{
	...
	if( m_pWorld ) 
	{
					...
					if( fheight > m_vFall[ i ].y ) 
					{
						if( xRandom(10) >= 7 )
						{
								...
								( pWaterHeight->byWaterTexture & (byte)(~MASK_WATERFRAME) )	== WTYPE_WATER )
							{
								...
								v3Pos.y = fHeight;
								CSfx *pObj = CreateSfx( g_Neuz.m_pd3dDevice, XI_INDEX( 72, XI_GEN_RAINCIRCLE01 ), v3Pos );
		...
		if( m_nWeather == WEATHER_SNOW || m_nWeather == WEATHER_SAKURA || m_nWeather == WEATHER_LEAF )
		{
			for( int i = 0; i < (int)( m_nFall ); i++) 
			{
				FLOAT fheight = m_pWorld->GetLandHeight_Fast( m_vFall[ i ].x, m_vFall[ i ].z ) - 1.0f;
...
//	pd3dDevice->SetRenderState( D3DRS_FOGEND,     FtoDW(100.0f) );
	...
	if( !m_pWorld->HookRenderSky( SKY_CLOUD, 28, fAlphaEX ) )
	{
		...
		if( g_GameTimer.m_nHour == 6 ) 
		{
			...
			pd3dDevice->DrawPrimitive( D3DPT_TRIANGLEFAN, 0, 28);
		//	FLTRACE_LOG( PROGRAM_NAME, _T( "%d" ), g_GameTimer.m_nMin*255/59 );
		}
		else 
		if( g_GameTimer.m_nHour < 17 ) 
		{
...
void CSkyBox::RenderFall( LPDIRECT3DDEVICE9 pd3dDevice, int nType )
{
	if(!g_Option.m_nWeatherEffect) return;
...
void CSkyBox::DrawSnow(LPDIRECT3DDEVICE9 pd3dDevice, int nType )
{
	pd3dDevice->SetRenderState( D3DRS_ZWRITEENABLE, FALSE );
	pd3dDevice->SetRenderState( D3DRS_LIGHTING, FALSE );
	pd3dDevice->SetRenderState( D3DRS_ALPHABLENDENABLE,   TRUE );
...
//	pd3dDevice->SetRenderState( D3DRS_DESTBLEND, D3DBLEND_INVSRCALPHA );
	...
	if( 0 == nType )
	{
		...
		{
			D3DXMATRIX matScal;
			D3DXMatrixScaling( &matScal, 1.5f, 1.5f, 1.5f );
			matWorld *= matScal;
		}
	...
	else if( 1 == nType )	//Event arena type
	{
		m_nFall = 800;
		static int sRnd[ 800 ];
		...
		for(int i=0;i<(int)( m_nFall );++i) 
		{
			...
			if( nRnd == 0 )
			{
				pd3dDevice->SetTexture( 0, m_pSnowTexture[0] );
			}
			else 
			{
				if( nRnd < 6 )
					pd3dDevice->SetTexture( 0, m_pSnowTexture[nIndex] );
				else pd3dDevice->SetTexture( 0, m_pSnowTexture[nIndex+1] );
			}
			pd3dDevice->DrawPrimitive( D3DPT_TRIANGLEFAN, 0, 2);
		}
	}
...
*/
	...
	if( FAILED( LoadTextureFromRes( pd3dDevice, MakePath( DIR_WEATHER, "Flare_3.tga"), &m_pSnowTexture[0] ) ) )
	{
		return D3DAPPERR_MEDIANOTFOUND;
	}

	if( FAILED( LoadTextureFromRes( pd3dDevice, MakePath( DIR_WEATHER, "ColorPaper11.tga"), &m_pSnowTexture[1] ) ) )
	{
		return D3DAPPERR_MEDIANOTFOUND;
	}

	if( FAILED( LoadTextureFromRes( pd3dDevice, MakePath( DIR_WEATHER, "ColorPaper12.tga"), &m_pSnowTexture[2] ) ) )
	{
		return D3DAPPERR_MEDIANOTFOUND;
	}

	if( FAILED( LoadTextureFromRes( pd3dDevice, MakePath( DIR_WEATHER, "ColorPaper21.tga"), &m_pSnowTexture[3] ) ) )
	{
		return D3DAPPERR_MEDIANOTFOUND;
	}

	if( FAILED( LoadTextureFromRes( pd3dDevice, MakePath( DIR_WEATHER, "ColorPaper22.tga"), &m_pSnowTexture[4] ) ) )
	{
		return D3DAPPERR_MEDIANOTFOUND;
	}
...
HRESULT CSkyBox::DeleteDeviceObjects()
{
	...
	for(int i=0;i<SNOW_TEX_MAX;++i)SAFE_RELEASE(m_pSnowTexture[i]);
