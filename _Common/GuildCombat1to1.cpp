...
{
	std::vector < __GC1TO1_PLAYER_INFO >::iterator it = m_vecGuildCombat1to1_Players.begin();
...
BOOL CGuildCombat1to1Mng::LoadScript()
{
	...
	{
		FLERROR_LOG( PROGRAM_NAME, _T( "[ FAILED LOAD SCRIPT. FILE_NAME:(%s), Defaults loaded.]" ), szFullPath );
		FLASSERT(0);

		// force defaults
		m_nJoinPenya = 110000000;
		m_nGuildLevel = 20;
		m_nMinJoinGuild = 2;
		m_nMaxJoinGuild = 8;
		m_nMinJoinPlayer = 4;
		m_nMaxJoinPlayer = 8;
		m_nMinJoinPlayerLevel = 1;
		m_nCancelReturnRate = 80;
		m_nFailReturnRate = 96;
		m_nPlayerLife = 1;
		const T_ITEM_SPEC* pItemSpec = g_xSpecManager->GetSpecItem(II_CHP_RED);
		if (pItemSpec == nullptr)
			return false;
		SetRewardItemID(II_CHP_RED);
	}
	else
	{
		...
		if (pItemSpec == nullptr)
		{
			FLERROR_LOG(PROGRAM_NAME, _T("[ INVALID REWARD ITEM ID. ITEM_ID(%u) ]"), dwRewardItemID); //RELEASE_BREAK;
			return false;
		}
...
void	CGuildCombat1to1Mng::GuildCombat1to1Open( BOOL bGMText )
{
	...
	std::vector<__GC1TO1TENDER>::iterator it;
	if( !bGMText )
	{
	...
	for( it=m_vecTenderGuild.begin(); it!=m_vecTenderGuild.end(); it++ )
	{
		...
		if( pGuild )
		{
			for( std::map<u_long, CGuildMember*>::iterator it2=pGuild->m_mapPMember.begin(); it2!=pGuild->m_mapPMember.end(); it2++ )
			{
...
void	CGuildCombat1to1Mng::SetCancelTenderGuild( FLWSUser* pUser )
{
	...
	if( pGuild )
	{
		for( std::vector<__GC1TO1TENDER>::iterator it=m_vecTenderGuild.begin(); it!=m_vecTenderGuild.end(); it++ )
		{
...
void	CGuildCombat1to1Mng::SetFailedTenderGuild( FLWSUser* pUser )
{
	...
	if( pGuild )
	{
		for( std::vector<__GC1TO1TENDER>::iterator it=m_vecTenderFailGuild.begin(); it!=m_vecTenderFailGuild.end(); it++ )
		{
			if( (*it).ulGuildId == pGuild->GetGuildId() )
			{
...
// 참가자 구성
void	CGuildCombat1to1Mng::SetMemberLineUp( FLWSUser* pUser, std::vector<u_long>& vecMemberId )
{
