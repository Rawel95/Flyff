#include "stdafx.h"
#if defined( __WORLDSERVER ) && defined( __ZCK_WORLD_BOSS )
#include "WorldBoss.h"
#endif // __WORLDSERVER && __ZCK_WORLD_BOSS
...
#include "SecretRoom.h"		//sun: 12, ºñ¹ÐÀÇ ¹æ
...
#include "collecting.h"	//sun: 11, Ã¤Áý ½Ã½ºÅÛ

// ¼¼Æ® ¾ÆÀÌÅÛ ÀÌÆåÆ® È¿°ú ·¹º§ ´Ü°è
#pragma	warning( push )
...
namespace
{
	CSfx* CreateAilesSfxZacks( LPDIRECT3DDEVICE9 pd3dDevice, DWORD dwSfxObj )
	{
		if( !pd3dDevice || dwSfxObj == NULL_ID || dwSfxObj == 0 )
			return NULL;
		...
		D3DXVECTOR3 vPosSrc( 0.0f, 0.0f, 0.0f );
		D3DXVECTOR3 vPosDest( 0.0f, 0.0f, 0.0f );

		const int nResult = pSfx->SetSfx(
			pd3dDevice,
			dwSfxObj,
			vPosSrc,
			NULL_ID,
		...
		if( !nResult
			|| !pSfx->GetModel()
			|| !pSfx->GetModel()->m_pModelElem
			|| !pSfx->m_pSfxObj
			|| !pSfx->m_pSfxObj->m_pSfxBase )
		{
			SAFE_DELETE( pSfx );
			return NULL;
		}
...
{
#if defined( __WORLDSERVER ) && defined( __ZCK_WORLD_BOSS )
	CWorldBossManager::GetInstance()->FinishWorldBoss( this );
#endif // __WORLDSERVER && __ZCK_WORLD_BOSS
#if defined __WORLDSERVER && defined __ENCHANT_BARUNA16		
	if( IsPlayer() == TRUE )																							//¡§u¨Ï¨£¡§u¢®¨Ï AOAC m_bPlayer¡Ë?I m_CalledNPCOBJID¡Ë?I ¢®¨¡u¢®¢´A
...
//	m_dwUpdateDestTick	= 0;
	...
	m_nTutorialState	= 0;	//sun: 12, Æ©Åä¸®¾ó °³¼±
	...
	m_nZckManagedSfxLevel = -1;
	m_nZckManagedSfxEffectValue = -1;
...
#ifdef __WORLDSERVER
	m_decreaseExperience	= true;		// ±æµå´ëÀü, µà¾ó, ¾Æ·¹³ª µî Æ¯Á¤ »óÈ² ÀÌ¿Ü¿¡´Â »ç¸Á ½Ã °æÇèÄ¡ ÇÏ¶ô Ã¼Å©.
	...
	//sun: 8,     // 8Â÷ µà¾óÁ¸¿¡ °ü°è¾øÀÌ PVP°¡´ÉÇÏ°ÔÇÔ   Neuz, World
	m_bPVPEnd	=	FALSE;

	m_nLegend	=	LEGEND_CLASS_NORMAL;	//sun: 10Â÷ Àü½Â½Ã½ºÅÛ	Neuz, World, Trans

	memset( dwRemoveSfxObj, 0, sizeof( dwRemoveSfxObj ) );		//sun, 11, È®À²½ºÅ³ È¿°ú¼öÁ¤ world,neuz
...
#ifdef __CLIENT
	m_SkillTimer.Set(1500); //Function Key´©¸£°í ÀÖÀ» °æ¿ì Skill Delay°ª.
#endif //__CLIENT
...
#if defined(__ZCK_AILES_SFX)
	m_pAilesSfxZacks = NULL;
	m_dwAilesSfxZacksSfxId = NULL_ID;
#endif // __ZCK_AILES_SFX
#endif //__CLIENT
	...
	memset( m_aHonorTitle, 0, sizeof( m_aHonorTitle ) );// ´ÞÀÎ¼öÄ¡
	m_strTitle.Empty();
	...
	m_bDelete_IntoIdleMode	= FALSE;		// ÀüÅõ°¡ ³¡³ª¸é »èÁ¦µÇ´Â ¿¹¾à
#endif	// __WORLDSERVER
...
#if defined (__INFO_DUNGEONS) && defined (__CLIENT) //
	m_bShowDungeonInfo = FALSE;
#endif
...
#ifdef __ZCK_WORLD_BOSS
void CMover::AddDamageByAttacker( CMover* pMoverAttacker, __int64 n64Damage )
{
	if( m_bWorldBoss == false || IsPlayer() || IsValidObj( pMoverAttacker ) == FALSE || pMoverAttacker->IsPlayer() == FALSE || n64Damage <= 0 )
		return;

	const u_long idPlayer = pMoverAttacker->m_idPlayer;
	if( idPlayer == 0 || idPlayer == NULL_ID )
		return;

	std::map<u_long, __int64>::iterator it = m_mapAttackersDamage.find( idPlayer );
	if( it == m_mapAttackersDamage.end() )
	{
		m_mapAttackersDamage.insert( std::make_pair( idPlayer, n64Damage ) );
		return;
	}

	const __int64 n64MaxValue = 0x7fffffffffffffffLL;
	if( it->second > n64MaxValue - n64Damage )
		it->second = n64MaxValue;
	else
		it->second += n64Damage;
}
#endif // __ZCK_WORLD_BOSS
...
int	CMover::GetHairCost( CMover* pMover, BYTE nR, BYTE nG, BYTE nB, BYTE nHair )
{
	...
	int nHairCost = 0;
	// sun: 8Â÷ Çì¾î¼¥°ü·Ã ¼öÁ¤ ¹× ¼ºÇü¼ö¼ú ±â´É Ãß°¡.
	...
	// sun: 8Â÷ Çì¾î¼¥°ü·Ã ¼öÁ¤ ¹× ¼ºÇü¼ö¼ú ±â´É Ãß°¡. ( < 8 )
...
void CMover::ProcessQuest()
{
	...
	for( size_t i = 0; i < m_nQuestSize; i++ )
	{
		...
		if( lpQuest )
		{
			...
			{
				//FLERROR_LOG( PROGRAM_NAME, _T( "ProcessQuest : pQuestProp NULLÀÌ´Ù." ) );
				//FLTRACE_LOG( PROGRAM_NAME, _T( "%sÀÇ Quest %dÀÇ pQuestProp NULLÀÌ´Ù." ), m_szName, lpQuest->m_wId );
			}
		}
		else
		{
			//FLERROR_LOG( PROGRAM_NAME, _T( "ProcessQuest : lpQuest°¡ NULLÀÌ´Ù." ) );
			//FLTRACE_LOG( PROGRAM_NAME, _T( "%sÀÇ lpQuest°¡ NULLÀÌ´Ù." ), m_szName );
		}
	}
#else // __WORLDSERVER
	if( IsPlayer()  ) 
...
void CMover::ProcessAbnormalState()
{
	...
	if( nHeal > 0 )
	{
		...
		if( m_nHealCnt == 0 )
		{
			if( IsLive() )
			{
				m_nHealCnt	= (short)( PROCESS_COUNT * 2.0f );		// ÄðÅ¸ÀÓ 2ÃÊ
				SetDestParam( DST_HP, nHeal, NULL_CHGPARAM, TRUE );
				//SetPointParam( DST_HP, nHeal );
				// mirchang_100830 È¸»ýÀÇ ±âµµ..
			}
		}
	}
}
	...
	if( IsNPC() )
	{
		if( m_bRegenItem )
		{
			...
			for( int i = 0; i < MAX_VENDOR_INVENTORY_TAB; i ++ )
			{
				...
				if(pCharacter->m_nVenderType == VENDOR_TYPE_CHIP) // Ä¨À¸·Î °Å·¡ÇÏ´Â venderÀÏ °æ¿ì
				{
				...
				if (pCharacter->m_venderItemAry3[i].GetSize()) {
					...
					for (int j = 0; j < pCharacter->m_venderItemAry3[i].GetSize(); j++) {
						...
						PT_ITEM_SPEC pItemProp = g_xSpecManager->GetSpecItem(pVendor->m_dwItemId);
						if (pItemProp) {
							FLItemElem itemElem;
							itemElem.m_dwItemId = pVendor->m_dwItemId;
							itemElem.m_nItemNum = static_cast<short>(pItemProp->dwPackMax);
							itemElem.m_nHitPoint = static_cast<short>(pItemProp->dwEndurance);

							if (!m_ShopInventory[i]->Add(&itemElem))
								break;
						}
					}
				}
...
// 
// 	// Ä³¸¯ÅÍ°¡ °¡Áö°í ÀÖ´Â ÃÑ ½ºÅ³Æ÷ÀÎÆ®¿Í ¿ø·¡ °¡Áö°í ÀÖ¾î¾ß ÇÒ ÃÑ ½ºÅ³Æ÷ÀÎÆ®°¡ ´Ù¸£¸é ÀçºÐ¹è..
...
int   CMover::SetLevel( int nSetLevel )
{
	...
	( (FLWSUser *)this )->AddSetExperience( GetExp1(), (WORD)m_nLevel,  m_nSkillPoint, m_nSkillLevel, m_nDeathExp, (WORD)m_nDeathLevel );		// ÇØ´çÀ¯Àú¿¡°Ô exp1,exp2º¯°æµÈ Á¤º¸¸¦ º¸³¿.
	g_xWSUserManager->AddSetLevel( this, (WORD)m_nLevel );
...
int   CMover::AddGPPoint( int nAddGPPoint )
{
	...
	( (FLWSUser*)this )->AddSetGrowthLearningPoint( m_Stat.GetRemainStatPoint() );		// pUser¿¡°Ô GPº¯µ¿µÈ°ÍÀ» º¸³¿.
	g_dpDBClient.SendLogLevelUp( this, 1 );	// ·¹º§¾÷ ·Î±×
#endif
	return 0; 
}
...
BOOL CMover::InitSkillExp()
{
	...
	//sun: 10Â÷ Àü½Â½Ã½ºÅÛ	Neuz, World, Trans
// 	int nMaxPoint = SKILLPOINT_MAX;	// ÃÖ´ëÀÎ JOB_ELEMENTOR ²¨·Î ¼³Á¤ÇÔ
...
#if defined(__ZCK_AILES_SFX)
void CMover::ClearAilesSfxZacks()
{
	SAFE_DELETE( m_pAilesSfxZacks );
	m_pAilesSfxZacks = NULL;
	m_dwAilesSfxZacksSfxId = NULL_ID;
}

void CMover::UpdateAilesSfxZacks( DWORD dwSfxId )
{
	if( dwSfxId == NULL_ID || dwSfxId == 0 )
	{
		ClearAilesSfxZacks();
		return;
	}
	...
	if( m_pAilesSfxZacks
		&& m_dwAilesSfxZacksSfxId == dwSfxId
	...
	CSfx* pNewSfx = CreateAilesSfxZacks( D3DDEVICE, dwSfxId );
	if( !pNewSfx )
		return;

	m_pAilesSfxZacks = pNewSfx;
	m_dwAilesSfxZacksSfxId = dwSfxId;
}

void CMover::ProcessAilesSfxZacks()
{
	if( !m_pAilesSfxZacks )
		return;

	if( !m_pAilesSfxZacks->GetModel()
		|| !m_pAilesSfxZacks->GetModel()->m_pModelElem
		|| !m_pAilesSfxZacks->m_pSfxObj
		|| !m_pAilesSfxZacks->m_pSfxObj->m_pSfxBase )
	{
...
#ifdef __RENEW_EAT_PET_TELEPORT
	CMover* pEatPet = 0;
	if (HasActivatedEatPet())
		pEatPet = prj.GetMover(GetEatPetId());
#else
...
#endif // __RENEW_EAT_PET_TELEPORT	

	//////////////////////////////////////////////////////////////////////////
	// BEGIN110503 ´Ù¸¥¿ùµå·Î ÀÌµ¿ÇÒ¶§¸¸ ÇÈ¾÷Æê ÇØÁ¦
// 	if( HasActivatedEatPet() )
//		InactivateEatPet();
	// END110503
	//////////////////////////////////////////////////////////////////////////
	...
	//if( IsPlayer() )
	//{
	//	// ´øÀü¿¡¼­ ´øÀüÀ¸·Î ÀÌµ¿ÇÑ´Ù¸é??
	//	// µÎ¹øÂ° °Ë»ç¸¦ ´Ü¼øÈ÷ pWorld->GetID() != dwWorldID Ã¼Å©ÇÏ´Â°Ô ³ªÀ» µí.
...
// ¸®ÅÏ°ªÀÌ 0º¸´Ù Å¬°æ¿ì ´Ù Áö¿ìÁö ¸øÇÑ °æ¿ì..
...
int	CMover::DoDropItemRandom( BOOL bExcludeEquip, CMover* pAttacker, BOOL bOnlyEquip )
{
	...
	for( DWORD dwObjIndex = 0; dwObjIndex < dwMax; ++dwObjIndex ) 		// ÀÎº¥ °¹¼ö¸¸Å­ µ¹¸é¼­ ¾ÆÀÌÅÛÀÌ ÀÖ´ÂÄ­¸¸ Æ÷ÀÎÅÍ¸¦ »Ì¾Æ³õ´Â´Ù.
	{
		...
		if( pItemElem->IsEatPet()
			|| IsUsing( pItemElem )	)	//sun: 9, 9-10Â÷ Æê
			continue;
...
void CMover::ProcessRegion()
{
	...
	if( IsPosChanged() )
	{
		pPortkey = UpdateRegionAttr();			//°¡Àå °¡±î¿î ÅÚ·¹Æ÷Æ® À§Ä¡¸¦ Ã£´Â´Ù.
		SetPosChanged( FALSE );
	}
...
void CMover::ProcessScaleSlerp()
{
	if( m_fDestScaleSlerp > 0 )		// ÁÙ¾î µé¾î¾ß ÇÏ´Â »óÅÂ´Ù.
	{
		...
		if( pMoverProp )
		{
			...
			{
				FLERROR_LOG( PROGRAM_NAME, _T( "ÀÚ¿ø¸÷ÀÌ ¾Æ´Ñµ¥ µé¾î¿Ô´Ù.%s %f, %f, %f %f %f" ), GetName(), m_fDestScaleSlerp, m_fDestScale, vScale.x, vScale.y, vScale.z );
				m_fDestScaleSlerp = 0;
			}
			else
			{
...
void CMover::ProcessETC()
{
	...
	if( (m_nCount & 131071) == 0 ) // 30A¨¬AA¢¬A¢ÒA¢¥A™
	{
		if( xRandom(100) > 50 )
		{
			PT_ITEM_SPEC pItemProp = NULL;
	...
	// A¡¾A|A¥ìA¡ÍA„A„A©öA¢ç A¢¬AŠA€AŒA¢ÒA©øA¢¬A¨Ï...A¨¬AŽA‡AA¢¬A| AA™A€AŒA¡¾A¡Ë A€A¡×A‡A˜ A¡íA©øA…A‚A¢¯A¢® A¥ìA¡íA¢ÒA©ø A¡¤A¡ÌA¢¥A¥ìA¢¬A¥ìA€A¡í A¡¾AAA¢ÒA‡A‘A¢¥A™.
	if( GetWorld() && GetWorld()->GetID() == WI_WORLD_GUILDWAR )
	{
		...
		if( IsPlayer() && g_pPlayer && pWndWorld )
		{
			...
			// A¨úA—A†A¨ùA¨¬A¨£(AA–A€AŽA¡ÆA¢¬)A€AŒ A€A¨ùA€A?AAŸA€AŒA¢¬A¨Ï
			if( nActivePlayer == 1 )
			{
				// A€A¨ùA€A?AAŸA€AŒ A¨úA†A¢¥A‘A¨úA–A¥ìA¨ÏA€A¨¬ A¡¤A¡ÌA¢¥A¥ìA¡¾AAA¢Ò
				if( nPlayer != 1 )
				{
					bFlag = TRUE;
				}
				...
				// A¨úA—A†A¨ùA¨¬A¨£(AA–A€AŽA¡ÆA¢¬)A€AŒ A¢¥A¡ìA¡¾A¡ËAAŸA€AŒA¢¬A¨Ï
				if( nActivePlayer == 0 )
				{
					// A€AA©öAA€AŽA¥ìA¨ÏA€A¨¬ A¡¤A¡ÌA¢¥A¥ìA¡¾AAA¢Ò
					if( nPlayer == -1 )
					{
						bFlag = TRUE;
					}
...
void CMover::Process()
{
...
#ifdef __WORLDSERVER
	//if( /*m_2pc.size() == 0 &&*/ m_dwAIInterface != AII_PET )		//AO¨¬?¢¯¢® pc¡Æ¢® ¨ú©ª¡Æi .. ¨¡O¥ì¥ì ¨ú¨¡¢¥I¢¬e    //pc¡Æ¢® ¨ú¨¡¢¥I¡Æi(AO¨¬?¢¯¢® pc¥ì¥ì ¨ú©ª¡Æi ) ¨¡O¥ì¥ì ¨ú¨¡¢¥I¢¬e ¢¬¢çAI
	//	return;
	...
	//if( m_dwUpdateDestTick + UPDATE_TICK < ::GetTickCount() ) //¡Æ©ª¡ÆY A©¬AI¨úO¢¥A ¨úE¨¬¢¬©ø¡í¥ì¥ì ¥ìE
	//{
...
#ifdef __CLIENT
	if( IsNPC() && IsFlyingNPC() )				// ¨¬nCa¢¬¡ÀA¨¬ ¨¡A¨¡¨ùA¡þ ¡íy¨ù¨¬
	...
	//sun: 8A¡À ¢¯¡ÌA¨Ï ¨ùOE? Neuz, World, Trans
	...
	{
		ProcessAI();							// 1E¢¬ AI¥ì¢¯CI¢¬e ProcessAI¡Æ¢® ©öY¥ìa¨öA E¡ÌAa¥ìC¨úi¨ú©¬ CN¢¥U.
	}

	vPosTemp = vPos	= GetPos();

	{
		m_pActMover->ProcessPreAction();		// CoAc Ca¥ì¢¯ ¡íoAA¢¯¢® ¥ìu¢Òo AuAyCN	 ¢¬©£¨ùCA¡í A©ø¢¬¢çCO
		ProcessAniSpeed();						// ¡Æ©ª¨ùO ¨úO¢¥I¢¬¨­AI¨ùC ¨ùO¥ì¥ì A¢ÒAy.
	}
...
//				if( !( IsPlayer() && IsFly() ) && ( m_pActMover->IsState( OBJSTA_FMOVE ) || m_pActMover->IsState( OBJSTA_TURN_ALL ) ) )
					...
					)
				{
					...
					if( fabs( pProp->fFrame - 0.0F ) < 0.000001F )		// 0AI¢¬e ©ö¡ì¨öA
						fFactor		= 1.0F;
					else if( pProp->dwFlying == 1 )
						fFactor		= 1.0F;
					else
						fFactor		*= pProp->fFrame;
					if( fabs( fFactor - 0.0F ) < 0.000001F )		// fFactor¡Æ¢® 0AI¢¬e ¡¾a¨¬¡í ¡Æ¨£
						fFactor		= 1.0F;
				}
				pModel->FrameMove( &vPos, m_fAniSpeed * fFactor );		// ¨úO¢¥I¢¬¨­AI¨ùC CA¡¤©öAO Ao¡Æ¢®

				//!				pModel->FrameMove( &vPos, m_fAniSpeed );		// ¨úO¢¥I¢¬¨­AI¨ùC CA¡¤©öAO Ao¡Æ¢®
		...
		if( m_nAtkCnt > 0 )		// ¡Æ©ª¡ÆYCaA¡×¢¬| CI¡Æi©ø¡© AIEA¢¥A A¡ì¢¯i¨¡¢ç¡Æ¢® ¡Æe¨ùO Ao¡Æ¢® CN¢¥U.
			++m_nAtkCnt;
	}
	/*
	#ifdef __CLIENT
	...
	#endif*/
	{
		bCollision = m_pActMover->ProcessCollision( &vPos );	// AI¥ì¢¯¨ù¨¬¨¬¨¢ ¢¥oCI¡¾a & A©¡¥ì©öA©ø¢¬¢ç.
	}

#ifdef __CLIENT
	ProcessWaterCircle( vPosTemp, vPos );		// ¨ùo¢¬e E¢¯¡Æu - Ao¢¬iAI ¨ú¨¡¢¥O¢Ò¡×¢¬¢¬ Aa¡¤A
#endif
...
// 			}
	...
	{
#ifdef __WORLDSERVER
		ProcessRecovery();	// E¡À¨¡¢ç¨¡¡ÀAI¨¡¢ç, ¢¬¢Ò©ø¨£¨¡¡ÀAI¨¡¢ç, CC¡¤I¨¡¡ÀAI¨¡¢ç E¢¬¨¬©ö CI¡¾a  AI¢¥UA¨¬ CA¡¤©öAI¨úi¢¬¢¬ AU¥ì¢¯A¢¬¡¤I Au¢¥U.
		...
		if( IsNPC() )
		{
			...
			if( (m_nCount & 127) == 0 )		// ¨úa 8AE¢¬¢¬¢¯¢® CN©ö©ª¨ú¢¯
			{
				ProcessTarget();
				ProcInstantBanker();
				ProcInstantGC();
	...
	++m_nCount;		// ©ö¡ì©öo¡Æ¢® ©öu¢¯eA¢¬¡¤I ¨ú©÷¢¥A ¨ù©ªA¡ÀAu A¡ì¢¯iAI. ¡íy¨ù¨¬AU ¢¯U¢¯¡Ì 0A¢¬¡¤I AE¡¾aE¡© CIAo ¢¬¡í¡ÆI.
...
//---------------------------------------------------------------------------------------------
// ¼¼Æ® ¾ÆÀÌÅÛ ÀÌÆåÆ®ÀÇ ÀÎµ¦½º ¾ò±â 
// param	: ·»´õ·¯
// return	: void
// ¼³¸í		: ( ½Ã°£ÀÌ ´Ù±ÞÇÏ¿© PD´Ô°ú ÀÇ³í°á°ú ¶â¾î°íÄ¥ ½Ã°£ÀÌ ¾ø¾ú´Ù... ÁË¼ÛÇÕ´Ï´Ù... )
//---------------------------------------------------------------------------------------------
void CMover::GetItemSFXIndex( IN const int nAbilityOption, OUT DWORD& dwSFXIndex, OUT DWORD& dwSFXHandsIndex )
{
	...
	switch( GetSetItemSFX_EffectValue() )
	{
		...
		default:
			{
				FLTRACE_LOG( PROGRAM_NAME, _T("[¼¼Æ® ¾ÆÀÌÅÛ ÀÌÆåÆ®] ¾ø´Â ÀÌÆåÆ® ´Ü°è") );
				FLASSERT( NULL );
			}
			break;
	}
...
#if defined(__ZCK_SFX_AMELIORER)
void CMover::AddZckManagedSetSfx( DWORD dwSfxId, int nPartsLink, FLOAT fOffsetY, const D3DXVECTOR3& vScale )
{
	if( dwSfxId == 0 || dwSfxId == NULL_ID )
		return;

	const D3DXVECTOR3 vPos( 0.0f, 0.0f, 0.0f );
	CSfx* pCreatedSfx = CreateSfx( D3DDEVICE, dwSfxId, vPos, NULL_ID, vPos, NULL_ID, -1, FALSE );
	if( pCreatedSfx == NULL )
		return;

	CSfxPartsLinkShoulder* pSfx = static_cast<CSfxPartsLinkShoulder*>( pCreatedSfx );
	pSfx->m_nPartsLink = nPartsLink;
	pSfx->m_v3Offset.y = fOffsetY;
	pSfx->SetScale( vScale );
	pSfx->m_nOldAbilityOption = m_nZckManagedSfxLevel;
	pSfx->m_nOldEffectValue = m_nZckManagedSfxEffectValue;
	pSfx->SetZckSfxMover( this );
	m_vecZckManagedSfx.push_back( pSfx );
}

void CMover::CreateZckManagedSfx()
{
	DeleteZckManagedSfx();

	m_nZckManagedSfxLevel = GetSetItemClient();
	m_nZckManagedSfxEffectValue = GetSetItemSFX_EffectValue();

	if( m_nZckManagedSfxLevel < eSetItemSFX_Level::eLevel_3 )
		return;

	DWORD dwSfx = 0;
	DWORD dwSfxHands = 0;
	GetItemSFXIndex( m_nZckManagedSfxLevel, dwSfx, dwSfxHands );

	const D3DXVECTOR3 vDefaultScale( 1.0f, 1.0f, 1.0f );
	const D3DXVECTOR3 vBodyScale( 3.0f, 3.0f, 3.0f );
	const D3DXVECTOR3 vLevel10BodyScale( 3.0f, 4.5f, 3.0f );
	const D3DXVECTOR3 vLevel9FeetScale( 1.5f, 1.5f, 1.5f );

	switch( m_nZckManagedSfxLevel )
	{
		case eSetItemSFX_Level::eLevel_3:
			AddZckManagedSetSfx( dwSfx, 0, 0.1f, vDefaultScale );
			AddZckManagedSetSfx( dwSfx, 1, 0.1f, vDefaultScale );
			break;

		case eSetItemSFX_Level::eLevel_4:
			AddZckManagedSetSfx( dwSfx, 6, 0.0f, vBodyScale );
			break;

		case eSetItemSFX_Level::eLevel_5:
			AddZckManagedSetSfx( dwSfx, 0, 0.1f, vDefaultScale );
			AddZckManagedSetSfx( dwSfx, 1, 0.1f, vDefaultScale );
			AddZckManagedSetSfx( dwSfx, 4, 0.0f, vDefaultScale );
			AddZckManagedSetSfx( dwSfx, 5, 0.0f, vDefaultScale );
			break;

		case eSetItemSFX_Level::eLevel_6:
			AddZckManagedSetSfx( dwSfx, 0, 0.0f, vDefaultScale );
			AddZckManagedSetSfx( dwSfx, 1, 0.0f, vDefaultScale );
			AddZckManagedSetSfx( dwSfx, 4, 0.0f, vDefaultScale );
			AddZckManagedSetSfx( dwSfx, 5, 0.0f, vDefaultScale );
			AddZckManagedSetSfx( dwSfx, 2, 0.1f, vDefaultScale );
			AddZckManagedSetSfx( dwSfx, 3, 0.1f, vDefaultScale );
			break;

		case eSetItemSFX_Level::eLevel_7:
			AddZckManagedSetSfx( dwSfx, 0, 0.0f, vDefaultScale );
			AddZckManagedSetSfx( dwSfx, 1, 0.0f, vDefaultScale );
			AddZckManagedSetSfx( dwSfx, 4, 0.0f, vDefaultScale );
			AddZckManagedSetSfx( dwSfx, 5, 0.0f, vDefaultScale );
			AddZckManagedSetSfx( dwSfx, 2, 0.1f, vDefaultScale );
			AddZckManagedSetSfx( dwSfx, 3, 0.1f, vDefaultScale );
			AddZckManagedSetSfx( dwSfx, 8, 0.1f, vDefaultScale );
			AddZckManagedSetSfx( dwSfx, 9, 0.1f, vDefaultScale );
			break;

		case eSetItemSFX_Level::eLevel_8:
			AddZckManagedSetSfx( dwSfx, 0, 0.0f, vDefaultScale );
			AddZckManagedSetSfx( dwSfx, 1, 0.0f, vDefaultScale );
			AddZckManagedSetSfx( dwSfx, 4, 0.0f, vDefaultScale );
			AddZckManagedSetSfx( dwSfx, 5, 0.0f, vDefaultScale );
			...
			AddZckManagedSetSfx( dwSfx, 8, 0.1f, vDefaultScale );
			AddZckManagedSetSfx( dwSfx, 9, 0.1f, vDefaultScale );
			AddZckManagedSetSfx( dwSfxHands, 9, 0.2f, vDefaultScale );
			break;

		case eSetItemSFX_Level::eLevel_9:
			AddZckManagedSetSfx( dwSfx, 0, 0.0f, vDefaultScale );
			AddZckManagedSetSfx( dwSfx, 1, 0.0f, vDefaultScale );
			AddZckManagedSetSfx( dwSfx, 4, 0.0f, vDefaultScale );
			AddZckManagedSetSfx( dwSfx, 5, 0.0f, vDefaultScale );
			...
			AddZckManagedSetSfx( dwSfx, 8, 0.1f, vDefaultScale );
			AddZckManagedSetSfx( dwSfx, 9, 0.1f, vDefaultScale );
			AddZckManagedSetSfx( dwSfx, 26, 0.0f, vLevel9FeetScale );
			AddZckManagedSetSfx( dwSfx, 29, 0.0f, vLevel9FeetScale );
			AddZckManagedSetSfx( dwSfxHands, 9, 0.2f, vDefaultScale );
			break;

		case eSetItemSFX_Level::eLevel_10:
			AddZckManagedSetSfx( dwSfx, 0, 0.0f, vDefaultScale );
			AddZckManagedSetSfx( dwSfx, 1, 0.0f, vDefaultScale );
			AddZckManagedSetSfx( dwSfx, 4, 0.0f, vDefaultScale );
			AddZckManagedSetSfx( dwSfx, 5, 0.0f, vDefaultScale );
			AddZckManagedSetSfx( dwSfx, 6, 0.0f, vLevel10BodyScale );
			AddZckManagedSetSfx( dwSfx, 7, 0.0f, vBodyScale );
			AddZckManagedSetSfx( dwSfx, 8, 0.1f, vDefaultScale );
			AddZckManagedSetSfx( dwSfx, 9, 0.1f, vDefaultScale );
			AddZckManagedSetSfx( dwSfx, 2, 0.0f, vDefaultScale );
			AddZckManagedSetSfx( dwSfx, 3, 0.0f, vDefaultScale );
			AddZckManagedSetSfx( dwSfx, 26, 0.0f, vDefaultScale );
			AddZckManagedSetSfx( dwSfx, 29, 0.0f, vDefaultScale );
			AddZckManagedSetSfx( dwSfxHands, 8, 0.2f, vDefaultScale );
			AddZckManagedSetSfx( dwSfxHands, 9, 0.2f, vDefaultScale );
			break;
...
void CMover::DeleteZckManagedSfx()
{
	for( std::vector<CSfx*>::iterator it = m_vecZckManagedSfx.begin(); it != m_vecZckManagedSfx.end(); ++it )
		SAFE_DELETE( *it );

	m_vecZckManagedSfx.clear();
	m_nZckManagedSfxLevel = -1;
	m_nZckManagedSfxEffectValue = -1;
}

void CMover::ProcessZckManagedSfx()
{
	for( std::vector<CSfx*>::iterator it = m_vecZckManagedSfx.begin(); it != m_vecZckManagedSfx.end(); )
	{
		CSfx* pSfx = *it;
		if( pSfx == NULL )
		{
			it = m_vecZckManagedSfx.erase( it );
			continue;
		}
...
void CMover::RefreshZckManagedSfx()
{
	const int nAbilityOption = GetSetItemClient();
	const int nEffectValue = GetSetItemSFX_EffectValue();
	const BOOL bShouldHaveSfx = nAbilityOption >= eSetItemSFX_Level::eLevel_3 ? TRUE : FALSE;
	const BOOL bHasSfx = HasZckManagedSfx();

	if( m_nZckManagedSfxLevel == nAbilityOption &&
		m_nZckManagedSfxEffectValue == nEffectValue &&
		bShouldHaveSfx == bHasSfx )
	{
		return;
	}
...
#ifdef __WORLDSERVER
	if( IsPlayer() )	{
		((FLWSUser* )this)->OnMsgArrival( dwParam1 );    // dwParam1 - pickup´ë»ó
	}
	else	{
		PostAIMsg( AIMSG_ARRIVAL, dwParam1, dwParam2 );
		ClearDestObj();		// ¸ñÇ¥¿¡ µµ´ÞÇÏ¸é ÃßÀûÀ» ¸ØÃã.
...
void CMover::RemoveItemId( DWORD dwId )
{
	if( m_Inventory.IsEquip( dwId ) )
		DoEquip( (FLItemElem*)m_Inventory.GetAtId( dwId ), FALSE );		// ½ÇÆÐÇÒ °æ¿ì, ÀåÂø ÇØÁ¦ È¿°ú°¡ ³ªÅ¸³ªÁö ¾ÊÀ¸¹Ç·Î °í·Á°¡ ÇÊ¿äÇÔ.
...
BOOL CMover::SetMotion( DWORD dwMotion, int nLoop, DWORD dwOption )
{
	...
	//#0012912 [Àü±¹°¡_Á¤¼·_16Â÷,17Â÷] Ä³¸¯ÅÍ°¡ ¾É¾Ò´Ù ÀÏ¾î ³¯ ¶§ ¹«±â¾ÆÀÌÅÛ º¯°æ½Ã ÀÌµ¿ ºÒ°¡µÇ´Â Çö»ó
	// ¸ð¼Ç°ú »óÅÂ°¡ ´ë·« ²¿¿©ÀÖ´Ù. 
	// ÀÏ¾î³ª±â µ¿ÀÛÁß ¹æÇØ¸¦ ¹ÞÀ¸¸é °á±¹ SIT»óÅÂ°¡ º¯ÇÏÁö ¸øÇÒ¼öµµ ÀÖ´Ù. 
	// ÇÑ¸¶µð·Î ¸ð¼Ç°ú »óÅÂ´Â ºÐ¸®µÇ¾î¾ß ÇÏ´Âµ¥ ¼­·Î ¿µÇâÀ» ³¢Ä¡°íÀÖ´Ù. 
#ifdef __CLIENT
	if( MTI_GETUP == m_dwMotion && MTI_GETUP != dwMotion )
	{
		//ÀÏ¾î¼­±âÁß ¹æÇØ¸¦ ¹ÞÀ¸¸é ¹Ù·Î ¸®¼Â½ÃÅ²´Ù.
			...
			nIndex == OBJECT_INDEX( 202, MI_DWARPETMAS ) ) ) )  
		{
			...
			else
			{
				...
				case IK3_BOW:
					...
					//¿ì¸® ½ºÅ³Àº ÇÊ¿ä¹«±â¸¦ 2°³ÀÌ»ó µî·ÏÇÒ¼ö ¾ø´Ù ÇØ¼­ IK3_CROSSBOW->IK3_BOW·Î ¹Ù²ã¼­ ´Ù ¾µ¼öÀÖ°Ô ÇÏ°í
					//¸ð¼ÇÀº Å©·Î½ºº¸¿ì ¸ð¼ÇÀÌ ³ª¿À°Ô ÇØ¾ßÇÑ´Ù.
				...
				case IK3_WAND:
					...
					//sun: 10Â÷ Àü½Â½Ã½ºÅÛ	Neuz, World, Trans
					//gmpbigsun( 20101111 ) : ¸àÅ»¸®½ºÆ® ÀÌµ¿¸ð¼Ç ¼öÁ¤
...
int CMover::DoDie( CCtrl *pAttackCtrl, DWORD dwMsg )
{
	...
	if( IsPlayer() && m_nDead )
	{
		//FLERROR_LOG( PROGRAM_NAME, _T( "%s Á×Àº ÈÄ 5ÃÊÀÌ³» ¶ÇÁ×¾ú´Ù. µÎ¹øÁ×Àºµí?" ), GetName() );		//¶ÇÁ×À» ¼ö ÀÖ´Â ÄÚµå ¸¹´Ù. ·Î±× Á¶³½ ÂïÈù´Ù
		return 0;
	}
	...
	{
		pAttacker->SendAIMsg( AIMSG_INIT_TARGETCLEAR, m_objid );		// °ø°ÝÀÚ´Â Å¸°ÙÀ» Å¬¸®¾îÇÏ°í ´ë±â¸ðµå·Î µ¹¾Æ°¨.
	}
	...
	if( CInstanceDungeonHelper::GetInstance()->IsInstanceDungeon( GetWorld()->GetID() ) )
	{
		if( !IsPlayer() && IsValidObj( pAttacker ) == TRUE && pAttacker->IsPlayer() )
			...
			CInstanceDungeonHelper::GetInstance()->SetInstanceDungeonKill(pAttacker->m_idPlayer, GetWorld()->GetID(), static_cast<DWORD>(this->GetLayer()), GetProp()->dwID);
#else
...
#if defined __WORLDSERVER && defined __AGGRO16
	//pMonster->SendAIMsg( AIMSG_DSTDIE, m_objid );				//¾È¿¡¼­ m_AggroOwnerList.erase¸¦ ÇÏ±â¶§¹®¿¡.. vec¿¡¼­ ¼öÁýÈÄ send
...
int	CMover::SubPK( CMover *pAttacker, int nReflect )
{
	...
	/// Ä«¿À¼öÄ¡, ¼ºÇâÀ» ¿Ã·ÁÁÜ
	if( bAdd && nReflect == 0 && pAttacker != this ) // chipi_081119 Áö¼ÓÇü µ¥¹ÌÁö ½ºÅ³ ¹Þ°í ÀçÁ¢ÇÏ¸é ÀÚ»ìµÇ¾î Ä«¿ÀµÇ´Â ¹®Á¦ ¼öÁ¤...
	{
...
#define MAX_AROUND_USER		512
	...
	FLTRACE_LOG( PROGRAM_NAME, _T( "º¸½º¸÷ Á×ÀÓ %s" ), pAttacker->GetName() );
	...
	NEXT_VISIBILITYRANGE( this )
		...
		if( nMaxList == 0 )
		{
			FLERROR_LOG( PROGRAM_NAME, _T( "%sÁÖÀ§¿¡ À¯Àú°¡ ¾ø´Ù." ), GetName() );
			return;
		}
		...
		for( i = 0; i < nMaxList; i ++ )
		{
			...
			{
				// ·¹º§¾÷ µÆ´Ù.
				g_xWSUserManager->AddSetLevel( pUser, (WORD)pUser->m_nLevel );		// pUserÀÇ ÁÖÀ§»ç¶÷¿¡°Ô pUser°¡ ·¹º§ÀÌ ¿Ã¶ú´Ù´Â°É º¸³¿.
				((FLWSUser*)pUser)->AddSetGrowthLearningPoint( m_Stat.GetRemainStatPoint() );		// pUser¿¡°Ô GPº¯µ¿µÈ°ÍÀ» º¸³¿.
...
void CMover::AddExperienceSolo( EXPFLOAT fExpValue, MoverProp* pMoverProp, float fFxpValue, BOOL bParty )
{
	...
	if( dw_Level > 0 )
	{
		if( dw_Level >= 1 && dw_Level <= 5 )		// 1~5 Â÷ÀÌ
		{
			fExpValue	*= static_cast<EXPFLOAT>( 0.7f );
			fFxpValue	*= 0.7f;
		}
		else if( dw_Level >= 6 && dw_Level <= 10 )	// 6~10 Â÷ÀÌ
		{
			fExpValue	*= static_cast<EXPFLOAT>( 0.5f );
			fFxpValue	*= 0.5f;
		}
		else if( dw_Level >= 10 && dw_Level <= 14 )	// 10~14 Â÷ÀÌ
		{
	...
	FLTRACE_LOG( PROGRAM_NAME, _T( "Name : %s ¾ò¾îÁö´Â : %I64d" ), this->GetName(), static_cast<EXPINTEGER>( fExpValue ) );
	FLTRACE_LOG( PROGRAM_NAME, _T( "·¹º§¾÷¿¡ ÇÊ¿äÇÑ:%I64d" ), prj.m_aExpCharacter[m_nLevel+1].nExp1 - m_nExp1 );
...
void CMover::AddExperienceParty( CMover *pDead, EXPFLOAT fExpValue, MoverProp* pMoverProp, float fFxpValue, CParty* pParty, FLWSUser* apMember[], int* nTotalLevel, int* nMaxLevel10, int* nMaxLevel, int* nMemberSize )
{
	...
	// ¸ó½ºÅÍ ·¹º§°ú ±Ø´ÜÃÑ·¹º§ÀÌ 5ÀÌ»ó ¸ó½ºÅÍ·¹º§ÀÌ ÀûÀ¸¸é °æÇèÄ¡¿Í Æ÷ÀÎÆ®°¡ ¾È¿Ã¶ó°¨ - 20À¸·Î º¯°æÇÕ´Ï´Ù(2016.05.03.)
	pParty->GetPoint( (*nTotalLevel), (*nMemberSize), pDead->GetLevel() );

	if( pParty->GetKind() == PARTY_KIND_EXTEND ) // ¼øÈ¸±Ø´Ü
	{
...
//		float fContribution = 0;
	...
	float fAddEventExp = 0.0f;	// ±Ø´Ü °æÇèÄ¡ ÀÌº¥Æ®
...
{
	//sun: ÆÖ, º¯½Å ¾ÆÀÌÅÛ Åä±Û¹öÇÁ Àû¿ë... Neuz, World
	...
	case 1:		// °³ÀÎ PVP
		{
			...
			{
				((FLWSUser*)this)->AddDefinedText( TID_PK_LEVEL_GAP, "" );	// ·¹º§ÀÌ 30ÀÌ»ó Â÷ÀÌ°¡ ³ª¸é PVP¸¦ ÇÒ¼ö ¾ø½À´Ï´Ù
				return FALSE;
			}
			//				1	// 2	// 3	// 4	// 5
			...
			if( nError != 0 )
			{
				...
				{
				case 1:
					((FLWSUser*)this)->AddDefinedText( TID_PK_SAFETY_NO, "" );	// ¾ÈÀü¿µ¿ª¿¡¼­´Â PVPÇÒ¼ö ¾ø½À´Ï´Ù
					break;
				case 2:
					((FLWSUser*)this)->AddDefinedText( TID_PK_SAME_NO, "" );	// °°Àº¿µ¿ª¿¡ ÀÖ¾î¾ß PVP¸¦ ÇÒ¼ö ÀÖ½À´Ï´Ù
					break;
				case 3:
					((FLWSUser*)this)->AddDefinedText( TID_PK_CHANGEJOB_NO, "" );	// 1Â÷ÀüÁ÷ ÀÌÈÄ¿¡ PVP¸¦ ÇÒ¼ö ÀÖ½À´Ï´Ù
					break;
				}
				return FALSE;
			}
...
BOOL CMover::SubPKPVPInspectionBase( CMover* pMover, CMover* pMover1, DWORD dwPKAttr, int /*nFlag*/ )
{
	if( IsBaseJob() ) // ÀüÁ÷À» ¾ÈÇÞÀ½
	{
		...
		if( IsValidObj( pMover ) )
			((FLWSUser*)pMover)->AddDefinedText( TID_PK_CHANGEJOB_NO, "" );	// 1Â÷ÀüÁ÷ ÀÌÈÄ¿¡ PVP¸¦ ÇÒ¼ö ÀÖ½À´Ï´Ù
#endif // __WORLDSERVER
...
#endif // __CLIENT
#ifdef __WORLDSERVER
		((FLWSUser*)this)->AddDefinedText( TID_PK_NOFLIGHT_DURING, "" );	// ºñÇàÁßÀÌ¸é µà¾óÀ» ÇÒ¼ö ¾ø½À´Ï´Ù
		if( IsValidObj( pMover ) )
			((FLWSUser*)pMover)->AddDefinedText( TID_PK_NOFLIGHT_DURING, "" );	// ºñÇàÁßÀÌ¸é µà¾óÀ» ÇÒ¼ö ¾ø½À´Ï´Ù
#endif // __WORLDSERVER		
		//		pMover->PrintString( pMover1, TID_GAME_PVP_LOWLEVEL2 );
		return FALSE;
	}
	...
	if( IsRegionAttr( dwPKAttr ) == FALSE )	// °°Àº Áö¿ª¿¡ ÀÖ´ÂÁö È®ÀÎ
	{
		...
		((FLWSUser*)this)->AddDefinedText( TID_PK_SAME_NO, "" );	// °°Àº¿µ¿ª¿¡ ÀÖ¾î¾ß PVP¸¦ ÇÒ¼ö ÀÖ½À´Ï´Ù
		if( IsValidObj( pMover ) )
			((FLWSUser*)pMover)->AddDefinedText( TID_PK_SAME_NO, "" );	// °°Àº¿µ¿ª¿¡ ÀÖ¾î¾ß PVP¸¦ ÇÒ¼ö ÀÖ½À´Ï´Ù
#endif // __WORLDSERVER	
...
int CMover::IsSteal( CMover *pTarget )
{
	...
	if( g_xFlyffConfig->GetMainLanguage() == LANG_THA && pTarget->GetIndex() == OBJECT_INDEX( 199, MI_MRPUMPKIN5 ) )	// ÅÂ±¹ÀÇ °æ¿ì ±«¼ö ¹Ì½ºÅÍÆßÅ²Àº ½ºÆ¿ÀÌ Çã¿ëµÊ.
		return 0;												
		...
		//!	if( pTarget->IsNPC() || (pTarget->IsPlayer() && pTarget->IsChaotic() == FALSE) )	// °ø°ÝÀÚ:ÇÃ·¹ÀÌ¾î, ¸Â´ÂÀÚ:NPC È¤Àº ÂøÇÑ»ç¶÷. (½½·ÎÅÍ´Â ´Ù±¸¸®´çÇØµµ µÈ´Ù).
	{
		...
		{
			((FLWSUser*)this)->AddDefinedText( TID_GAME_PRIORITYMOB, "" );	// ¸ó½ºÅÍ°¡ ´Ù¸¥Ä³¸¯ÅÍ¶û ÀüÅõÁßÀÓ´Ù.
			return 1;		// ½ºÆ¿.
		}
	}
...
// 			{
// 				g_xWSUserManager->AddSetLevel( this, (WORD)m_nLevel );		// thisÀÌ¿ÜÀÇ ÁÖº¯¿¡ ·¹º§º¯°æÁ¤º¸¸¦ º¸³¿.
// 				((FLWSUser *)this)->AddSetGrowthLearningPoint( m_Stat.GetRemainStatPoint() );		// ÇØ´çÀ¯Àú¿¡°Ô GPº¯°æÁ¤º¸¸¦ º¸³¿.
// 
// 				g_dpDBClient.SendUpdatePlayerData( static_cast<FLWSUser*>( this ) );		// ÇÃ·¹ÀÌ¾î µ¥ÀÌÅÍ °»½Å
// 			}
// 
// 			( (FLWSUser *)this )->AddSetExperience( GetExp1(), (WORD)m_nLevel,  m_nSkillPoint, m_nSkillLevel, m_nDeathExp, (WORD)m_nDeathLevel );		// ÇØ´çÀ¯Àú¿¡°Ô exp1,exp2º¯°æµÈ Á¤º¸¸¦ º¸³¿.
// 
...
BOOL CMover::OnDie( CMover & kAttacker )
{
	...
	if( m_nPartyQuestKeeping > 0 ) //ÆÄÆ¼Äù½ºÆ® ÁøÇàÁßÀÏ¶§? ÆÄÆ¼Äù½ºÆ®¸¦ Å°ÇÎÇØ¾ßÇÒ¶§?
	{
...
BOOL CMover::DropItem( CMover & kAttacker )
{
	...
	// NPC°¡ ·íÇÑ ¾ÆÀÌÅÛÀ» “b´Â´Ù.
//	MoverProp* pMoverProp	= GetProp();
	if( GetProp()->m_nLoot == 2 )	// 2¾Æ d¿É¼Ç.  ¾ÆÀÌÅÛ ¸Ô°í ¹ñ±â±îÁö ÇÏ´Â ¿É¼Ç.
	{
...
bool	CMover::IsForceItemDropMonster( CMover* pAttacker )
{
	...
	{
// 		// mirchang_110628 ÄÝ·Î¼¼¿ò¿¡¼­ Á×ÀÎ ¸ó½ºÅÍ´Â ·¹º§Â÷ÀÌ¿¡ ÀÇÇÑ µå·ÓÀ² ÀúÇÏ°¡ ¾øµµ·Ï ¶«»§Ã³¸®..
...
BOOL CMover::DropItem( CMover* pAttacker )
{
	...
	if( pAttacker->IsPlayer() && IsNPC() )
	{
		//	mulcom	BEGIN100315	A¨¬A¡ÌA†A¢çA©øA©÷ A¨öAƒA¡ÆA¡Ì AA|A‡A‘
 		if( g_xFlyffConfig->GetMainLanguage() == LANG_VTN )
 		{
		...
		//	mulcom	END100315	A¨¬A¡ÌA†A¢çA©øA©÷ A¨öAƒA¡ÆA¡Ì AA|A‡A‘
		...
		if( m_nPartyQuestKeeping > 0 ) //A†A„A†A¨ùA„A©öA¨öA¨¬A†A¢ç AA¢¬A‡A?AAŸA€AA¢ÒA¡×? A†A„A†A¨ùA„A©öA¨öA¨¬A†A¢çA¢¬A| A…A¡ÆA‡AŽA‡A˜A¨úAŸA‡A’A¢ÒA¡×?
		{
			CPartyQuestProcessor* pProcessor	= CPartyQuestProcessor::GetInstance();
			PARTYQUESTELEM* pElem	= pProcessor->GetPartyQuest( m_nPartyQuestKeeping );
			if( pElem && pElem->objidWormon == GetId() )
		...
		if( ( pAttacker->m_nLevel - (int)lpMoverProp->dwLevel ) < 10 )
		{
			...
			while( pEventItem )
			{
				...
				else
				{
					...
					#ifdef __ZCK_ARME_RARE
					pItemElem->SetRandomZckArmeRare();
					#endif // __ZCK_ARME_RARE
					pItemElem->m_nHitPoint	= 0;
			...
			//sun: 9, A€AŒA¨¬A¡ÍA†A¢ç (A¡¤A¡×A¨úA† A¨öA¨¬A…A¨ÏA¢¬A©øA†A¢ç A€A¡íA¢¯A¡ì)
		...
		// A„A©öA¨öA¨¬A†A¢ç A¨úA†A€AŒA…A› A¥ìA¡ÍA¢ÒA¢¬
			...
			if( lpQuest && lpQuest->m_nState == pQuestItem->dwState ) 
			{
				...
				if( xRandom( 3000000000 ) <= dwProbability ) 
				{
					if( pQuestItem->dwNumber == 0 )
						FLERROR_LOG( PROGRAM_NAME, _T( "%sA€A‡ quest item drop %dA©öA¢¬A‚A¡ÆA€A‡ dwNumberA¡ÆA¢® 0" ), GetName(), i );
		...
		int nloop = 1;
		BOOL bUnique = FALSE;
		if( pAttacker->m_idparty ) // A©øA¡íA¡ÆA¢® A†A„A†A¨ùA¢¯A¢® A‚A¨ùA¢¯A¨Ï A¢¯A¨ÏA¨¬AŽ
		{
			CParty* pParty = g_PartyMng.GetParty( pAttacker->m_idparty );
			if( pParty )
			{
				// A¨ùA¢¬AˆA¢¬A¡¾A˜A¢¥AœA€AŒA¢¬A¡× A¢¬A¥ìA…A¨ÏA¨úA¢çA…AƒAAŸA€AŒA¢¬A¡× A¢¥AœA€A¡ÍA¢¬A¢¬ A¥ìA¡ÍA©öAŒAA¢ÒA¢¬A| A©øA¢¥A¢¯A¨ÏAA˜A¢¥A™
				if( pParty->GetKind() == PARTY_KIND_EXTEND )
				{
					...
					{
						//sun: 12, A†A„A¨öA¨¬A…A©ø A¨úA†A€AŒA…A› A¨ùA¢ÒAA¢´
//						if( pParty->m_nModeTime[PARTY_PARSKILL_MODE] )
		...
		D3DXVECTOR3 vPos;		// A¥ìA¡ÍA¢ÒA¢¬A¥ìA‰ A€A¡×A„A¢®.

#ifdef __EVENT_MONSTER
		// A€AŒA¨¬A¡ÍA†A¢ç A¢¬A©øA¨öA¨¬A…AA¢¥A‚ A¨ùA©øAA¢´A¢¯A¢® A¥ìA¡íA¢ÒA©ø A‡A‘A©öA¢¬A¢¬A¢¬ A¥ìA¡ÍA¢ÒA¢¬A‡A‘A¢¥A™.
		...
		{
			// A¨úA†A€AŒA…A› A¥ìA¡ÍA¡¤A“ AˆA¢çA¡¤A¨ù		
			int nProbability	= 100, nPenyaRate	= 100;
			BOOL bAdjDropRate = TRUE;	// A€AA©öAA€A¡íA€A¢¬A¡¤A A¡¤A©öA¨¬A¡×A‚A¡¤A¢¯A¢® A€A‡A‡A‘ A¥ìA¡ÍA¢ÒA¢¬A¡¤A¨ùA€A¨¬A‡AA¡ÆA¢® A€AŒA¡¤A?AA¢¬A¢¥A™.
			...
			if( xRandom( 100 ) < fItemDropRate )	// A¨úA†A€AŒA…A›A€A¡í A¥ìA¡ÍA¡¤A“A‡A’AA¢Ò A¢¬A¡íAA¢Ò A¡ÆA¢®AA¢´. A¡¤A©öA¨¬A¡×A‚A¡¤A¡ÆA¢® A¢¬A©öA€AŒ A©øA¨£A¢¬A¨Ï A¨úA†A¢¯A©ö A¢ÒA©øA¨úA¢çA†A¢çA¢¬A¢çAA¢Ò A¨úAŠA¢¥A‚A¢¥A™.
			{
				...
				for( int i = 0; i < nSize; i++ )
				{
					if( ( lpDropItem = lpMoverProp->m_DropItemGenerator.GetAt( i, bUnique, GetPieceItemDropRateFactor( pAttacker ) ) ) != NULL )
					{
						if( lpDropItem->dtType == DROPTYPE_NORMAL )
						{
							...
							if( lpMoverProp->dwFlying )
							{
								...
								itemElem.m_nItemNum	= max( 1, itemElem.m_nItemNum );	// AƒA–A¨ùA’ A‡AA©øA¨£A¢¥A‚ A¢ÒA©øA¡¾A…A¨úAŸA‡AA©øA¨£ A¨¬A¨ö A¢´A¡í
							...
							pItemElem->m_nItemNum	= max( 1, pItemElem->m_nItemNum );	// AƒA–A¨ùA’ A‡AA©øA¨£A¢¥A‚ A¢ÒA©øA¡¾A…A¨úAŸA‡AA©øA¨£ A¨¬A¨ö A¢´A¡í
							...
							pItemElem->SetAbilityOption( lpDropItem->dwLevel );		// AƒAŸA¡ÆA¢® A¢¥A‰A¡¤A‚A„A¢® +1, +2 A¡ÆA¡ÆA€A¨¬A¡ÆA….
							#ifdef __ZCK_ARME_RARE
							pItemElem->SetRandomZckArmeRare();
							#endif // __ZCK_ARME_RARE
							pItemElem->SetSerialNumber();
							CItem* pItem	= new CItem;
							pItem->m_pItemBase	= pItemElem;
							BOOL bJJim = TRUE;		// AA–A€AŽA€AŒ A€A–A¢¥A™A¢¥A‚A¡ÆA‰ A‡A¡ÍA¨öAƒA‡A˜A¨úAŸA‡AA¢¥A‚ A…A›A€AŽA¡ÆA¢®.
							if( lpMoverProp->dwClass == RANK_SUPER )		// A¨¬A¢¬A¨öA¨¬A¢¬A¡¤A€AŒ A¥ìA¡ÍA¡¤A“A‡A‘ A¨úA†A€AŒA…A›A€A¨¬ A¨úA†A©öA¡ìA©øA¨£ A¢¬A”A€A¡íA¨ùA¢Ò A€A–A¢¥A™.
								bJJim = FALSE;
							if( GetIndex() == OBJECT_INDEX( 196, MI_DEMIAN5 ) || GetIndex() == OBJECT_INDEX( 197, MI_KEAKOON5 ) || GetIndex() == OBJECT_INDEX( 198, MI_MUFFRIN5 ) )
								bJJim = FALSE;		// A¨úA¨£A¥ìA¨ÏA€A¨¬ A€AŒA¨¬A¡ÍA†A¢çA¢¬A¡¤A€AŒA©öA‡A¡¤AŽ A‚A©÷A¨úAˆA‡A˜A©øA¢ÒA¥ìA¥ì A¥ìAˆA¢¥A™. A¨úA†A©öA¡ìA©øA¨£ A¢¬A”A€A¡íA¨ùA¢Ò A€A–A€A¨ö
							if( bJJim )
							{
								pItem->m_idOwn = pAttacker->GetId();	// A€AŒ A¨úA†A€AŒA…A›A€A‡ A¨ùA’A€A?A¡ÆA¢® pAttacker(A¨úA¢çA…A‚A„A¢¯)A©÷A¡§A¢ÒA¥ìA¡ÆA‰ A‡A¡ÍA¨öAƒ.
								pItem->m_dwDropTime = timeGetTime();	// A¥ìA¡ÍA¢ÒA¢¬ A‡AŸA€A¡íA¢¥A¡×A¨öAƒA€A‡ A¨öAƒA¡ÆA¡ÌA€A¡í A¡¾A¡ËA¡¤AA‡A”.
							}
							pItem->m_bDropMob = TRUE;		// A¢¬A¡¤A€AŒ AA—A¨úA¢çA¨ùA¡© A¢ÒA©øA¡¾A¨¬ A¥ìA¡¤A€A¨¬ A‡A¡ÍA¨öAƒA¢¬A| A‡A˜A¥ìA’.
							...
							vPos.y = GetPos().y;
#ifdef __EVENT_MONSTER
							// A€AŒA¨¬A¡ÍA†A¢ç A¢¬A©øA¨öA¨¬A…AA¡ÆA¢® A¥ìA¡ÍA¢ÒA¢¬A‡A‘ A¨úA†A€AŒA…A›A€A¨¬ A¢¬A©øA¨öA¨¬A…AA€A‡ IDA¢¬A| A¡¾A¡ËA¨úA?A‡A‘A¢¥A™(A†A¨£A€AŒ A¢¬A¢¬ AAA¡ÆA”...)
							if( CEventMonster::GetInstance()->SetEventMonster( lpMoverProp->dwID ) )
							{
								// A€AŒA¨¬A¡ÍA†A¢ç A¢¬A©øA¨öA¨¬A…AA¢¥A‚ A©öA¡ìAA¢ÒA¡ÆA‡ A¨ùA¡¾AA¢®A¡¾A‡A€A¡í A¡ÆA¢çA¢¥A‚A¢¥A™.
								pItem->m_idOwn	= pAttacker->GetId();
								pItem->m_dwDropTime		= timeGetTime();
							...
							if( GetWorld()->ADDOBJ( pItem, TRUE, GetLayer() ) == TRUE )
							{
									...
									|| ( pItemProp->dwItemKind1 == IK1_GENERAL && pItemProp->dwItemKind2 == IK2_JEWELRY ) )
								{
									switch( pItemProp->nLog )
									{
									case 1:		g_dpDBClient.SendLogUniqueItem( pAttacker, pItem, pItemElem->GetAbilityOption() );	// A€AA©öA A¨úA†A€AŒA…A› +A¨úA†A€AŒA…A›
										break;
									case 2:		g_dpDBClient.SendLogUniqueItem( pAttacker, pItem, 200 );	// A€AŒA†A¡ÍA†A¢ç A¨úA†A€AŒA…A›
										break;
									case 3:		g_dpDBClient.SendLogUniqueItem( pAttacker, pItem, 100 );	// A€A?A¢¥AA…A¨Ï A¨úA†A€AŒA…A›
										break;
									}
								}
						...
						// A¥ìA¡¤A€A¨¬ A©öA¡ìAA¢ÒA¡ÆA‡A¢ÒA©øA¨úA¢çAA¢çA¨úAŸ A‡A‘A¢¥A™.
						else if( lpDropItem->dtType == DROPTYPE_SEED && k == 0 )
						{
							int	nSeedID = 0;
							int nNumGold = lpDropItem->dwNumber + xRandom( lpDropItem->dwNumber2 - lpDropItem->dwNumber );	// Number ~ Number2 A¡íA¡×A€AŒA€A‡ A¡¤A¡ÌA¢¥A¨öA¡ÆA¨£.
							...
							nNumGold	= (int)( nNumGold * prj.m_EventLua.GetGoldDropFactor() );	//sun: 9, A€AŒA¨¬A¡ÍA†A¢ç (A¡¤A¡×A¨úA† A¨öA¨¬A…A¨ÏA¢¬A©øA†A¢ç A€A¡íA¢¯A¡ì)

#ifndef __AUTO_PENYA_PICKUP
							if (lpMoverProp->dwFlying)
#endif
							{
							...
							{
								// A¥ìA¡¤A¨úA—A¨ùA¢ÒA¢¯A¢® A¥ìA¡íA¢ÒA©ø A¨úA¢çA¢ÒA©÷A¢¬A¡ÆA¨úA¡×A€A‡ A¨öAƒA¥ìA¡ÍA¢¬A| A¡íA¡×A¢¯A¡ìA‡A’AA¢Ò A¡ÆA¢®AA¢´A‡A‘A¢¥A™.
								...
								pItemElem->m_nItemNum	= nNumGold;	// A¥ìA¡¤A¨úA—A¨ùA¢Ò
					...
					//////////////
					//  A¢¯A¨ÏA¡¾A¡ËA¡¾A¢çAA¢Ò for-loopA¨úAˆA©øA¡íA¡¤AA¢¯A€A¡ÆA¡© continueA‡AA¢¥A‚ A¨ùA¢ÒA¥ìA¥ì A€A–A€A¢¬A¢¥AA¡¾A¢ç A¢¯A¨ÏA¡¾A¡ËA¢¥A™ A„AšA¥ìA¡Í A©øA–AA¢Ò A¢¬A¡íA¡ÆA.
					///////////
				} // for nSize
				...
				for( int i = 0; i < nSize; i++ )
				{
					...
					{
						if( bDrop )		// KINDA¨úA†A€AŒA…A›A€A¡í A‡AA©øA¨£ A¢ÒA©øA¨úA¢çA†A¢çA¡¤AˆA¢¥A™A¢¬A¨Ï 
							break;		// A¢¥A¥ìA€AŒA¡íA©ø A¢ÒA©øA¨úA¢çA†A¢çA¢¬A¢çAA¢Ò A¨úAŠA¡ÆA¡© AA¨úA¡¤A¢®.
					}
				} // for( i = 0; i < nSize; i++ )
	...
	if( IsNPC() )	// A¢¬A¡¤A€AŒ AA—A¨úA¨¬A€A¡íA¢ÒA¡×.. // AA–A€A¨¬ A¨úA†A€AŒA…A› a€œbA¡¾A¡Ë A€AŽA¥ìA¡©??????
	{
		MoverProp* pMoverProp	= GetProp();
		if( pMoverProp->m_nLoot == 2 )	// 2A¨úA† dA¢¯A‰A¨ùA‡.  A¨úA†A€AŒA…A› A¢¬A”A¡ÆA¡© A©öA¡¾A¡¾A¡ËA¡¾A¢çAA¢Ò A‡AA¢¥A‚ A¢¯A‰A¨ùA‡.
		{
			DWORD dwStart = m_Inventory.GetMax() - m_Inventory.GetSize();
			DWORD dwSize = m_Inventory.GetMax();
...
{
	if( g_xFlyffConfig->GetMainLanguage() == LANG_THA )	// ÅÂ±¹ÀÇ °æ¿ì
		if( GetIndex() == OBJECT_INDEX( 199, MI_MRPUMPKIN5 ) )	// ±«¼ö ¹Ì½ºÅÍÆßÅ²Àº
			return;													// ½ºÆ¿ÀÌ Çã¿ëµÊ.
				...
				else if( m_nDuel == 2 )
				{
					...
					if( pParty )
					{
						...
						else
						{
							...
							((FLWSUser *)this)->AddDuelPartyStart( pszPartyName, pParty->m_nSizeofMember, pDstMember, pParty->m_uPartyId, 1 );		// »ó´ë¹æ ¸â¹öÀÇ ID¸¦ ´Ùº¸³½´Ù. 2°ªÀ¸·Î
							((FLWSUser*)this)->AddDuelCount( 0 );
						}
					}
				}
				...
				for( int k = 0 ; k < pParty->m_nSizeofMember ; ++k )
				{
					...
					if( IsValidObj( pMember ) )
					{
						...
						else 
						{
							...
							FLERROR_LOG( PROGRAM_NAME, _T( "ÆÄÆ¼¸â¹ö %sÀÇ Á¤º¸ÀÌ»ó. %d %d" ), pMember->GetName(), pMember->m_idDuelParty, uidDuelParty );
						}
					}
				}
				if( pParty2 )
...
BOOL CMover::IsBullet( SkillProp* pSkillProp )
{
	...
	if( pSkillProp->dwNeedItemBulletKind2 != NULL_ID )
	{
		...
		if( pProp )
		{
			if( pProp->dwItemKind2 != pSkillProp->dwNeedItemBulletKind2 )		// ÇÊ¿äÃÑ¾ËÁ¾·ù¶û ´Ù¸¥°É ÀåÂøÇÏ°í ÀÖÀ¸¸é ¿¡·¯
			{
#ifdef __CLIENT
				g_WndMng.PutString( prj.GetText( dwTip ), NULL, prj.GetTextColor( dwTip ) );
				( (CWndWorld *)g_WndMng.m_pWndWorld )->m_bAutoAttack	= FALSE;
...
void CMover::ArrowDown( int nCount )
{
	...
	FLItemElem* pItemElem = m_Inventory.GetEquip( PARTS_BULLET );	// ÀåÂøµÇ¾î ÀÖ´Â È­»ì ¾ÆÀÌÅÛ ²¨³¿
	if( pItemElem != NULL )
	{
		...
		pItemProp->dwSkillReadyType != 0xffffffff )
	{
		if( pItemProp->dwItemKind2 == IK2_RIDING )
		{
			if( HasActivatedEatPet() || HasActivatedSystemPet() || HasPet() )
			{
				...
				m_dwUseItemId = 0;
				m_bItemFind = FALSE;
				SetStateNotMode( STATE_BASEMOTION_MODE, STATEMODE_BASEMOTION_CANCEL );	// ™VÆÃ »©±â
				return FALSE;
			}
		...
		if( IsStateMode( STATE_BASEMOTION_MODE ) ) 
		{
			...
			{
				( (FLWSUser*)this )->AddDefinedText(TID_PK_BLINK_LIMIT, "" );	// ½ÃÀüÁß¿¡´Â »ç¿ëÇÒ¼ö ¾ø½À´Ï´Ù
				return FALSE;
			}
			else	// ½ÃÀüÀÌ ¿Ï·áµÈ ½ÃÁ¡
			{
...
void CMover::DestParamPiercingAvail( FLItemElem* pItemElem, BOOL bSET )
{
	...
	if( pItemElem->IsPierceAble() == FALSE )	// TODO : ¿Ö ÀÌ°É·Î Ã¼Å©¸¦ ÇÏ´Â°¡?
	{
		return;
	}
...
{
	// 9Â÷ Ãß°¡ (Å¸°Ý, ÇÇ°Ý 10ÃÊ ÈÄ ÀÚµ¿ È¸º¹)
	return ( m_nAtkCnt && m_nAtkCnt < (SEC1 * 10) );
	//	return ( m_nAtkCnt && m_nAtkCnt < (SEC1 * 5) );		// 9Â÷ Ãß°¡ (Å¸°Ý, ÇÇ°Ý 10ÃÊ ÈÄ ÀÚµ¿ È¸º¹)( < 9 )
}
...
BOOL CMover::IsDropable( FLItemElem* pItemElem, BOOL bPK )
{
	return ( IsDropableState( bPK )
		...
		&& !pItemElem->IsCharged()
		&& pItemElem->GetProp()->dwItemKind3 != IK3_EGG	//sun: 9, 9-10Â÷ Æê
	...
	if( pQuestProp )
	{
		...
		if( pQuestProp->m_nEndCondState == 0 )
		{
			...
			if( pQuestProp->m_nEndCondState == pCurQuest->m_nState )
				nResult++;
		// ¿Ï·á Äù½ºÆ® Ã¼Å© 10,11,12,13,14, 15  18
				...
				if( pQuestProp->m_nEndCondOneItemNum == 0 )
					...
					if( pQuestProp->m_nEndCondItemNum == 0 )
						...
						else
						{
							...
							if( pGuild != NULL )
							{
								if( pGuild->m_nWin >= pQuestProp->m_nEndCondGuildWarWin )		// ±æµå¿ö ½Â¼ö..
								{
									++nResult;
								}
							}
			...
			if( pQuestProp->m_nBeginCondNotItemNum == 0 )
				...
				if( pQuestProp->m_nBeginCondItemNum == 0 )
					...
					else
					{
						...
						if( pGuild != NULL )
						{
							if( pGuild->m_nWin >= pQuestProp->m_nBeginCondGuildWarWin )		// ±æµå¿ö ½Â¼ö..
							{
								++nResult;
							}
						}
...
void CMover::ProcessSFXDamage( void )
{
	...
	while( iInfo != m_mapSFXInfo.end() )
	{
		...
		while( qInfo.size() > 0 && qCount.size() > 0 )
		{
			...
			{
				FLTRACE_LOG( PROGRAM_NAME, _T( "¿¹¿Ü Ã³¸®" ) );
...
void CMover::ProcessSFXExpire( void )
{
	...
	while( iInfo != m_mapSFXInfo.end() )
	{
		...
		while( q.size() > 0 )
		{
			...
			{
				q.pop();	// ¸¸·á Á¤º¸ Á¦°Å
				FLTRACE_LOG( PROGRAM_NAME, _T( "idTarget=%d, INFO ¸¸·áÁ¤º¸ Á¦°Å µÊ" ), idTarget );
				continue;
			}
			else
				break;
		...
		{
			m_mapSFXInfo.erase( idTarget );
			FLTRACE_LOG( PROGRAM_NAME, _T( "idTarget=%d, INFO ¿ÏÀü »èÁ¦" ), idTarget);
		}
	}
	// 1.2
	...
	while( iCount != m_mapSFXCount.end() )
	{
		...
		while( q.size() > 0 )
		{
			...
			{
				q.pop();	// ¸¸·á Á¤º¸ Á¦°Å
				FLTRACE_LOG( PROGRAM_NAME, _T( "idTarget=%d, COUNT ¸¸·áÁ¤º¸ Á¦°Å µÊ" ), idTarget );
				continue;
			}
			else
				break;
		...
		{
			m_mapSFXCount.erase( idTarget );
			FLTRACE_LOG( PROGRAM_NAME, _T( "idTarget=%d, COUNT ¿ÏÀü »èÁ¦" ), idTarget);
		}
	}
}
#endif	// __WORLDSERVER
...
///>_SUN_RIDER_ANIMATION --BEGIN
	//STAND³ª ACC»óÅÂ¿¡¼­ LTURN, RTURN»óÅÂ°¡ Áßº¹µÇ¹Ç·Î
	//else if·Î Ã³¸®ÇÏ±âÀ§ÇØ¼± L,RTURNÀ» ¸ÕÁ® Ã³¸®ÇØ¾ß ÇÑ´Ù.
...
float CMover::GetRideFrameSpeed( void )
{
	...
	if( m_pRide )
	{
		...
		if( m_pActMover->GetTurnState() == OBJSTA_LTURN || m_pActMover->GetTurnState() == OBJSTA_RTURN )
			fFrameSpeed = 1.0f;
		else if( ( m_pActMover->GetStateFlag() & OBJSTAF_ACC ) && ( m_pActMover->GetStateFlag() & OBJSTAF_TURBO ) )// °¡¼Ó »óÅÂÀÌ°í ÅÍº¸ »óÅÂÀÏ ¶§
			fFrameSpeed += 1.2f;
...
void CMover::PetLevelup( void )
{
	...
	if( pPet->GetLevel() == PL_EGG )	// ºÎÈ­
	{
		...
		((FLWSUser*)this)->SetHonorAdd(HS_HATCHING_EGG,HI_COUNT_CHECK);	//sun: 13, ´ÞÀÎ
	}
	...
	( (FLWSUser*)this )->AddPet( pPet, PF_PET_LEVEL_UP );	// í»
	g_xWSUserManager->AddPetLevelup( this, MAKELONG( (WORD)pPet->GetIndex(), (WORD)pPet->GetLevel() ) );	// öâ
#endif	// __CLIENT
}
...
void CMover::ProcessPetEnergy( void )
{
		...
		)
	{
		...
		for( DWORD i = 0 ; i < dwMax; i++ )
		{
			...
			if( IsUsableItem( ptr ) && ptr->m_dwItemId == ITEM_INDEX( 21037, II_SYS_SYS_FEED_01 ) )	// »ç¿ë °¡´ÉÇÑ ¸ÔÀÌ¸é
			{
...
{	// »ç¿ëÁßÀÎ ¾ÆÀÌÅÛÀÎ°¡?
	...
	if ( pItemElem->m_dwObjId == m_objIdVisPet )	// 2018.02.27 À¯ÇÁ·¹½Ã¾Æ °­¿¡¼­ ¹öÇÁÆê ¼ÒÈ¯ ÈÄ »ç¶óÁö°Ô ÇÏ°í »ç¶óÁø ¹öÇÁÆêÀ» °¡¹æÀÌ³ª ±³È¯À» ÅëÇØ ÀÌµ¿ÇÏ°í ´Ù¸¥ Æê ¼ÒÈ¯ÈÄ ´Ù½Ã ¼ÒÈ¯ÇÏ¸é ÁßÃ¸¿À·ù ¹ß»ý
		return TRUE;
	//sun: 9, 9-10Â÷ Æê
...
#if defined __WORLDSERVER && defined __ENCHANT_BARUNA16	
	if( GetBarunaNPCSummoner().GetNPCItemOBJID() == pItemElem->m_dwObjId )		//@Á¦³Ê¸¯ÇÏ°Ô »©¾ßÇÒµí.
		return TRUE;
#endif //__ENCHANT_BARUNA16
	return FALSE;
}
...
//ÀÌ¸§±ÔÄ¢ : ¹«¹öÀÇ UV Texture animationÀ» À§ÇÑ ¹«¹öÀÌ¸§ ³¡¿¡ ºÙ´Â ÀÌ¸§±ÔÄ¢ÀÌ´Ù.
...
void CMover::SetAngle( FLOAT fAngle )
{
	...
	if( pProp )
	{
		...
		{
			if( pProp->nBodyState & 1 )		//¹æÇâÀüÈ¯ ±ÝÁö
				return; 
		}
	}
...
bool	CMover::CheckAttackDistance( CObj* pTarget, const DWORD AttackRangeType )
{
	/************************************************************************/
	/*
	°ø°Ý °Å¸® °Ë»ç. this - Attacker, pTarget - Defender
	*/
	/************************************************************************/
#ifdef __WORLDSERVER
	if( IsPlayer() == FALSE )
...
void	CMover::DecreaseExperience( const bool Transfer, const bool Resurrection )
{
	...
	if( Transfer == true ) {
		if( downLevel == true ) {
			g_xWSUserManager->AddSetLevel( this, static_cast<WORD>( m_nLevel ) );						// thisÀÌ¿ÜÀÇ ÁÖº¯¿¡ ·¹º§º¯°æÁ¤º¸¸¦ º¸³¿.
			static_cast<FLWSUser*>( this )->AddSetGrowthLearningPoint( m_Stat.GetRemainStatPoint() );	// ÇØ´çÀ¯Àú¿¡°Ô GPº¯°æÁ¤º¸¸¦ º¸³¿.
			g_dpDBClient.SendUpdatePlayerData( static_cast<FLWSUser*>( this ) );						// ÇÃ·¹ÀÌ¾î µ¥ÀÌÅÍ °»½Å
			g_pMadrigalGift->OnLevel( static_cast<FLWSUser*>( this ) );
		}

		static_cast<FLWSUser*>( this )->AddSetExperience( GetExp1(), static_cast<WORD>( m_nLevel ),  m_nSkillPoint, m_nSkillLevel, m_nDeathExp, static_cast<WORD>( m_nDeathLevel ) );		// ÇØ´çÀ¯Àú¿¡°Ô exp1,exp2º¯°æµÈ Á¤º¸¸¦ º¸³¿.
	}
}
