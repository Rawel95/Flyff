...
void CGuild::Clear( void )
{
	...
	for( std::map<u_long, CGuildMember*>::iterator i	= m_mapPMember.begin(); i != m_mapPMember.end(); ++i )
	{
		pMember	= i->second;
		SAFE_DELETE( pMember );
	}
	...
	for (std::list <CGuildVote*>::iterator it = m_votes.begin() ;it != m_votes.end(); ++it )
		SAFE_DELETE( *it );
	m_votes.clear();
}
...
BOOL CGuild::AddMember( CGuildMember* pMember )
{
	...
	m_mapPMember.insert( std::map<u_long, CGuildMember*>::value_type( pMember->m_idPlayer, pMember ) );
	return TRUE;	//
}
	...
	else
	{
		...
		if( !bDesc )
		{
			...
			for( i = 0; i < nSize; i++ )
			{
				...
				bool bResult = m_mapPMember.insert( std::map<u_long, CGuildMember*>::value_type( pMember->m_idPlayer, pMember ) ).second;
				if( bResult == false )
				{
					SAFE_DELETE( pMember );
				}
...
{
	std::map<u_long, CGuildMember*>::iterator i	= m_mapPMember.find( idPlayer );
...
{
	int cb	= 0;
	for( std::map<u_long, CGuildMember*>::iterator i	= m_mapPMember.begin();
...
{
	std::list <CGuildVote*>::iterator it = m_votes.begin();
...
//	lstrcpy( m_szNotice, source.m_szNotice );
	for( std::map<u_long, CGuildMember*>::iterator i = source.m_mapPMember.begin(); i != source.m_mapPMember.end(); ++ i )
	{
		CGuildMember* pMember	= new CGuildMember;
		*pMember	= *( i->second );
...
void CGuildMng::Clear( void )
{
	...
	for( std::map<u_long, CGuild*>::iterator i = m_mapPGuild.begin(); i != m_mapPGuild.end(); ++i )
...
BOOL CGuildMng::SetName( CGuild* pGuild, const char* szName )
{
	...
	if( lstrlen( pGuild->m_szGuild ) > 0 )
		m_mapPGuild2.insert( std::map<std::string, CGuild*>::value_type( pGuild->m_szGuild, pGuild ) );	// insert
	return TRUE;
}
...
//	locked
	...
	bool bResult = m_mapPGuild.insert( std::map<u_long, CGuild*>::value_type( m_id, pGuild ) ).second;
	if( bResult == false )
	{
	...
	if( lstrlen( pGuild->m_szGuild ) > 0 )
	{
		m_mapPGuild2.insert( std::map<std::string, CGuild*>::value_type( pGuild->m_szGuild, pGuild ) );
	}
	return m_id;
}
...
{
	std::map<u_long, CGuild*>::iterator i	= m_mapPGuild.find( idGuild );
...
{
	std::map<std::string, CGuild*>::iterator i	= m_mapPGuild2.find( szGuild );
...
void CGuildMng::Serialize( CAr & ar, BOOL bDesc )
{
	...
	else
	{
		...
		for( int i = 0; i < nSize; i++ )
		{
			...
			bool bResult = m_mapPGuild.insert( std::map<u_long, CGuild*>::value_type( pGuild->m_idGuild, pGuild ) ).second;
			if( bResult == false )
			{
				SAFE_DELETE( pGuild );
			}
