...
{
	m_texPack_ex[0].LoadScript( pd3dDevice, MakePath( DIR_THEME,g_xFlyffConfig->GetMainLanguage(), _T( "texDialogBox.inc" ) ) );
	m_texPack_ex[1].LoadScript( pd3dDevice, MakePath( DIR_THEME,g_xFlyffConfig->GetMainLanguage(), _T( "texDialogBoxParty.inc" ) ) );
	m_texPack_ex[2].LoadScript( pd3dDevice, MakePath( DIR_THEME,g_xFlyffConfig->GetMainLanguage(), _T( "texDialogBoxGuild.inc" ) ) );
	
	m_texEmoticon.LoadScript( pd3dDevice, MakePath( DIR_THEME,g_xFlyffConfig->GetMainLanguage(), _T( "texDialogEmoticon.inc" ) ) );


	m_texEmoticonUser.LoadScript( pd3dDevice, MakePath( DIR_THEME, g_xFlyffConfig->GetMainLanguage(),_T( "texChatEmoticon.inc" ) ) );
	LoadEmotion();

	m_pTex[0] = CWndBase::m_textureMng.AddTexture( g_Neuz.m_pd3dDevice, MakePath( DIR_THEME, g_xFlyffConfig->GetMainLanguage(),"texDialogBox_left.tga" ), 0xffff00ff );
	m_pTex[1] = CWndBase::m_textureMng.AddTexture( g_Neuz.m_pd3dDevice, MakePath( DIR_THEME,g_xFlyffConfig->GetMainLanguage(), "texDialogBox_center.TGA" ), 0xffff00ff );
	m_pTex[2] = CWndBase::m_textureMng.AddTexture( g_Neuz.m_pd3dDevice, MakePath( DIR_THEME,g_xFlyffConfig->GetMainLanguage(), "texDialogBox_right.tga" ), 0xffff00ff );
		...
		else
		{
			...
			if( pObj->IsCull() == FALSE )
			{
				...
				if( lpCustomText->m_nKind == CHAT_SHOUT )
				{
					...
					if( nlen >= 1 && nlen <= 1 )
						FLStrcpy( buffer, _countof( buffer ), "texDialogBoxShout00.bmp" );
					else
						if( nlen >= 2 && nlen <= 3 )
							FLStrcpy( buffer, _countof( buffer ), "texDialogBoxShout01.bmp" );
						else
							if( nlen >= 4 && nlen <= 5 )
								FLStrcpy( buffer, _countof( buffer ), "texDialogBoxShout02.bmp" );
							else
								if( nlen >= 6 && nlen <= 9 )
									FLStrcpy( buffer, _countof( buffer ), "texDialogBoxShout03.bmp" );
								else
									if( nlen >= 10 && nlen <= 20 )
										FLStrcpy( buffer, _countof( buffer ), "texDialogBoxShout04.bmp" );
									else
										if( nlen >= 21 && nlen <= 40 )
											FLStrcpy( buffer, _countof( buffer ), "texDialogBoxShout05.bmp" );
										else
											if( nlen >= 41 && nlen <= 60 )
												FLStrcpy( buffer, _countof( buffer ), "texDialogBoxShout06.bmp" );
											else
												if( nlen >= 61 )
													FLStrcpy( buffer, _countof( buffer ), "texDialogBoxShout07.bmp" );

					pShoutTex = CWndBase::m_textureMng.AddTexture( g_Neuz.m_pd3dDevice, MakePath( DIR_THEME, g_xFlyffConfig->GetMainLanguage(),buffer ), 0xffff00ff );
					if( pShoutTex )
					{
				...
				for( i=0; i<3; i++ )
				{
					...
					for( int j=0; j<3; j++ )
					{
						...
						//gmpbigsun(100607) : Error log and check array range
						if( lpCustomText->m_nKind < 0 || lpCustomText->m_nKind > 2 )
						{
							FLERROR_LOG( PROGRAM_NAME, _T( "lpCustomText->m_nKind is %d" ), lpCustomText->m_nKind );
							break;
						}

						pTexture = m_texPack_ex[lpCustomText->m_nKind].GetAt( nIndex );

						//gmpbigsun(100607) : Error log and check pointer
						if( NULL == pTexture )
						{
							FLERROR_LOG( PROGRAM_NAME, _T( "pTexture is NULL, lpCustomText->m_nKind %d, nIndex %d" ), lpCustomText->m_nKind, nIndex );
							break;
						}
...
{
	if( nEmoticonIdx >= 0 && nEmoticonIdx < (int)( m_texEmoticonUser.GetNumber() ) )
		AddTexture( pObj, m_texEmoticonUser.GetAt( nEmoticonIdx ) );
}

void CDialogMsg::AddEmoticon( CObj* pObj, int nEmoticonIdx )
{
	if( nEmoticonIdx >= 0 && nEmoticonIdx < (int)( m_texEmoticon.GetNumber() ) )
...
bool CDialogMsg::LoadEmotion()
{
	...
	memset( m_EmiticonCmd, 0, sizeof(m_EmiticonCmd) );
	...
	while( script.tok != FINISHED )
	{
		...
		{
			FLERROR_LOG( PROGRAM_NAME, _T( "textEmotion.txt 갯수가 맞지 않음" ) );
			return FALSE;
		}

		m_EmiticonCmd[ i ].m_dwIndex = atoi( script.token );
		script.GetToken();
		FLStrcpy( m_EmiticonCmd[ i ].m_szCommand, _countof( m_EmiticonCmd[ i ].m_szCommand ), script.token );
