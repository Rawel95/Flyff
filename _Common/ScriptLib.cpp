...
int GetGValue_( CScript *pScript )
{
	...
	if( v < 0 || v > 15 )
	{
		FLERROR_LOG( PROGRAM_NAME, _T( "GetGValue_에서 %d(0~15) 값 이상" ), v );
		v = 0;
	}
...
int SetGValue( CScript *pScript )
{
	...
	if( v1 < 0 || v1 > 15 )
	{
		FLERROR_LOG( PROGRAM_NAME, _T( "SetGValue에서 %d(0~15) 값 이상" ), v1 );
		v1 = 0;
	}
...
int AddGValue( CScript *pScript )
{
	...
	if( v1 < 0 || v1 > 15 )
	{
		FLERROR_LOG( PROGRAM_NAME, _T( "AddGValue에서 %d(0~15) 값 이상" ), v1 );
		v1 = 0;
	}
	pScript->m_panGlobalVal[ v1 ] += v2;
	return TRUE;
...
int GetParam( CScript* pScript )
{
	...
	{
		v1 = 0;
		FLERROR_LOG( PROGRAM_NAME, _T( "GetParam에서 Idx 값 범위 초과(0~3)" ) ); 
	}
...
int SetTimer( CScript* pScript )  
{
	...
	{
		FLERROR_LOG( PROGRAM_NAME, _T( "Script : SetTimer에서 타이머 ID Max 초과 ( %d / %d )" ), nTimerId, MAX_SCRIPT_TIMER  );
		nTimerId = 0 ;
	}
...
int IsTimeOut( CScript* pScript )
{
	...
	if( nTimerId >= MAX_SCRIPT_TIMER )
	{
		FLERROR_LOG( PROGRAM_NAME, _T( "Script : IsTimeOut 타이머 ID Max 초과 ( %d / %d )" ), nTimerId, MAX_SCRIPT_TIMER  );
		nTimerId = 0 ;
	}
...
#ifdef SCRIPT_ENTER_INSTANCEDUNGEON
	"EnterInstanceDungeon",	EnterInstanceDungeon,			//인스턴스 던전에 입장한다.(파티)
