...
#define COLOR_WORLDBOSS			0xffa48ae6
...
#ifdef __ZCK_WORLD_BOSS
	bool				m_bWorldBoss;
	std::map<u_long, __int64> m_mapAttackersDamage;
	void AddDamageByAttacker( CMover* pMoverAttacker, __int64 n64Damage );
#endif // __ZCK_WORLD_BOSS
...
//sun: 13, 달인
	...
	CSfx*			m_pAilesSfxZacks;
	DWORD			m_dwAilesSfxZacksSfxId;

	void			UpdateAilesSfxZacks( DWORD dwSfxId );
	void			ClearAilesSfxZacks();
	void			ProcessAilesSfxZacks();
	void			RenderAilesSfxZacks( LPDIRECT3DDEVICE9 pd3dDevice, CModelObject* pModelObject, const D3DXMATRIX* pWorld, BOOL bPreserveDeviceState = FALSE );
#endif // __ZCK_AILES_SFX
#if defined(__ZCK_SFX_AMELIORER)
	std::vector<CSfx*> m_vecZckManagedSfx;
	int			m_nZckManagedSfxLevel;
	int			m_nZckManagedSfxEffectValue;
...
#if defined(__ZCK_SFX_AMELIORER)
	BOOL			RefreshZckSfxEquipInfo( FLItemElem* pItemElem );
#endif // __ZCK_SFX_AMELIORER
...
#if defined(__ZCK_SFX_AMELIORER)
	void			AddZckManagedSetSfx( DWORD dwSfxId, int nPartsLink, FLOAT fOffsetY, const D3DXVECTOR3& vScale );
	void			CreateZckManagedSfx();
	void			DeleteZckManagedSfx();
	void			ProcessZckManagedSfx();
	void			RefreshZckManagedSfx();
	BOOL			HasZckManagedSfx() const { return m_vecZckManagedSfx.empty() ? FALSE : TRUE; }
	void			RenderZckSfxViewport( LPDIRECT3DDEVICE9 pd3dDevice, CModelObject* pModel, const D3DXMATRIX* pWorld );
	void			RenderZckWeaponElementSfxViewport( LPDIRECT3DDEVICE9 pd3dDevice, CModelObject* pModel, const D3DXMATRIX* pWorld );
#endif // __ZCK_SFX_AMELIORER
