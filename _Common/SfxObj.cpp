...
CSfx::~CSfx()
{
	...
	if( m_idSfxHit )
	{
		...
		{
			FLERROR_LOG( PROGRAM_NAME, _T( "%d %d %d" ), m_dwIndex, m_idSfxHit, m_idSrc );
		} 
	#ifdef __CLIENT
		else
				   ...
				   const D3DXVECTOR3& vPosSrc, OBJID idSrc, const D3DXVECTOR3& vPosDest, OBJID idDest, int nSec )
{
	...
	D3DXVECTOR3 vNewPos = vPosSrc;
	vNewPos.y += 0.01f; // 지면에서 약간 띄워주자. 안그러면 밑에 뭍혀서 지글거림.
	SetPos( vNewPos );
...
{
#ifdef _DEBUG
	if( m_dwIndex == XI_INDEX( 265, XI_SKILL_MAG_FIRE_HOTAIR01 ) )	// 디버깅 하려면 이걸 바꿔 쓰세요.
	{
		int a = 0;
	}
#endif
	...
	if( m_idDest != NULL_ID )		// Dest가 지정되어 있을때.
	{
		...
		if( IsValidObj( pObjDest ) )		// 유효한넘인가?
		{
			...
			//gmpbigsun( 20101229 ) : 일반 sfx 스케일 조정( 모델크기에 맞게, 최대 5배 ), 20110116 back
			//float fR = pObjDest->GetRadius( );
			//	if( fR > 0.1f && fR < 5.0f )
			//	SetScale( D3DXVECTOR3( fR, fR, fR ) );
			//else if( fR >= 5.0f )
