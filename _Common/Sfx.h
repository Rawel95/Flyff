...
//	DWORD	m_dwSkillLevel;	// 스킬레벨.
	...
	virtual	int SetSfx( LPDIRECT3DDEVICE9 pd3dDevice, int nIndex, const D3DXVECTOR3& vPosSrc, OBJID idSrc, 
		const D3DXVECTOR3& vPosDest, OBJID idDest, int nSec = 0 ); // 사용할 SFX 지정
...
#ifdef __CLIENT
public:
	MEMPOOLER_DECLARE( CSfxGenNormalDmg01 );
#endif	// __CLIENT
};
...
#ifdef __CLIENT
public:
	MEMPOOLER_DECLARE( CSfxGenMoveMark );
#endif	// __CLIENT
	int nRemove;
};
// 일반 이펙트 : 물의 파장
...
#ifdef __CLIENT
public:
	MEMPOOLER_DECLARE( CSfxGenWaterCircle );
#endif	// __CLIENT
};
...
#ifdef __CLIENT
public:
	MEMPOOLER_DECLARE( CSfxGenRainCircle );
#endif	// __CLIENT
};
...
public:
	...
	int				m_nOldEffectValue;
	D3DXVECTOR3		m_v3Offset;
#if defined(__ZCK_SFX_AMELIORER) && defined(__CLIENT)
	CMover*			m_pZckSfxMover;
	void			SetZckSfxMover( CMover* pMover ) { m_pZckSfxMover = pMover; }
#endif // __ZCK_SFX_AMELIORER && __CLIENT
...
public:
	...
	//gmpbigsun : 무버에 붙어 다니는 일반적인 상황연출용
	CSfxHitParts( const HIT_PARTS eParts  = HIT_BODY, BOOL bBuff = FALSE );
	virtual ~CSfxHitParts( );
	virtual void Process( );
	virtual void Render( LPDIRECT3DDEVICE9 pd3dDevice );
...
public:
	...
	//gmpbigsun : 무버에 붙어 다니는 일반적인 상황연출용
	CSfxAttackParts( const HIT_PARTS eParts  = HIT_BODY );
	virtual ~CSfxAttackParts( );
	virtual void Process( );
	virtual void Render( LPDIRECT3DDEVICE9 pd3dDevice );
...
//통합형 for lua
class CSfxUnion : public CCtrl //public CSfx	
{
public:
	enum N_PATTERN
	{
		PATTERN_FIXED,			//변하지 않는 고정위치, bone이 유효하다면 bone의 위치에서 아니면 master의 위치
		PATTERN_FIXED_A,		//공격방향으로 1m가량 떨어진곳, master의 위치로부터
		PATTERN_LINKED,			//연결되서 따라다니는
		PATTERN_ANGLED,			//연결되서 각도까지 따라다니는
		PATEERN_PROJECTILE, 	//발사체
	};

public:
	//time : flyff구조상 들어감( 유지시간 ), _angle : 초기각도
	CSfxUnion( ) { }
	CSfxUnion( const OBJID _master, const char* _sfx, const char* _bone = "", N_PATTERN _pattern = PATTERN_FIXED, 
		const float _scale = 1.0f, int _sec = 0, float _angle = 0.0f );
...
protected:
	BOOL UpdateByPattern_FIXED( );
	BOOL UpdateByPattern_FIXED_A( );
	BOOL UpdateByPattern_LINKED( );
	BOOL UpdateByPattern_ANGLED( );
	BOOL UpdateByPattern_PROJECTILE( );
