...
BOOL CLandscape::LoadLandscape( LPCTSTR lpszFileName, int xx, int yy )
{
	...
	for(int j = 0; j < nLayer; j++ )  
	{
		...
		//gmpbigsun( 20100913 ) : 4버젼은 이상은 중복레이어를 허용한다.
		if( m_dwVersion >= 4 )
		{
			FLERROR_LOG( PROGRAM_NAME, _T( "m_dwVersion >= 4 " ) );	//아직 지원안함
...
//	CalculateBound();
	...
	FLTRACE_LOG( PROGRAM_NAME, _T( "Load Object Num = %d" ), dwObjNum );
	for( int i = 0; i < (int)( dwObjNum ); i++ )
	{
		...
		{
			if( g_xFlyffConfig->GetMainLanguage() == LANG_JAP && pObj->m_dwIndex == 360 )
			{
				SAFE_DELETE( pObj );
				dwObjNum--;
			}
...
BOOL CLandscape::InsertObjLink( CObj* pObj )
{
	...
	if( nPos >= 0 && nPos < nMaxWidth * nMaxWidth )
	{
		...
		if( pBegObj )
		{
			...
			{
				FLTRACE_LOG( PROGRAM_NAME, _T( "헉 InsertObjWorld 새로 추가된 오브젝트가 Next와 Prev에 값이 있네?? Type = %d" ), pObj->GetType());
			}
	...
	{
		FLTRACE_LOG( PROGRAM_NAME, _T( "헉 InsObjInFld에서 범위를 벗어난게 추가되려고 하네?" ) );
		return FALSE;
	}
	return TRUE;
}
...
CLandLayer* CLandscape::NewLayer( WORD nTex, BOOL bOverlap )
{
	...
	//gmpbigsun( 20100913 ) : 중복레이어가 아닐경우만 기존의 레이어에서 찾는다.
	if( FALSE == bOverlap )
	{
		for(int i = 0; i < m_aLayer.GetSize(); ++i) 
		{
	...
	const int MAX_PATCHES = NUM_PATCHES_PER_SIDE*NUM_PATCHES_PER_SIDE;
	...
	else 
	{
		...
		D3DLOCKED_RECT rectLock;
		pLayer->m_pLightMap->LockRect( 0 , &rectLock, 0, 0);
		const int MAX_MAPSIZE = MAP_SIZE * MAP_SIZE;
