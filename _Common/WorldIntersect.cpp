...
FLOAT CWorld::GetFullHeight( const D3DXVECTOR3& vPos )
{
	...
	FOR_LINKMAP_COLLISION( this, vPos, pObj, nRange, CObj::linkStatic, nDefaultLayer )
	{
...
#if 0		//gmpbigsun: 아래코드는 올바른 방법이지만 퍼포먼스에 악영향을 줌으로  디자이너의 실수를 커버하기엔 비용이 너무 비쌈
			//gmpbigsun( 20110729 ) : 가장 가까운 오브젝트를 찾더래도 지면보다 낮다면 지면높이를 리턴해야함.
			float fLandHeight = GetLandHeight( vPos.x, vPos.z );
...
CObj *CWorld::GetObjByName(TCHAR* ObjName)
{
	...
	if(g_pPlayer)
	{
		...
		FOR_LINKMAP_COLLISION( this, g_pPlayer->m_vPos, pObj, 0, CObj::linkStatic, nDefaultLayer )
		{
...
{
	//gmpbigsun(20101021): SimpleTriIntersect->TriIntersect_BBOX 교체 
	if( ((CModelObject*)pWallObj->m_pModel)->GetObject3D()->TriIntersect_BBOX(pWallObj->GetMatrixWorld(), pTargetObj, pTargetObj->GetMatrixWorld()))
		return TRUE;
...
{
	//suntodo: 문제발견, 수직벡터와 충돌하는 오브젝트가 복수개더라도 제일 마지막에 걸리는 오브젝트만 검사하고있다.
	//gmpbigsun( 20110406 ) : 복수개의 오브젝트를 검출한후 가장 가까운 y좌표를 리턴시킨다.
...
#endif	// __LAYER_1015
{
	...
	*pOut = vDir;
	...
	//충돌은 Dynamic obj중 OT_CTRL만 하고있다.
...
{
#ifndef __CLIENT
	UNREFERENCED_PARAMETER( bSkipTrans ); 	UNREFERENCED_PARAMETER( bWithObject );
#endif
			...
			if( pObj )
			{
				if( pObj->GetType() == OT_OBJ || pObj->GetType() == OT_CTRL )
				{
					if( pObj->IsCull() == FALSE /*&& pObj->GetModel()->m_pModelElem->m_dwDistant <= 1*/ ) 
					{
						...
						{
							FLERROR_LOG( PROGRAM_NAME, _T( "범위를 넘어섬." ) );
							break;
						}
...
#ifdef _DEBUG
	if( b == FALSE )
		FLERROR_LOG( PROGRAM_NAME, _T( "D3DXIntersectTri x:%f z:%f" ), vPos.x, vPos.z );
#endif
	vIntersect.x = vPos.x;
