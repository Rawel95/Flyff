...
class CObj 
#ifdef __WORLDSERVER
	: public FLRing< CObj >::FLNode				//시야 체크용
#endif //__WORLDSERVER
{ 
	friend	class	CLandscape;

	static			size_t					ms_uLinkedObjCount;
...
public:
	...
	int				m_nAddObjsArrayIndex;	//neodeath : Delete때를 위한 상호 참조
	...
	D3DXVECTOR3		m_vOldRotate;			//sun: 9,	//__AI_0509
#endif	// __WORLDSERVER
...
public:
	...
	static void		SetAnimate( BOOL bAnimate )		{ m_bAnimate = bAnimate; }

	virtual	BOOL	SetMotion(DWORD /*dwMotion*/, int nLoop = ANILOOP_LOOP, DWORD dwOption = 0 ) {	UNREFERENCED_PARAMETER( nLoop );	UNREFERENCED_PARAMETER( dwOption );	return FALSE; }
...
public:
	bool			IsCCtrl() const;
	float			GetModelMaxSize() const;
	bool			IsCollitionObj() const;
};
