...
#define XE_MTE_NOSKIN		(0x00400000)		
...
#define		PE_GEN_BR	15	//gmpbigsun: 바루나 일반

enum INCHANT_EFFECT_TYPE	//신제련 이펙트 타입
{
	IET_NONE = 0,
	IET_BARUNA = 1,					//바루나아이템..
	IET_COMPOSE_TWOHAND = 2,		//양손무기합성된...
};
...
#define MAX_PARTS_FIRE_2	16			//sun: 10, __CSC_ENCHANT_EFFECT_2
...
public:
	CPartsFire() 
	{
		...
		memset( m_pList_2, 0, sizeof(m_pList_2) );	//sun: 10, __CSC_ENCHANT_EFFECT_2
	}
...
//sun: 제련 이팩트 향상 버전
	...
	void	Create( LPDIRECT3DDEVICE9 pd3dDevice, const D3DXVECTOR3 vPos, DWORD dwSfx );	//sun: 10, __CSC_ENCHANT_EFFECT_2
...
public:
	...
	//by filename
	BOOL	Create( LPDIRECT3DDEVICE9 pd3dDevice, const D3DXVECTOR3& vPos, TCHAR* filename, const D3DXVECTOR3& vScale );
	//by index
	BOOL	Create( LPDIRECT3DDEVICE9 pd3dDevice, const D3DXVECTOR3& vPos, const DWORD dwSfx, const D3DXVECTOR3& vScale );

	void	Render( LPDIRECT3DDEVICE9 pd3dDevice, const D3DXVECTOR3& kPos, const D3DXMATRIX *mWorld );

	BOOL IsEqual( const int nType, const int nLevel );
...
public:
	...
	{ 
		...
		memset( m_vBeam, 0, sizeof(m_vBeam) );
		memset( m_vList, 0, sizeof(m_vList) );
//		mem_set( m_vCenter, 0, sizeof(m_vCenter) );
	}
	~CPartsLaser() {}
