...
BOOL CTerrainMng::LoadScript( LPCTSTR lpszFileName )
{
	...
	while( nBrace )
	{
		...
		FLStrcpy( lpTerrain->m_szTextureFileName, _countof( lpTerrain->m_szTextureFileName ), scanner.token );
		lpTerrain->m_bBlock = scanner.GetNumber(); // block
		scanner.GetToken(); // sound
		FLStrcpy( lpTerrain->m_szSoundFileName, _countof( lpTerrain->m_szSoundFileName ), scanner.token );
		scanner.SetMark();
