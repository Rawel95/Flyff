...
private:
	...
	std::map<CString, DWORD>			m_mapRebornStaff;
	DWORD						GetRebornStaffAuthValue( const CString& strAuth ) const;
...
//sun: 10, __FILTER_0705
	...
	ZckArmeRareProp				m_aZckArmeRare[ZCK_ARME_RARE_LEVEL_COUNT];
	int							m_nHighZckArmeRareLevel;
...
#ifdef __ZCK_ARME_RARE
	BOOL			LoadZckArmeRare( LPCTSTR lpszFileName );
	const ZckArmeRareProp* GetZckArmeRare( int nLevel ) const;
	BOOL			CheckZckArmeRare( int nLevel ) const;
#endif // __ZCK_ARME_RARE
...
public:
	...
	BOOL	LoadRebornStaff();
	BOOL	TryGetRebornStaffAuthorization( LPCTSTR lpszCharacterName, DWORD& dwAuthorization ) const;
...
//sun: 10, __RULE_0615
	...
	std::map<DWORD, std::vector<DWORD>> m_aTeleportItem;
	void LoadTeleportItem();
	std::vector<DWORD> * GetTeleportItem(DWORD dwWorldId);
#endif
