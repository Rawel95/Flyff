...
FLOAT	CMover::GetAttackRange( DWORD dwAttackRange )
{
	...
	case AR_HWAND:	fAttRange = 18.0f;		break;	//sun: 10차 전승시스템	Neuz, World, Trans
...
int	CMover::CMD_SetUseSkill( OBJID idTarget, int nSkillIdx, SKILLUSETYPE sutType )
{
	...
	case WUI_TARGETCURSORPTZ:
		{
			...
			if( GetWorld()->ClientPointToVector( NULL, rect, pWndWorld->GetMousePoint(), &GetWorld()->m_matProj, &GetWorld()->GetCamera()->m_matView, &vPos, TRUE ) )
			{
				...
				{
					FLERROR_LOG( PROGRAM_NAME, _T( "%s. add스킬(%d)의 프로퍼티가 없다." ), m_szName, nSkillIdx );
	...
	if( idTarget != GetId() )		
	{
		...
		{
			if( pSkillProp->IsDebuff() == TRUE )	// 나쁜 스킬은 
				if( IsAttackAble(pTarget) == FALSE )	// 공격허용이 되지 않으면 사용할 수 없음.
					return 0;
				
			if( pSkill->dwSkill == SKILL_INDEX( 45, SI_ASS_HEAL_RESURRECTION ) )	// 부활을 사용했을때
			{
	...
	if( bSetDestObj ) {
#ifdef __WORLDSERVER
		SetDestObj( idTarget, fArrivalRange, FALSE );		// 이동할 목표물을 idTarget으로 설정.
...
void	CMover::CMD_SetMeleeAttack( OBJID idTarget, FLOAT fRange )
{
	...
	if( GetDestId() == idTarget )	// 중복 패킷 전송 막기
		return;

	SetDestObj( idTarget, fRange, TRUE );		// 이동할 목표물을 idTarget으로 설정.
...
void	CMover::CMD_SetRangeAttack( OBJID idTarget, int nPower )
{
	...
	if( pItemProp->dwItemKind3 != IK3_BOW )	// 들고있는 무기가 보우가 아니면 에러
		...
		if( pItemProp->dwItemKind3 != IK3_CROSSBOW )	// 크로수 보우도 아니면 에러 
...
void	CMover::SetActParam( OBJACT act, int nParam1, int nParam2, int nParam3, int nParam4, int nParam5 )
{
	...
	m_nAParam[1] = nParam2;
	m_nAParam[2] = nParam3;
	m_nAParam[3] = nParam4;		// 스킬이 아닌경우 공격 판정( dwAtkFlag(AF_GENERIC, AF_MAGIC, AF_MISS, ... ) : CActionMover::m_dwAtkFlags 대체
	m_nAParam[4] = nParam5;
}
