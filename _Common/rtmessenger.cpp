...
void	CRTMessenger::SetFriend( u_long idFriend, Friend * pFriend )
{
	...
	if( !pFriend )
		pFriend		= &f;
	std::map<u_long, Friend>::iterator i	= find( idFriend );
	if( i != end() )
		memcpy( &i->second, pFriend, sizeof(Friend) );
	else
		insert( std::map<u_long, Friend>::value_type( idFriend, *pFriend ) );
}
...
int CRTMessenger::Serialize( CAr & ar )
{
	if( ar.IsStoring() )
	{
		...
		for( std::map<u_long, Friend>::iterator i = begin(); i != end(); ++i )
		{
			ar << i->first;
...
{
	clear();
	for( std::map<u_long, Friend>::iterator i = rRTMessenger.begin(); i != rRTMessenger.end(); ++i )
