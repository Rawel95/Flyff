...
#endif // __WORLDSERVER
		std::vector<HOUSINGINFO>::iterator it = m_vecHousingInfo.begin();
		it += nIndex;
...
void CHousing::SetVisitAllow( DWORD dwTargetId, BOOL bAllow )
{
	...
	{
		for( std::vector<DWORD>::iterator it=m_vecIdVisitAllow.begin(); it!=m_vecIdVisitAllow.end(); it++ )
		{
			if( (*it) == dwTargetId )
			{
...
void CHousingMng::SetRemoveVisitable( DWORD dwPlayerId, DWORD dwTargetId )
{
	...
	if( it != m_mapVisitable.end() )
	{
		for( std::vector<DWORD>::iterator it2=it->second.begin(); it2!=it->second.end(); it2++ )
		{
			if( (*it2) == dwPlayerId )
			{
				it->second.erase( it2 );
			...
			case NORMAL_MODE:
				{
					...
					for(int i = 0; i < m_nNumTex; ++i)
					{
						std::vector<LPDIRECT3DTEXTURE9>::iterator	iter = 	m_vecOriginal.begin();
