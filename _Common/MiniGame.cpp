...
//	if( pUser->GetGold() < m_nPayPenya )
	...
	for( int i=0; i<nCount; ++i )
	{
		...
		std::map<OBJID, int>::iterator it = map_Item.find( objItemId[i] );
		if( it == map_Item.end() )
...
int CMiniGame::Result_FiveSystem( FLWSUser* pUser )
{
	...
	std::map<int, __FIVESYSTEM>::iterator it = m_mapFiveSystem.find( nMaxCount );
...
CMiniGame::__KAWIBAWIBO CMiniGame::FindKawibawibo( int nWinCount )
{
	...
	if( 0 <= nUserWin )
	{
		...
		std::map<int, __KAWIBAWIBO>::iterator it = m_mapKawiBawiBo.find( nUserWin );
...
BOOL CMiniGame::Load_Alphabet()
{
	...
	while( s.tok != FINISHED )
	{
		...
		mapQuestion.insert(std::make_pair( nID, strQuestion ) );
		s.GetToken();
	}
	...
	while( s.tok != FINISHED )
	{
		...
		else if( s.Token == _T( "WORD" ) )
		{
			...
			s.GetLastFull();
			std::map< int, CString >::iterator it = mapQuestion.find( atoi( s.Token.Right(4) ) );
...
BOOL CMiniGame::Load_KawiBawiBo()
{
	...
	while( s.tok != FINISHED )
	{
		if( s.Token == _T( "SENDITEM" ))
		{
			...
			m_mapKawiBawiBo.insert(std::make_pair( nWinCount, KawiBawiBo) );
//			m_mapKawiBawiBo.insert( std::map<int, __KAWIBAWIBO>::value_type( nWinCount, KawiBawiBo ) );
...
BOOL CMiniGame::Load_FiveSystem()
{
	...
	while( s.tok != FINISHED )
	{
		...
		else if( s.Token == _T( "PERCENT" ) )
		{
			...
			m_mapFiveSystem.insert( std::map<int, __FIVESYSTEM>::value_type( nCount, FiveSystem ) );
		}
