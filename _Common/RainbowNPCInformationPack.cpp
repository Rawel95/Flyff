...
BOOL CRainbowNPCInformationPack::LoadScript( const CString& strFileName )
{
	...
	{
		FLERROR_LOG( PROGRAM_NAME, _T( "%s not found" ), strFileName );
		return FALSE;
	}
...
// 				LoadTextureFromRes( 
// 					D3DDEVICE, 
// 					MakePath( _T( DIR_THEME ), g_xFlyffConfig->GetMainLanguage(), strTexturePackFileName ), 
// 					D3DX_DEFAULT, 
// 					D3DX_DEFAULT, 
// 					D3DX_DEFAULT, 
// 					0, 
...
// 			{
				LoadTextureFromRes( 
					D3DDEVICE, 
					MakePath( DIR_THEME, g_xFlyffConfig->GetMainLanguage(),strTexturePackFileName ), 
					D3DX_DEFAULT, 
					D3DX_DEFAULT, 
					D3DX_DEFAULT, 
					0, 
...
// 			}
		...
		else if( scanner.Token == _T( "serialize" ) )
		{
			...
			{
				FLERROR_LOG( PROGRAM_NAME, _T( "%s 에러, 할당 :%d, 실제갯수 : %d" ), strFileName, m_dwNumber, dwCount );
				return FALSE;
			}
			int nCount = 0;
