...
BOOL CPCBang::LoadScript()
{
	...
	const TCHAR szFileName[]	= _T( "PCBang.lua" );
...
void CPCBang::SetPCBangPlayer( FLWSUser* pUser, DWORD dwPCBangClass )
{
	if( dwPCBangClass == 0 )
	{
	...
	if( IsApply() )
		pUser->AddBuff( BUFF_EQUIP, ITEM_INDEX( 26790, II_PCBANG_BUFF01 ), 1, 999999999, 0, BT_TICK );	// TODO
}
...
void CPCBang::SetApply( BOOL bApply )
{
	...
	else
	{
		for( MAPPBI::iterator it=m_mapPCBang.begin(); it!=m_mapPCBang.end(); it++ )
		{
			FLWSUser* pUser = static_cast<FLWSUser*>( prj.GetUserByID( it->first ) );
			if( IsValidObj( pUser ) )
				pUser->RemoveBuff( BUFF_EQUIP, ITEM_INDEX( 26790, II_PCBANG_BUFF01 ) );
		}
	}
}
...
float CPCBang::GetExpInfo( DWORD dwHour )
{
	...
	if( nMaxIndex >= 0 )
	{
		if( (int)( dwHour ) > nMaxIndex )
			return m_vecfExp.at( nMaxIndex );
		else
			return m_vecfExp.at( dwHour );
	}

	return 1.0f;
}

float CPCBang::GetExpFactor( FLWSUser* pUser )
{
	if( !IsApply() || !IsValidObj( pUser ) || !pUser->m_buffs.HasBuff( BUFF_EQUIP, ITEM_INDEX( 26790, II_PCBANG_BUFF01 ) ) )
		return 1.0f;
...
float CPCBang::GetPartyExpFactor( FLWSUser* apUser[], int nMemberSize )
{
...
float CPCBang::GetPieceItemDropInfo( DWORD dwHour )
{
	...
	{
		if( (int)( dwHour ) > nMaxIndex )
			return m_vecfDropRate.at( nMaxIndex );
		else
			return m_vecfDropRate.at( dwHour );
	}

	return 1.0f;
}

float CPCBang::GetPieceItemDropFactor( FLWSUser* pUser )
{	
	if( !IsApply() || !IsValidObj( pUser ) || !pUser->m_buffs.HasBuff( BUFF_EQUIP, ITEM_INDEX( 26790, II_PCBANG_BUFF01 ) ) )
		return 1.0f;
