...
BOOL CSfxModelMng::AddData( SfxModelSet* pData, BOOL bChecked )
{
	...
	else
	{
		...
		std::pair< SfxModelSetIter, bool > rst =  _cDatas.insert( SfxModelSetContainer::value_type( pData->_idMaster, cSMS ) );
...
void CSfxModelMng::Update( )
{
	...
	{
		for( std::map<DWORD,int>::iterator iter = _cWaitingObj.begin(); iter != _cWaitingObj.end(); ++iter )
		{
...
void run_lua_sfx( int nState, OBJID caller, const char* szMoverName )
{
	...
	if( !pWho )
	{
		...
		CSfxModelMng::GetThis()->_cWaitingObj.insert( std::map< DWORD, int >::value_type( caller, nState ) );
		return;
	}
