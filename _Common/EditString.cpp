...
void CEditString::Init( CD3DFont* pFont, CRect* pRect )
{
	...
	if( m_nWidth != pRect->Width() ||
		m_pFont != pFont )	// 사이즈와 폰트 둘중에 하나라도 기존것과 다르다면 정렬을 해주어야한다.
		bAlign = TRUE;
...
#ifdef __HYPERLINK_ITEM16
			//gmpbigsun: 아래코드는 뭘 할려고 한걸까?? 하이퍼 스타일이 다르고 칼라가 다르면 무조건 아이템링크인가?
			...
			case 'i':
				{
					...
					if( dwPStyle & PS_USE_MACRO )
					{
						//gmpbigsun_WKG : 여기서 링크 아이템정보를 분석해서 Data로 남겨야 한다.
						//masterid, itemOid, linenum, seq = 0( 한줄당 한개만 유효하다고 가정한다 )
						...
						if( dwPStyle & PS_REALADDING_CHATWND )
						{
							...
							DWORD key = ( tmp << 16 ) | 0; // 한라인당 한개만 일단 ( 라인번호 | 순서 )
...
void CEditString::Align( CD3DFont* pFont, int nBeginLine )
{
	...
	// g_xFlyffConfig->GetMainLanguage()를 constructor에서 호출 할 수 없기에 여기서 한다.
