...
void IElection::sProperty::Serialize( CAr & ar )
{
	if( ar.IsStoring() )
	{
		...
		for( std::vector<float>::iterator i1 = m_vReturnDepositRates.begin(); i1 != m_vReturnDepositRates.end(); ++i1 )
			ar << *( i1 );
		for( int iIndex = 0; iIndex < 2; iIndex++ )
		{
			ar << m_vItems[iIndex].size();
			for( std::vector<int>::iterator i2 = m_vItems[iIndex].begin(); i2 != m_vItems[iIndex].end(); ++i2 )
				ar << *( i2 );
		}
	}
	else
...
{
	std::sort(m_vCandidates.begin(), m_vCandidates.end(), [](const SPC& lhs, const SPC& rhs)
		{
			return (lhs->GetDeposit() == rhs->GetDeposit() ? lhs->GetCreate() < rhs->GetCreate() : lhs->GetDeposit() > rhs->GetDeposit());
		});
}
...
void IElection::SortVote(void)
{
	std::sort(m_vCandidates.begin(), m_vCandidates.end(), [](const SPC& lhs, const SPC& rhs)
		{
			return (lhs->GetVote() == rhs->GetVote() ? lhs->GetCreate() < rhs->GetCreate() : lhs->GetVote() > rhs->GetVote());
		});
}
	...
	while( s.tok != FINISHED )
	{
		if( s.Token == _T( "exp" ) )
		{
			...
			while( *s.token != '}' )
			{
				...
				m_vEFactor.push_back(std::make_pair( fRate, iCost ) );
				fRate	= s.GetFloat();
			}
		}
		else if( s.Token == _T( "item" ) )
		{
			...
			{
				__int64 iCost	= s.GetInt64();
				m_vIFactor.push_back(std::make_pair( fRate, iCost ) );
				fRate	= s.GetFloat();
			}
		}
		s.GetToken();
