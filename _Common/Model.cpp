...
// 		m_fFrameOld -= (float)m_nFrameMax;
	...
	int nCurrFrame = static_cast<int>( m_fFrameCurrent );				// 소숫점 떼내고 정수부만..

	m_fSlp = m_fFrameCurrent - static_cast<float>( nCurrFrame );	// 소숫점 부분만 가져옴
...
// 	}
	...
	m_fFrameCurrent = static_cast<float>( nCurrFrame ) + m_fSlp;	// 편의상 시각적으로 보기 쉽도록 수치 보정
}
