...
#include "CreateObj.h"
	...
	#include "..\_AIInterface\aimeteonyker.h"	//sun: __METEONYKER_0608

	
#ifdef __AGGRO16
	#include "..\_AIInterface\AIAggroNormal.h"	
	#include "../_AIInterface/FLAggroNormalFSM.h"
	#include "../_AIInterface/FLPartyAggroFSM.h"
#endif
	
	#include "../_AIInterface/FLArenaReaper.h"
...
//////////////////////////////////////////////////////////////////////
	...
	size_t				CObj::ms_uLinkedObjCount = 0;
...
void CObj::InsPrevNode(CObj* pObj)
{
	...
	++ms_uLinkedObjCount;
	//FLDEBUG_LOG( PROGRAM_NAME, _T("[] LINKED OBJ COUNT --> %u"), ms_uLinkedObjCount );
}
//
// Parameters 
//   pObj : 추가할 오브젝트 포인터
...
void CObj::ProcessAI()
{
	...
	{
#ifdef __WORLDSERVER
		if( m_pAIInterface->IsIdleMode() == TRUE )
		{
			if( static_cast< CMover * >( this )->TryDelete_IntoIdleMode() == TRUE  )
				return;
		}
#endif //__WORLDSERVER
	...
	{
		m_pFSM->UpdateAI( AIMSG_PROCESS, 0, 0 );
	}
#endif //__WORLDSERVER
}
...
void CObj::Render( LPDIRECT3DDEVICE9 pd3dDevice )
{
	...
	if( pModel->m_nNoEffect == 0 )
	{
		...
		else
		{
			...
			if( nLevel < 0 )	
			{
				FLERROR_LOG( PROGRAM_NAME, _T( "%d, lod lv=%d %f" ), m_dwIndex, nLevel, m_fDistCamera );
				nLevel = 0;
			}
...
{ 
	...
	if( pWorld )
	{
		...
		if( (int)vOld.x != (int)vCur.x || (int)vOld.z != (int)vCur.z )
		{
			if( m_vRemoval == D3DXVECTOR3( 0, 0, 0 ) )
			{
				...
				if( pWorld->m_cbModifyLink >= MAX_MODIFYLINK )
					FLERROR_LOG( PROGRAM_NAME, _T( "MODIFYLINK//MAX//%d" ), pWorld->m_cbModifyLink ); 
			}
		...
		if( m_dwType == OT_MOVER )
		{
			if( pWorld->m_linkMap.OnMove( this, vOldPos, vPos ) == false )
				m_vPos		= vOldPos;
		}
...
void CObj::SetAIInterface( DWORD dwAIInterface, DWORD dwState )
{
	...
	{
		case AII_MONSTER:		
			m_pAIInterface		= new CAIMonster( this );	
			//m_pFSM			= new FLArenaReaper( *static_cast< CMover * >( this ) );
			break;
		...
		case AII_METEONYKER:	m_pAIInterface	= new CAIMeteonyker( this );		break;		//sun: __METEONYKER_0608
#ifdef __AGGRO16
		case AII_AGGRO_NORMAL: 	
			{
				const DWORD	dwAggroID		= AGGRO_PROP().StringToAggroID( "NORMAL_AGGRO" );
				if( dwAggroID != INVALID_AGGRO_ID )
					m_pAIInterface			= new CAIAggroNormal( this, dwAggroID ); 	
			}
			break;
		case AII_PARTY_AGGRO_LEADER:
			{
				const DWORD	dwAggroID		= AGGRO_PROP().StringToAggroID( "SINGLE_AGGRO" );
				if( dwAggroID != INVALID_AGGRO_ID )
					m_pFSM		= new FLAggroNormalFSM( *static_cast< CMover * >( this ), dwAggroID ); 	
			}
			break;
		case AII_PARTY_AGGRO_SUB:
			{
				const DWORD	dwAggroID		= AGGRO_PROP().StringToAggroID( "SINGLE_AGGRO" );
				if( dwAggroID != INVALID_AGGRO_ID )
					m_pFSM		= new FLPartyAggroSubMonsterFSM( *static_cast< CMover * >( this ), dwAggroID ); 	
			}
			break;
#endif 
...
bool CObj::IsCCtrl() const
{
	return ( m_dwType == OT_CTRL || m_dwType == OT_MOVER || /*m_dwType == OT_OBJ ||*/ m_dwType == OT_SHIP || m_dwType == OT_SFX || m_dwType == OT_ITEM );
}

float CObj::GetModelMaxSize() const
{
	if( m_pModel == NULL || m_pModel->IsNullBoundBox() == TRUE )		// 스킬에서 쓰는 가상오브젝트때문에 모델이 없는 오브젝트도 생겼다.
		return 0.0f;
	...
	D3DXVec3Transform( &vMin, &m_pModel->m_vMin, &mWorld );	
	D3DXVec3Transform( &vMax, &m_pModel->m_vMax, &mWorld );	
	const float fMaxSize	= ( fabs(vMax.x - vMin.x) > fabs(vMax.z - vMin.z) ) ? vMax.x - vMin.x : vMax.z - vMin.z; 
	const float fRealMaxSize= fabs( fMaxSize );
	return fRealMaxSize;
}

bool CObj::IsCollitionObj() const
{
	if( m_dwType == OT_OBJ || m_dwType == OT_CTRL || m_dwType == OT_SHIP )
		return true;
