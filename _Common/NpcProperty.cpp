...
BOOL CNpcProperty::LoadDialog( LPCHARACTER lpCharacter )
{
	...
	if( n > 0 )
	{
		FLStrcpy( m_szName, _countof( m_szName ), lpCharacter->m_szDialog );
		m_szName[n] = '\0';
		FLStrLowercase( m_szName, _countof( m_szName ) );
		return TRUE;
	}
	else
		return FALSE;
	...
	FLTRACE_LOG( PROGRAM_NAME, _T( "dialog file: %s" ), lpCharacter->m_szDialog );

	m_Dialog.Free();
	return m_Dialog.LoadScript( MakePath( DIR_DIALOG, g_xFlyffConfig->GetMainLanguage(), lpCharacter->m_szDialog ) );
