...
#ifdef __RES0807
	memset( m_aResPoint, 0, sizeof(m_aResPoint) );
#endif	// __RES0807
...
CRespawnInfo::CRespawnInfo( const CRespawnInfo & ri )
{
	...
	memcpy_s( &m_CtrlElem, sizeof( m_CtrlElem ), &ri.m_CtrlElem, sizeof( m_CtrlElem ) );
...
#ifdef __RES0807
	memcpy_s( m_aResPoint, sizeof( m_aResPoint ), ri.m_aResPoint, sizeof(POINT) * MAX_RESPOINT_PER_REGION );
#endif	// __RES0807
	m_dwAI	= ri.m_dwAI;
}
...
//	FLTRACE_LOG( PROGRAM_NAME, _T( "nFail=%d" ), nFail );
	return TRUE;
}
#endif	// __RES0807
...
{	
	...
	if( nType == RESPAWNTYPE_SCRIPT )
	{
		for( VRI::iterator i = m_vRespawnInfo[nType].begin(); i != m_vRespawnInfo[nType].end(); ++i )
		{
			 ...
			 {
				FLERROR_LOG( PROGRAM_NAME, _T( "같은 ID 발견 : %d, %d, %f, %f, %d" ), ri.m_dwIndex, ri.m_dwType, ri.m_vPos.x, ri.m_vPos.z, nType );
				return -1;
			 }
		}
		m_vRespawnInfo[nType].push_back( ri );
...
BOOL CRespawner::DoRemove( int nRespawnNo, int nType )
{
	...
	if( nType == RESPAWNTYPE_REGION )
	{
		FLERROR_LOG( PROGRAM_NAME, _T( "RESPAWN_REGION 타입은 삭제할 수 없음 : %d" ), nRespawnNo );
		return FALSE;
	}
	else
	if( nType == RESPAWNTYPE_SCRIPT )
		...
		for( int i = 0; i < nSize; i++ )			// 04.10.11 - 480개 정도 이다.
		{
			...
			if( pi->m_cbTime < 0 )							// 0 이면 리스폰 시작
			{
				...
				while( cb-- > 0 )
				{
					if( pi->m_dwIndex == 0 )
					{
						FLERROR_LOG( PROGRAM_NAME, _T( "0 index" ) );
						continue;
					}
					...
					if( pObj == NULL )
					{
						FLERROR_LOG( PROGRAM_NAME, _T( "CRespawner::Spawn. CreateObj Failed - SPAWN: %d, %d" ), pi->m_dwType, pi->m_dwIndex );
						continue;
					}
					...
					if( pi->m_dwType == OT_ITEM )
					{
						...
						pItemElem->m_nItemNum	= (int)( pi->m_nItemMin + xRandom( pi->m_nItemMax - pi->m_nItemMin + 1 ) );
						...
						{
							FLTRACE_LOG( PROGRAM_NAME, _T( "CRespawner::Spawn. %d item property not found" ), pi->m_dwIndex );
					...
					{
						memcpy_s( &(((CCommonCtrl*)pObj)->m_CtrlElem), sizeof( (((CCommonCtrl*)pObj)->m_CtrlElem) ), &(pi->m_CtrlElem), sizeof( CCtrlElem ) );
					...
					if( pi->m_dwIndex != 0 ) // 구역정보를 쓴다면
						pObj->m_dwAIInterface = pi->m_dwAI;  //구역 AI로 설정 
...
lbl_retry:
					...
					( (CCtrl*)pObj )->SetRespawn( pi->m_nGMIndex, static_cast< BYTE >( nType ) );
					#ifdef __S1108_BACK_END_SYSTEM
						int nMaxAttckNum = 0;
					...
					{
						FLERROR_LOG( "RESPAWN", _T( "AddObj Failed. WorldID:[%d], ObjType:[%d], nLayer:[%d]" )
							, pWorld->GetID(), pObj->GetType(), nLayer );
						SAFE_DELETE( pObj );
					}
					else
					{
