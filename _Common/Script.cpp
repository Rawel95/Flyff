...
//
////////////////////////////////////////////////////////////////////////////////////////////
std::map<std::string, int> CScript::m_defines;
std::map<std::string, std::string>	CScript::m_mapString;
...
void CScript::LoadString( void )
{
	...
	while( tok != FINISHED )
	{
		...
		bool bResult = m_mapString.insert( std::map<std::string, std::string>::value_type( (LPCTSTR)str, (LPCTSTR)Token ) ).second;
		if( !bResult )
...
int CScript::GetToken( BOOL bComma )
{
	...
	if( tokenType == IDENTIFIER ) 
	{
		...
		{
			std::map<std::string, std::string>::iterator i = CScript::m_mapString.find( m_mszToken );
...
void CScript::ExecDefine()
{
	...
	if( m_defines.insert(std::make_pair( str, n ) ).second == false )
	{
...
BOOL CScript::LookupDefine( LPCTSTR lpszString, int& rValue )
{
	if( lpszString != NULL )
	{
		std::map< std::string, int >::iterator it = m_defines.find( lpszString );
		if( it != m_defines.end() )
		{
			rValue = it->second;
			return TRUE;
...
{
	std::map<std::string, std::string>::iterator iter = CScript::m_mapString.find( key );
