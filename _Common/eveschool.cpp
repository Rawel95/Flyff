...
// "/sbready
	...
	for( std::map<u_long, CGuild*>::iterator i = m_pSchoolMng->m_mapPGuild.begin(); i != m_pSchoolMng->m_mapPGuild.end(); ++i )
	{
		CGuild* pSchool	= i->second;
		pSchool->m_nDead	= 0;
		for( std::map<u_long, CGuildMember*>::iterator i2 = pSchool->m_mapPMember.begin(); i2 != pSchool->m_mapPMember.end(); ++i2 )
		{
...
//	"/sbreport"
	...
	int iIndex	= 0;
	for( std::map<u_long, CGuild*>::iterator i = m_pSchoolMng->m_mapPGuild.begin();
		i != m_pSchoolMng->m_mapPGuild.end(); ++i )
	{
		...
		for( std::map<u_long, CGuildMember*>::iterator i2 = pSchool->m_mapPMember.begin();
...
#ifdef __WORLDSERVER
	vecGCSendItem.clear();
	for( std::vector<__GuildCombatMember*>::iterator kItr = m_vecGuildCombatMem.begin(); kItr != m_vecGuildCombatMem.end(); ++kItr )
	{
		__GuildCombatMember* pGCMember = *kItr;
		for( std::vector<__JOINPLAYER*>::iterator kItr2 = pGCMember->vecGCSelectMember.begin(); kItr2 != pGCMember->vecGCSelectMember.end(); ++kItr2 )
		{
			SAFE_DELETE( *kItr2 );
		}
...
#ifdef __NEED_GUILDNAME
		if (strcmp (pGuild->m_szGuild,"") == 0)
		{
			pUser->AddText( "Guild Name is needed." );
			return;
		}
#endif // __NEED_GUILDNAME		
...
{
	std::vector<__REQUESTGUILD> vecGCRanking;
...
void CGuildCombat::GuildCombatOpen( void )
{
	...
	for( int veci = 0 ; veci < (int)( vecRequestRanking.size() ) ; ++veci )
	{
		...
		if( pGuild )
		{
			for( std::map<u_long, CGuildMember*>::iterator Guildi = pGuild->m_mapPMember.begin();	// neuz
...
//sun: 10, __S_BUG_GC
	for( int gcmi = 0 ; gcmi < (int)( m_vecGuildCombatMem.size() ) ; ++gcmi )
	{
		...
		{
			BOOL bInsert = FALSE;
			for( std::vector<__REQUESTGUILD>::iterator  itv = vecRequestRanking.begin() ; itv != vecRequestRanking.end() ; ++itv )
			{
...
void CGuildCombat::SetGuildCombatClose( BOOL bGM )
{
	...
	for( int nVeci = 0 ; nVeci < (int)( vecRequestRanking.size() ) ; ++nVeci )
	{
		...
		if( pGCMember != NULL )
		{
			...
			if( pGuildMsg )
			{
				for( std::map<u_long, CGuildMember*>::iterator iGuild = pGuildMsg->m_mapPMember.begin();
	...
	else
	{
		...
		if( __AutoOpen[ctime.GetDayOfWeek()-1].bUseing )
		{
				...
				__AutoOpen[ctime.GetDayOfWeek()-1].nMinute == ctime.GetMinute() && m_bMutex == FALSE )
			{
				...
				g_xWSUserManager->AddNextGuildWar(GetNextGuildWar());
#endif				
...
/*	for( std::map<u_long, __GuildCombatMember*>::iterator ita = m_GuildCombatMem.begin() ; ita != m_GuildCombatMem.end() ; ++ita )
	{
...
//sun: 10, __S_BUG_GC
	...
	if( pGuildMsg )
	{
		for( std::map<u_long, CGuildMember*>::iterator i = pGuildMsg->m_mapPMember.begin();
...
DWORD CGuildCombat::GetNextGuildWar()
{
	...
	DWORD dwNext; //= MIN(GC_TIME_MODULO - (tmNow.GetMinute() % GC_TIME_MODULO)) - SEC(tmNow.GetSecond());
