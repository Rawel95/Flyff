...
{
	if(m_strPictureFileName != _T( "" ) && m_pMapTexture == 0)	{
		CTexture* pMapTexture	= CWndBase::m_textureMng.AddTexture( g_Neuz.m_pd3dDevice, MakePath( _T( DIR_THEME ), g_xFlyffConfig->GetMainLanguage(), m_strPictureFileName ), 0xff000000 );
		SetMapTexture( pMapTexture );
	}
