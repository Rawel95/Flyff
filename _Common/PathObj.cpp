...
{
	std::map< DWORD, std::vector<_VECINFO> >::iterator it = m_mapPatrolPath.find( dwIndex );
	...
	// 새롭게 넣는다
	{
		...
		m_mapPatrolPath.insert( std::map< DWORD, std::vector<_VECINFO> >::value_type(dwIndex, pTemp) );
	}
}
...
{
	std::map< DWORD, std::vector<_VECINFO> >::iterator it1 = m_mapPatrolPath.find( pObj->m_dwPatrolIndex );
