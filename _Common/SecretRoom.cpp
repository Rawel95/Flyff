...
{
	m_mapSecretRoomContinent.insert(std::make_pair( CONT_EAST, new CSecretRoomContinent(CONT_EAST) ) );
	m_mapSecretRoomContinent.insert(std::make_pair( CONT_WEST, new CSecretRoomContinent(CONT_WEST) ) );
...
CSecretRoomMng::~CSecretRoomMng()
{
	std::map<BYTE, CSecretRoomContinent*>::iterator it = m_mapSecretRoomContinent.begin();
...
// 참가자 구성 신청
void CSecretRoomMng::SetLineUp( FLWSUser* pUser, std::vector<DWORD> vecLineUpMemeber )
{
	if( m_nState != SRMNG_LINEUP )
	{
		pUser->AddDefinedText( TID_GAME_SECRETROOM_NOLINEUP );
...
void CSecretRoomContinent::SetContLineUp( FLWSUser* pUser, std::vector<DWORD> vecLineUpMember )
{
	int nIndex = GetTenderGuild( pUser->m_idGuild );
	if( nIndex == NULL_ID )
		return;
...
void CSecretRoomContinent::SecretRoomContOpen()
{
	...
	std::vector<__SECRETROOM_TENDER>::iterator it = m_vecSecretRoomTender.begin();
...
void CSecretRoomContinent::CreateMonster()
{
	...
	for( DWORD i=0; i<m_vecSecretRoomTender.size(); i++ )
	{
		...
		std::vector<__SECRETROOM_MONSTER>::iterator it = m_vecSecretRoomTender[i].vecMonster.begin();
		if( (*it).nNum < nCount )
				nCount = (*it).nNum;
