...
{
	for( std::map<std::string, CObject3D*>::iterator i = m_mapObject3D.begin(); i != m_mapObject3D.end(); ++i )
	{
...
CObject3D		*CObject3DMng :: LoadObject3D( LPDIRECT3DDEVICE9 pd3dDevice, LPCTSTR szFileName )
{
	...
	std::map<std::string, CObject3D*>::iterator i		= m_mapObject3D.find( sFile );
	...
	bool bResult	= m_mapObject3D.insert( std::map<std::string, CObject3D*>::value_type( sFile, pObject3D ) ).second;
