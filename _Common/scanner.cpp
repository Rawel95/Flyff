...
// <strsafe.h> 로 인한 warning 차단
#pragma	warning( push )
#pragma warning( disable : 4995 )	// warning C4995: #pragma deprecated로 표시되었습니다.
#include <comdef.h>
#pragma	warning( pop )
...
const char*	CharNextEx(const char* strText, WORD /*codePage*/)
{
	...
	{
		return CharNextExA( g_xFlyffConfig->GetCodePage(), strText, 0 );
	}
}
...
{
	if( g_xFlyffConfig->GetMainLanguage() != LANG_RUS )
		return FALSE;
...
{
	int nLength;
	if(g_xFlyffConfig->GetMainLanguage() == LANG_VTN && lpMultiByteStr != NULL && cchMultiByte > 0)
	{
...
{
	int nLength;
	if(g_xFlyffConfig->GetMainLanguage() == LANG_VTN && lpWideCharStr != NULL && cchWideChar > 0)
	{
...
void CScanner::Read( const BYTE * pBuffer, const int nBufferSize )
{
	m_bMemFlag				= 0;
	m_nProgSize				= nBufferSize;

	const int nSize			= m_nProgSize + 2;
	char* pProg				= new char[ nSize ];

	m_pProg = m_pBuf = pProg;
	memcpy( m_pBuf, pBuffer, nBufferSize );
	// 0xfffe의 2바이트를 빼고 널(0x0000)을 2바이트를 추가해서 결국 m_nProgSize는 변화 없음.
	*(pProg + m_nProgSize ) = '\0';
	*(pProg + m_nProgSize + 1 ) = '\0';
...
void CScanner::WideToMultiByte( CHAR * o_pBuffer, const int nBufferSize )
{
	if( (BYTE)*(o_pBuffer + 0 ) == 0xff && (BYTE)*(o_pBuffer + 1 ) == 0xfe ) // is unicode ?
	{
		// 텍스트가 유니코드일 경우 멀티바이트로 변경 
		char* lpMultiByte = new char[ nBufferSize ];
		int nResult = WideCharToMultiByteEx( g_xFlyffConfig->GetCodePage(), 0, 
			(LPCWSTR)(o_pBuffer + 2), -1, 
			lpMultiByte, nBufferSize, 
			NULL, NULL );
		if( nResult > 0 )
		{
			lpMultiByte[nResult-1] = 0;
			memcpy( o_pBuffer, lpMultiByte, nResult );
		}
		SAFE_DELETE_ARRAY( lpMultiByte );
	}
}

BOOL CScanner::LoadMemory( const BYTE * pBuffer, const int nBufferSize )
{
	if( pBuffer == NULL || nBufferSize <= 0 )
		return FALSE;

	Read( pBuffer, nBufferSize );
	return TRUE;
}
...
{
	//gmpbigsun(20101018): m_bMemFlag == 0 이면 할당을 한경우이므로 제거
	if( !m_bMemFlag )
		Free( );

	//지역변수가 들어올수도있다는 가정하에 저장소에 넣어서 사용함.
	m_strStore = CString( (char*)pProg );
	m_bMemFlag = 1; m_pProg = m_pBuf = (CHAR*)m_strStore.GetString();
...
//			*pszCur++ = *m_pProg++;
	...
	// 변수와 명령
	if( iswalpha( static_cast<unsigned char>( *m_pProg ) ) || IsMultiByte( m_pProg ) || IsCyrillic( *m_pProg ) 
