...
#define SYNTAX_ERROR		FLERROR_LOG( PROGRAM_NAME, _T( "%s(%d) : MoverID=%d syntax error %s" ), szFileName, script.GetLineNum(), nVal, script.token );
#define UNIDENTIFY_ERROR	FLERROR_LOG( PROGRAM_NAME, _T( "%s(%d) : MoverID=%d 지정되지 않은 디파인 error %s" ), szFileName, script.GetLineNum(), nVal, script.token );
#define CUSTOM_ERROR(A)		FLERROR_LOG( PROGRAM_NAME, _T( "%s(%d) : MoverID=%d %s %s" ), szFileName, script.GetLineNum(), nVal, A, script.token );		
...
BOOL CProject::LoadPropMoverEx_AI_SCAN( LPCTSTR szFileName, CScript &script, int nVal )
{
	...
	{
		FLERROR_LOG( PROGRAM_NAME, _T( "%s(%d) : MoverID=%d SCAN 섹터블럭 { 빠졌음." ), szFileName, script.GetLineNum(), nVal );
		return FALSE;
	}
...
BOOL CProject::LoadPropMoverEx_AI_BATTLE( LPCTSTR szFileName, CScript &script, int nVal )
{
	...
	{
		FLERROR_LOG( PROGRAM_NAME, _T( "%s(%d) : MoverID=%d BATTLE 섹터블럭 { 빠졌음." ), szFileName, script.GetLineNum(), nVal );
		return FALSE;
	}
...
//					script.GetToken();
			...
			if( _strcmpi( script.token, "Helper" ) == 0 )		// 도움요청.
			{
				...
				pProp->m_tmUnitHelp		= std::numeric_limits< DWORD >::max();			// 몇초마다 한번씩 헬퍼를 부를꺼냐. 0이면 최초한번만.
				pProp->m_nHelpRangeMul	= 2;			// 디폴트는 시야반경 2배내.
				pProp->m_bHelpWho		= 1;
				//pProp->m_nCallHelperMax	= 1;			// default로 시야에 있는 아무 몬스터를 불러 들인다.
			} else
			// helper에서 쓰이는..
		...
		if( script.tokenType == NUMBER )
		{
			...
			if( nCommand == AICMD_SUMMON )		// 소환명령이 나왔었다.
			{
				...
				if( pProp->m_nSummNum > MAX_SUMMON )
				{
					...
					pProp->m_nSummNum = MAX_SUMMON;		//sun: 12, 몬스터 소환 규칙 변경
				}
			...
			{
				pProp->m_nRunawayHP = static_cast< short >( atoi( script.token ) );		// 몇%남으면 도망가냐?
			} else
			if( nCommand == AICMD_HELPER )	// 헬퍼 명령이 나왔었다.
			{
				int nNum = atoi( script.token );
				if( pProp->m_tmUnitHelp == std::numeric_limits< DWORD >::max() )		// 파라메터가 아직 지정되지 않았다면..
...
BOOL CProject::LoadPropMoverEx_AI_MOVE( LPCTSTR szFileName, CScript &script, int nVal )
{
	...
	{
		FLERROR_LOG( PROGRAM_NAME, _T( "%s(%d) : MoverID=%d MOVE 섹터블럭 { 빠졌음." ), szFileName, script.GetLineNum(), nVal );
		return FALSE;
	}
...
BOOL CProject::LoadPropMoverEx_AI( LPCTSTR szFileName, CScript &script, int nVal )
{
	...
	if( script.token[0] != '{' )
	{
		FLERROR_LOG( PROGRAM_NAME, _T( "%s(%d) : MoverID=%d AI 다음에 { 없음." ), szFileName, script.GetLineNum(), nVal );
		return FALSE;
	}
...
//			script.GetToken();	// SCAN/BATTLE/MOVING
			...
			{
				FLERROR_LOG( PROGRAM_NAME, _T( "%s(%d) : MoverID=%d 섹터블럭이름이 이상함(%s)" ), szFileName, script.GetLineNum(), nVal, script.Token );
				return FALSE;
			}
		} else
		{
			// 섹터블럭이 없음.
			FLERROR_LOG( PROGRAM_NAME, _T( "%s(%d) : MoverID=%d 섹터블럭(<)이 없음." ), szFileName, script.GetLineNum(), nVal );
			return FALSE;
		}
	}
