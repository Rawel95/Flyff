...
//	LPSCRIPT		Lookup(int nId);
	...
	{	
		FLStrcpy( s_pszName, _countof( s_pszName ), pszName );	FLStrcpy( s_pszKey, _countof( s_pszKey ), pszKey );	
	}
	static	void	SetBeforeRun( const char* pszBeforeRun )
	{	
		FLStrcpy( s_pszBeforeRun, _countof( s_pszBeforeRun ), pszBeforeRun );	
	}
	static	void	SetAfterRun( const char* pszAfterRun )
	{	
		FLStrcpy( s_pszAfterRun, _countof( s_pszAfterRun ), pszAfterRun );	
	}
