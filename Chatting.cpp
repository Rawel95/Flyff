...
MEMPOOLER_IMPLEMENT( CChatting, 512 );
...
BOOL CChatting::AddChattingMember( u_long uidPlayer, char * szName )
{
	if( IsChattingMember( uidPlayer ) == FALSE && m_nSizeofMember < MAX_CHATTINGMEMBER )
	{
		...
		FLStrcpy( m_szName[m_nSizeofMember], _countof( m_szName[m_nSizeofMember] ), szName );
		m_nSizeofMember++;
		return TRUE;
	}
	return FALSE;
...
BOOL CChatting::RemoveChattingMember( u_long uidPlayer )
{
	...
	if( nFindMember >= 0 )
	{
		for( int i = nFindMember ; i < m_nSizeofMember-1 ; ++i )
		{
			...
			FLStrcpy( m_szName[i], _countof( m_szName[i] ), m_szName[i+1] );
		}
		m_nSizeofMember--;
		return TRUE;
	}
