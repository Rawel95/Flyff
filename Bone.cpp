...
{
//sun: 12, // __LEAK_0827
	for( std::map<std::string, CBones*>::iterator i = m_mapBones.begin(); i != m_mapBones.end(); ++i )
...
//sun: 12, // __LEAK_0827
	...
	std::map<std::string, CBones*>::iterator i	= m_mapBones.find( sFile );
	if( i != m_mapBones.end() )
		return i->second;
	...
	bool bResult	= m_mapBones.insert( std::map<std::string, CBones*>::value_type( sFile, pBones ) ).second;
	if( bResult == false )
	{
		SAFE_DELETE( pBones );
		return NULL;
...
{
//sun: 12, // __LEAK_0827
	for( std::map<std::string, CMotion*>::iterator i = m_mapMotions.begin(); i != m_mapMotions.end(); ++i )
		SAFE_DELETE( i->second );
	m_mapMotions.clear();
}
...
//sun: 12, // __LEAK_0827
	...
	std::map<std::string, CMotion*>::iterator i	= m_mapMotions.find( sFile );
	...
	bool bResult	= m_mapMotions.insert( std::map<std::string, CMotion*>::value_type( sFile, pMotion ) ).second;
	if( bResult == false )
	{
		SAFE_DELETE( pMotion );
		return NULL;
