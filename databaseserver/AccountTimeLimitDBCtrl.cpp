...
void CAccountTimeLimitDBCtrl::Handler( LPDB_OVERLAPPED_PLUS pov, ULONG_PTR dwCompletionKey )
{
	...
	switch( pov->nQueryMode )
	{
		case QUERY_TIMELIMIT_LOAD:
		{
			...
			if( nPlayTime < 0 )
				FLERROR_LOG( PROGRAM_NAME, _T( "PlayTime is wrong - PlayerId : %d, PlayTime : %d" ), dwPlayerId, nPlayTime );
...
CAccountTimeLimitDBMng::CAccountTimeLimitDBMng()
{
	if( !m_AccTimeLimitDBDtrl.CreateDbHandler( MIN(1) ) )
		FLERROR_LOG( PROGRAM_NAME, _T( "m_HousingDBCtrl.CreateDbHandler()" ) );
}
