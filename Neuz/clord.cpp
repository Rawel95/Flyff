...
{	// 투표 종료
	...
	{
		// OOO님이 군주로 선출되었습니다. 
		FLSPrintf( lpString, _countof( lpString ), prj.GetText( TID_GAME_ELECTION_CAPTION_END_VOTE_02 ), CPlayerDataCenter::GetInstance()->GetPlayerString( idPlayer ) );
		PrintCaption( lpString );
		// OOO님이 선출되어, 앞으로 2주 동안 군주로 집권을 하게 됩니다.
		FLSPrintf( lpString, _countof( lpString ), prj.GetText( TID_GAME_ELECTION_END_VOTE_S000 ), CPlayerDataCenter::GetInstance()->GetPlayerString( idPlayer ) );
		dwColor		= prj.GetTextColor( TID_GAME_ELECTION_END_VOTE_S000 );
	}
	else
	{
		// 이번 군주 선거는 무효가 되어, 2주 동안 군주가 없습니다. 
		FLSPrintf( lpString, _countof( lpString ), prj.GetText( TID_GAME_ELECTION_END_VOTE_S001 ) );
...
{	// 군주 입찰
	if( election::IsActivePlayer( idPlayer ) )
	{
		...
		char szText[260]	= { 0,};
		if( iOld )
			FLSPrintf( szText, _countof( szText ), prj.GetText( TID_GAME_ELECTION_ADD_DEPOSIT_S001 ), iOld, iDeposit, iOld + iDeposit );
		else
			FLSPrintf( szText, _countof( szText ), prj.GetText( TID_GAME_ELECTION_ADD_DEPOSIT_S002 ), iDeposit );
...
{	// 선거 상태 출력
	...
	FLSPrintf( lpString, _countof( lpString ), "state: %d", eState );
	g_WndMng.PutString( lpString, NULL, 0xff00ff00 );
	FLSPrintf( lpString, _countof( lpString ), "candidates list" );
	g_WndMng.PutString( lpString, NULL, 0xff00ff00 );
	for( int i = 0; i < (int)( m_vCandidates.size() ); i++ )
	{
		SPC ptr		= m_vCandidates[i];
		FLSPrintf( lpString, _countof( lpString ), "%02d: %07d: %I64d: %s: %d: %d", i, ptr->GetIdPlayer(), ptr->GetDeposit(), ptr->GetPledge(), ptr->GetVote(), ptr->GetCreate() );
		g_WndMng.PutString( lpString, NULL, 0xff00ff00 );
	}
#endif	// _DEBUG
}
...
{	// 이벤트 시작 화면 출력
	...
	{
		FLStrcpy( lpCaption, _countof( lpCaption ), prj.GetText( TID_GAME_L_EVENT_CREATE_S001 ) );
		FLSPrintf( lpString, _countof( lpString ), prj.GetText( TID_GAME_L_EVENT_CREATE_S004 ), CPlayerDataCenter::GetInstance()->GetPlayerString( m_pLord->Get() ), pComponent->GetEFactor() * 100, pComponent->GetIFactor() * 100 );
	}
	else if( pComponent->GetEFactor() > 0.0F )
	{
		FLStrcpy( lpCaption, _countof( lpCaption ), prj.GetText( TID_GAME_L_EVENT_CREATE_S002 ) );
		FLSPrintf( lpString, _countof( lpString ), prj.GetText( TID_GAME_L_EVENT_CREATE_S005 ), CPlayerDataCenter::GetInstance()->GetPlayerString( m_pLord->Get() ), pComponent->GetEFactor() * 100 );
	}
	else if( pComponent->GetIFactor() > 0.0F )
	{
		FLStrcpy( lpCaption, _countof( lpCaption ), prj.GetText( TID_GAME_L_EVENT_CREATE_S003 ) );
		FLSPrintf( lpString, _countof( lpString ), prj.GetText( TID_GAME_L_EVENT_CREATE_S006 ), CPlayerDataCenter::GetInstance()->GetPlayerString( m_pLord->Get() ), pComponent->GetIFactor() * 100 );
	}
