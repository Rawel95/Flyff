...
{
	FLTRACE_LOG( PROGRAM_NAME, _T( "OnSurrender" ) );
	...
	char lpString[128]	= { 0, };
	FLSPrintf( lpString, _countof( lpString ), prj.GetText( TID_GAME_GUILDWARGIVEUP ), pGuild->m_szGuild, sPlayer );
...
{
	FLTRACE_LOG( PROGRAM_NAME, _T( "OnQueryTruce" ) );
...
{
	FLTRACE_LOG( PROGRAM_NAME, _T( "OnDeclWar" ) );
	
	u_long idDecl;
	char szMaster[MAX_PLAYER];
	ar >> idDecl;
	ar.ReadString( szMaster, _countof( szMaster ) );
...
{
	FLTRACE_LOG( PROGRAM_NAME, _T( "OnAcpWar" ) );
	...
	if( idWar > 0 )
	{
		...
		char lpString[128]	= { 0, };
		FLSPrintf( lpString, _countof( lpString ), prj.GetText( TID_GAME_GUILDWARSTART ), pDecl->m_szGuild, pAcpt->m_szGuild );
...
{
	FLTRACE_LOG( PROGRAM_NAME, _T( "OnSetWar" ) );
...
{
	FLTRACE_LOG( PROGRAM_NAME, _T( "OnWar" ) );
	...
	if( pDecl && pAcpt )
	{
		FLSPrintf( lpString, _countof( lpString ), prj.GetText( TID_GAME_GUILDWARSTILL ), pDecl->m_szGuild, pAcpt->m_szGuild );
...
{
	FLTRACE_LOG( PROGRAM_NAME, _T( "OnWarDead" ) );
	...
	if( pWar )
	{
		if( bDecl )
		{
			...
			if( pGuild )
			{
				FLSPrintf( lpString, _countof( lpString ), prj.GetText( TID_GAME_GUILDWARDEAD ), pGuild->m_szGuild, sPlayer );
...
{
	FLTRACE_LOG( PROGRAM_NAME, _T( "OnWarEnd" ) );
