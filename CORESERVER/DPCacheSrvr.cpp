...
void CDPCacheSrvr::SendFriendState( CPlayer* pTo )
{
	...
	pTo->Lock();
	for( std::map<u_long, Friend>::iterator i = pTo->m_RTMessenger.begin(); i != pTo->m_RTMessenger.end(); ++i )
	{
...
{
	for( std::map<u_long, CGuildMember*>::iterator i = pDecl->m_mapPMember.begin();	// neuz
...
void CDPCacheSrvr::OnAcptWar( CAr & ar, DPID /*dpidCache*/, DPID dpidUser, u_long /*uBufSize*/ )
{
	...
	if( idWar > 0 )
	{
		...
		for( std::map<u_long, CGuildMember*>::iterator i = pDecl->m_mapPMember.begin(); i != pDecl->m_mapPMember.end(); ++i )
		{
