...
void CPlayerMng::Free()
{
	...
	std::map<DWORD, CPlayer*>::iterator it;
...
BOOL CPlayerMng::AddPlayer( u_long idPlayer, const CHAR* lpszPlayer, const CHAR* lpszAccount )
{
	...
	m_ulong2.insert( std::map<u_long, CPlayer*>::value_type( idPlayer, pPlayer ) );
	return TRUE;
}
...
BOOL CPlayerMng::UnregisterPlayerInfo( CPlayer* pPlayer, BOOL bNotify )
{
	...
	for( std::map<u_long, Friend>::iterator i = pPlayer->m_RTMessenger.begin(); i != pPlayer->m_RTMessenger.end(); ++i )
	{
...
{
	m_players.insert(std::make_pair( pPlayer->m_dwSerial, pPlayer ) );
	m_uCount++;
...
{
	std::map< DWORD, CPlayer* >::iterator it = m_players.find( dwSerial );
	if( it != m_players.end() )
		return it->second;
...
{
		CMclAutoLock	Lock( m_AddRemoveLock );
		std::set<u_long>::iterator i	= m_set.find( idPlayer );
		...
		//
		{
			...
			if( pGuild && pGuild->IsMember( idPlayer ) )
			{
				for( std::map<u_long, CGuildMember*>::iterator i = pGuild->m_mapPMember.begin(); i != pGuild->m_mapPMember.end(); ++i )
					mapIdPlayer.insert( std::map<u_long, u_long>::value_type( i->second->m_idPlayer, 0 ) );
			}
		}
		//
		{
			...
			if( pParty && pParty->IsMember( idPlayer ) )
			{
				for( int i = 0; i < pParty->m_nSizeofMember; i++ )
					mapIdPlayer.insert( std::map<u_long, u_long>::value_type( pParty->m_aMember[i].m_uPlayerId, 0 ) );
			}
		}
		//
		{
			pPlayer->Lock();
			for( std::map<u_long, Friend>::iterator i = pPlayer->m_RTMessenger.begin(); i != pPlayer->m_RTMessenger.end(); ++i )
				mapIdPlayer.insert( std::map<u_long, u_long>::value_type( i->first, 0 ) );
			pPlayer->Unlock();
		}
		for( std::map<u_long, int>::iterator i = mapIdPlayer.begin(); i != mapIdPlayer.end(); ++i )
		{
