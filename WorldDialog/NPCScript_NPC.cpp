...
void CNpcScript::dudk_drian_6()
{
	...
	{ 
		if( GetEmptyInventoryNum( II_SYS_SYS_KEY_NEVILPRISON ) < 1 )
		{
...
void CNpcScript::mada_bernard_6()
{
	...
	{ 
		if( GetEmptyInventoryNum( II_SYS_SYS_QUE_WAKEROACHWATER ) < 1)
		{
			PrintSystemMessage(TID_GAME_LACKSPACE);
			Exit();
		}
...
{
	Say( 343 );
	if( GetEmptyInventoryNum( II_SYS_SYS_QUE_LETTERRANKASHU ) < 1 )
	{
		PrintSystemMessage(TID_GAME_LACKSPACE);
		Exit();
	}
