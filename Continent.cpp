...
BYTE CContinent::GetContinent( const D3DXVECTOR3& vPos )
{
	...
	for( std::map< BYTE, std::vector<CPoint> >::iterator iter = m_MapCont.begin(); iter != m_MapCont.end(); ++iter )
	{
...
BYTE CContinent::GetTown( const D3DXVECTOR3& vPos )
{
	...
	for( std::map< BYTE, std::vector<CPoint> >::iterator iter = m_MapContTown.begin(); iter != m_MapContTown.end(); ++iter )
	{
		vecPoint  = iter->second;	
		if( Point_In_Poly( vecPoint, cpoint ) )
			return iter->first;
