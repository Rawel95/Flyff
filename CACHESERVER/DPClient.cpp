...
BOOL	CDPClientArray::Connect( CServerDesc *pServer )
{
	...
	if( i == m_apServer.end() )
	{
		...
		if( pClient->ConnectToServer( pServer->m_szAddr, PN_WORLDSRVR + (u_short)pServer->GetKey(), FLNormalProtocol::GetInstance() ) == TRUE )
		{
...
CDPClient* CDPClientArray::GetClient( u_long uIdofMulti, DWORD /*dwWorldID*/, const vector3 & /*vPos*/ )
{
